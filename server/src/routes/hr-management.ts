  ...
  try {
    ...
    if (category?.categoryType === "datos_personales") {
      try {
        ...
        const proposed = (order.metadata as any)?.proposedUserData || {};
...
router.put("/orders/:id/confirmar-cambio-bancario", async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
    ...
    if (order.userId) {
      await User.updateOne(
        { _id: order.userId, tenantId: req.tenantObjectId },
