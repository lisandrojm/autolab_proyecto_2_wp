 ...
 */
router.post("/asignar-sindicatos", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { asignaciones } = req.body as { asignaciones?: Array<{ convenioId?: string; sindicatoId?: string | null }> };
    ...
    const result = await Convenio.bulkWrite(ops.filter(Boolean) as never[]);
    res.json({
