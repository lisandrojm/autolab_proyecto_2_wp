      ...
      */
      ...
      if (permisosAntes.has(MOBILE_ACTIVITY_LOGS) && !permisosDespues.has(MOBILE_ACTIVITY_LOGS)) {
        ...
        if (proyectos.length > 0) {
          res.status(409).json({
            error: `Se quedaría sin el permiso "APP MOBILE | Cargar novedades" y coordina turnos en ${proyectos.map((p: any) => p.name).join(", ")}. Liberá esas coordinaciones desde el equipo del proyecto y volvé a intentar.`,
          });
          return;
        }
      }
