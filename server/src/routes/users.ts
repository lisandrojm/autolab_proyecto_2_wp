...
async function resolveProjectTeamFilterIds(projectId: string, filtros: TeamFilters): Promise<Types.ObjectId[]> {
  ...
  */
  const esCoordinador = (m: any): boolean => {
    if (permisosDeRoles(m.roles).has(PROJECT_COORDINATOR)) return true;
...
router.post("/", requireTenant, authenticateToken, permisoParaCrearUsuario, async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
    ...
    */
    const renovacionDe: any = (user.metadata as any)?.esRenovacion ? (user.metadata as any)?.renovacionDe : null;
    if (renovacionDe?.userProjectId && renovacionDe?.fechaBajaContrato) {
      try {
        const up: any = await UserProject.findById(renovacionDe.userProjectId).select("userId projectId").lean();
        if (up) {
          const quien: any = await User.findById(req.user!.userId).select("firstName lastName").lean();
          await RenovacionContrato.updateOne(
            { tenantId: req.tenantObjectId, userProjectId: up._id, fechaBajaContrato: String(renovacionDe.fechaBajaContrato) },
            {
              $set: {
                userId: up.userId,
    ...
    */
    if ((user.metadata as any)?.isSolicitud === true) {
      // Los proyectos de una solicitud vienen en `metadata`; los de un alta normal, en la raíz.
      const proyectos = [...(((user.metadata as any)?.projectIds || []) as any[]), ...((user.projectIds || []) as any[])];
      await notificar({
        tenantId: req.tenantObjectId!,
 ...
 */
const consultarDirectorio = async (tenantObjectId: any, status?: string) => {
 ...
 */
const directorioEnVuelo = new Map<string, Promise<any>>();

const traerDirectorio = (cacheKey: string, tenantObjectId: any, status?: string): Promise<any> => {
...
*/
router.get("/:id/contracts", requireTenant, authenticateToken, requirePermission("admin_users:view"), async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
    const { id } = req.params;
    ...
    const vinculo: any = await UserProject.findOne({ userId: id, projectId }).select("contracts nombre_rol_frame").lean();
...
router.patch("/:id", requireTenant, authenticateToken, permisoSobreSolicitudPropia({ incluirRechazadas: true }), async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
      ...
      */
      ...
      if (permisosAntes.has(MOBILE_ACTIVITY_LOGS) && !permisosDespues.has(MOBILE_ACTIVITY_LOGS)) {
        ...
        if (proyectos.length > 0) {
          res.status(409).json({
            error: `Se quedaría sin el permiso "APP MOBILE | Cargar novedades" y supervisa turnos en ${proyectos.map((p: any) => p.name).join(", ")}. Liberá esas supervisiones desde el equipo del proyecto y volvé a intentar.`,
          });
          return;
        }
      }
    ...
    */
    if ((req as any).solicitudPropiaDelMovil) {
...
router.patch("/:id/solicitud-status", requireTenant, authenticateToken, permisoSobreSolicitudPropia({ soloCancelar: true }), async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
    ...
    */
    const cambio: any = { "metadata.solicitudStatus": status };
    ...
    */
    const avisoPorEstado: Record<string, { type: string; title: string; message: string }> = {
    ...
    if (aviso) {
      const m: any = user.metadata || {};
      const aprobadores = status === "pendiente" ? await responsablesDeProyectos(req.tenantObjectId!, [...((m.projectIds || []) as any[]), ...((user.projectIds || []) as any[])]) : [];
      await notificar({
        tenantId: req.tenantObjectId!,
        destinatarios: [m.solicitudCreadaPor, ...aprobadores],
        type: aviso.type,
...
router.delete("/:id/solicitud", requireTenant, authenticateToken, permisoSobreSolicitudPropia({ incluirCerradas: true }), async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
    ...
    const solicitud: any = await User.findOne({ _id: id, tenantId: req.tenantObjectId }).select("firstName lastName email metadata.fullName metadata.isSolicitud metadata.solicitudStatus metadata.solicitudUserId metadata.projectIds metadata.startDate metadata.dueDate").lean();
    if (!solicitud) {
      res.status(404).json({ error: "Solicitud no encontrada" });
      return;
    }
    ...
    let contrato: Awaited<ReturnType<typeof borrarContratoDeSolicitud>> | undefined;
      ...
      if (esLaPersona) {
        await Promise.all([
          User.updateOne(
            ...
            { $unset: { "metadata.solicitudStatus": "", "metadata.solicitudCreadaPor": "", "metadata.solicitudUserId": "", "metadata.solicitudMotivoRechazo": "", "metadata.solicitudRechazadaPor": "", "metadata.solicitudRechazadaEl": "" } },
          ),
