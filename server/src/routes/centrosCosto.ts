...
const celda = (fila: Record<string, unknown>, encabezado: string): unknown => {
  const buscado = encabezado.toLowerCase().replace(/[\s_]/g, "");
  for (const clave of Object.keys(fila)) {
    if (clave.toLowerCase().replace(/[\s_]/g, "") === buscado) return fila[clave];
  }
  return undefined;
};
 ...
 */
const validarUno = (body: any): string | null => {
  const cod = String(body?.codAuxiliar ?? body?.nombre ?? body?.name ?? "").trim();
  if (!cod) return "El código (COD_AUXILIAR) es obligatorio: es el número del centro y es lo que se muestra.";
  const idCrudo = body?.idAuxiliar;
  if (idCrudo !== undefined && idCrudo !== null && String(idCrudo).trim() !== "") {
    const id = Number(idCrudo);
    if (!Number.isInteger(id) || id <= 0) return "El ID de Tango (ID_AUXILIAR) tiene que ser un número entero mayor a 0.";
  }
...
const camposDesdeBody = (body: any) => {
  const cod = String(body?.codAuxiliar ?? body?.nombre ?? body?.name ?? "").trim();
  const id = body?.idAuxiliar === undefined || body?.idAuxiliar === null || String(body.idAuxiliar).trim() === "" ? undefined : Number(body.idAuxiliar);
  const hab = body?.habilitado === undefined || body?.habilitado === null || String(body.habilitado).trim() === "" ? undefined : (String(body.habilitado).trim().toUpperCase() as "S" | "N");
  return {
    codAuxiliar: cod,
...
const buscarChoque = async (campos: { idAuxiliar?: number; codAuxiliar: string }, excluirId?: string): Promise<string | null> => {
  const o: any[] = [];
  if (campos.idAuxiliar !== undefined) o.push({ idAuxiliar: campos.idAuxiliar });
  if (campos.codAuxiliar) o.push({ codAuxiliar: campos.codAuxiliar });
  if (o.length === 0) return null;
  const filtro: any = { $or: o };
  if (excluirId) filtro._id = { $ne: excluirId };
  const existente: any = await CentroCosto.findOne(filtro).select("idAuxiliar codAuxiliar").lean();
  if (!existente) return null;
  if (campos.idAuxiliar !== undefined && Number(existente.idAuxiliar) === campos.idAuxiliar) return `Ya hay un centro de costo con el ID de Tango ${campos.idAuxiliar} (${existente.codAuxiliar || "sin código"}).`;
...
router.post("/", requireTenant, authenticateToken, requirePermission(PERMISO), async (req: AuthenticatedRequest, res: Response) => {
  try {
...
// PUT /:id - edición.
router.put("/:id", requireTenant, authenticateToken, requirePermission(PERMISO), async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
      res.status(400).json({ error: "Id inválido." });
      return;
    }
    const doc: any = await CentroCosto.findById(req.params.id);
    if (!doc) {
    ...
    if (campos.descAuxiliar !== undefined) doc.descAuxiliar = campos.descAuxiliar;
    if (campos.idAuxiliar !== undefined) doc.idAuxiliar = campos.idAuxiliar;
    if (campos.habilitado !== undefined) doc.habilitado = campos.habilitado;
    await doc.save();
...
router.post("/import", requireTenant, authenticateToken, requirePermission(PERMISO), upload.single("file"), async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.file) {
    ...
    const filas = xlsx.utils.sheet_to_json<any>(workbook.Sheets[workbook.SheetNames[0]]);
    if (filas.length === 0) {
...
async function upsertPorIdAuxiliar(items: ItemCentroCosto[]): Promise<{ creados: number; actualizados: number }> {
  ...
  const r: any = await CentroCosto.bulkWrite(ops, { ordered: false });
...
*/
router.post("/import-json", requireTenant, authenticateToken, requirePermission(PERMISO), async (req: AuthenticatedRequest & TenantRequest, res: Response) => {
  try {
    ...
    const proyectos: any[] = remapear ? await Project.find({ tenantId: req.tenantObjectId, "metadata.centroCostoId": { $gt: 0 } }).select("name metadata.centroCostoId metadata.centroCostoOrigen").lean() : [];
    ...
    */
    let borrados = 0;
    const documentos = items.map((i) => sincronizarCamposDerivados({ ...i }) as any);
...
*/
router.use(
  createSimpleCatalogRouter(CentroCosto, {
