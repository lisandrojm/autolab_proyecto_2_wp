...
const aBooleano = (v: unknown): boolean | undefined => (v === true || v === "true" ? true : v === false || v === "false" ? false : undefined);
const esRotulo = (v: unknown): boolean => (ROTULOS_CBU as readonly unknown[]).includes(v);
...
router.post("/", authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const ultimo: any = await Info.findOne({ type: TIPO_INFO }).sort({ "data.id": -1 }).select("data.id").lean();
...
router.patch("/:id", authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    for (const campo of ["activo", "pideTipoCuenta", "pideNroCuenta"]) {
      ...
      if (b !== undefined) set[`data.${campo}`] = b;
    }
...
router.delete("/:id", authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const doc: any = await Info.findOne({ _id: req.params.id, type: TIPO_INFO }).lean();
    if (!doc) {
      res.status(404).json({ error: "Tipo no encontrado" });
      return;
    }
