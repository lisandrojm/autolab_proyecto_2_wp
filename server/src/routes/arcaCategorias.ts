...
router.get("/convenios", authenticateToken, async (_req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const acc = new Map<string, { convenio: string; nombre: string; grupos: number; categorias: number; ultimaActualizacion: Date | string | null }>();
    const tocar = (cct: string) => {
      if (!acc.has(cct)) acc.set(cct, { convenio: cct, nombre: nombres.get(cct) || "", grupos: 0, categorias: 0, ultimaActualizacion: null });
      return acc.get(cct)!;
    };
