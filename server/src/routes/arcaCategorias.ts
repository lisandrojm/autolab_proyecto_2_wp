...
const aCodigoArca = (v: any): string => String(v ?? "").replace(/\D/g, "").padStart(6, "0");

/** Un código es real si tiene 6 dígitos y no es todo ceros. "0" y "" son la ausencia de código. */
const codigoArcaValido = (v: string): boolean => /^\d{6}$/.test(v) && v !== "000000";
 ...
 */
const escalaDelBody = (body: any): Record<string, any> => {
  ...
  if (body.fechaActualizacion !== undefined) set.fechaActualizacion = body.fechaActualizacion;
  return set;
};

/** Nombre del CCT según el catálogo de Convenios; "" si el código no está cargado ahí. */
const nombresDeConvenio = async (): Promise<Map<string, string>> => {
  const convenios = await Convenio.find().lean();
  return new Map(convenios.map((c: any) => [String(c.externalId || "").trim(), String(c.name || "")]));
};
 ...
 */
const contratosPorLegacyId = async (): Promise<Map<number, number>> => {
  ...
  for (const f of filas as any[]) {
    ...
    if (Number.isFinite(id)) uso.set(id, f.total);
  }
  return uso;
};
 ...
 */
router.get("/convenios", authenticateToken, async (_req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const acc = new Map<string, { convenio: string; nombre: string; grupos: number; categorias: number; ultimaActualizacion: Date | string | null; registrado: boolean }>();
    const tocar = (cct: string) => {
      if (!acc.has(cct)) acc.set(cct, { convenio: cct, nombre: nombres.get(cct) || "", grupos: 0, categorias: 0, ultimaActualizacion: null, registrado: false });
      return acc.get(cct)!;
    };
     ...
     */
    const porId = new Map((conveniosCat as any[]).map((c) => [String(c._id), c]));
    for (const e of empresas as any[]) {
      for (const id of e.convenioIds || []) {
        ...
        if (!item.nombre) item.nombre = String(cv?.name || "");
      }
    }
    ...
    for (const g of grupos as any[]) {
      ...
      const e = tocar(cct);
      e.grupos++;
      if (g.fechaActualizacion && (!e.ultimaActualizacion || new Date(g.fechaActualizacion) > new Date(e.ultimaActualizacion))) e.ultimaActualizacion = g.fechaActualizacion;
    }
 ...
 */
router.get("/huerfanas", authenticateToken, async (_req: AuthenticatedRequest, res: Response) => {
  try {
    const [cats, uso] = await Promise.all([Categoria.find().lean(), contratosPorLegacyId()]);

    const huerfanas = (cats as any[])
...
router.get("/", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!convenio) return res.status(400).json({ error: "Falta el convenio: las categorías se leen dentro de un convenio" });
    ...
    const porGrupoId = new Map((grupos as any[]).map((g) => [String(g._id), g]));
 ...
 */
router.post("/grupos", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!Number.isFinite(numero)) return res.status(400).json({ error: "El número de grupo es obligatorio" });
 ...
 */
router.put("/grupos/:id", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const grupo = await ConvenioGrupo.findById(req.params.id);
    if (!grupo) return res.status(404).json({ error: "Grupo salarial no encontrado" });

    const set = escalaDelBody(req.body);
    if (req.body.numero !== undefined) {
      const numero = Number(req.body.numero);
      if (!Number.isFinite(numero)) return res.status(400).json({ error: "Número de grupo inválido" });
    ...
    if (Object.keys(set).length === 0) return res.status(400).json({ error: "No se proporcionaron valores para actualizar" });
 ...
 */
router.delete("/grupos/:id", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const cuantas = await Categoria.countDocuments({ grupoId: req.params.id });
    if (cuantas > 0) return res.status(409).json({ error: `El grupo tiene ${cuantas} categoría(s): movelas o eliminalas antes.` });

    const result = await ConvenioGrupo.deleteOne({ _id: req.params.id });
    if (result.deletedCount === 0) return res.status(404).json({ error: "Grupo salarial no encontrado" });
...
const responderError = (res: Response, error: any, contexto: string) => {
  if (error instanceof DatoInvalido) return res.status(400).json({ error: error.message });
...
const resolverGrupo = async (convenio: string, body: any) => {
  if (body.grupoId) {
    const g = await ConvenioGrupo.findById(body.grupoId);
    if (!g) throw new DatoInvalido("El grupo salarial indicado no existe");
    if (String(g.convenio).trim() !== convenio) throw new DatoInvalido(`El grupo ${g.numero} es del convenio ${g.convenio}, no de ${convenio}`);
    return g;
  }
  /*
  ...
  */
  if (body.numeroGrupo === undefined || body.numeroGrupo === null || String(body.numeroGrupo).trim() === "") return null;
  const numero = Number(body.numeroGrupo);
  if (!Number.isFinite(numero)) throw new DatoInvalido("El grupo salarial tiene que ser un número, o venir vacío si el convenio no tiene grupos");
  const g = await ConvenioGrupo.findOne({ convenio, numero });
  if (!g) throw new DatoInvalido(`El convenio ${convenio} no tiene un grupo ${numero}: creá primero el grupo con su escala`);
  return g;
};
...
router.post("/", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!codigoArcaValido(codigoArca)) return res.status(400).json({ error: 'El código de ARCA son 6 dígitos y no puede ser 0 (ej. "035283")' });

    const yaExiste = await Categoria.findOne({ convenio, codigoArca });
    if (yaExiste) return res.status(409).json({ error: `El convenio ${convenio} ya tiene la categoría ${codigoArca} ("${yaExiste.nombre}")` });
...
router.put("/:id", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (req.body.codigoArca !== undefined) {
      ...
      if (choque) return res.status(409).json({ error: `El convenio ${convenio} ya tiene la categoría ${codigoArca} ("${choque.nombre}")` });
    ...
    */
    ...
    if (cambioConvenio || req.body.grupoId !== undefined || req.body.numeroGrupo !== undefined) {
      const grupo = await resolverGrupo(convenio, req.body);
      item.grupoId = (grupo ? grupo._id : null) as any;
    }
    ...
    const { nombre: _descartado, ...escalaPropia } = escalaDelBody(req.body);
    for (const [k, v] of Object.entries(escalaPropia)) (item as any)[k] = v;
...
router.delete("/:id", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if ((item as any).legacyId != null) {
      const uso = (await contratosPorLegacyId()).get(Number((item as any).legacyId)) || 0;
      if (uso > 0) {
        return res.status(409).json({ error: `No se puede eliminar: ${uso} contrato(s) usan "${(item as any).nombre}". Desactivala en su lugar (deja de ofrecerse en contratos nuevos y los existentes siguen resolviendo).`, contratos: uso });
      }
    }
 ...
 */
router.get("/plantilla", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const convenio = String(req.query.convenio || "").trim();
    if (!convenio) return res.status(400).json({ error: "Falta el convenio: la plantilla es de un convenio" });
 ...
 */
router.post("/importar", authenticateToken, upload.single("file"), async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.file) return res.status(400).json({ error: "Debe subir un archivo de Excel" });

    const workbook = xlsx.read(req.file.buffer, { type: "buffer" });
    const rows = xlsx.utils.sheet_to_json<any>(workbook.Sheets[workbook.SheetNames[0]]);
    if (rows.length === 0) return res.status(400).json({ error: "El archivo de Excel está vacío" });
    ...
    const items: Array<{ convenio: string; grupo: number | null; codigoArca: string; escala: Record<string, any> }> = [];
