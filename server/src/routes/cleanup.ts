 ...
 * 5. Delete "bad" roles.
 */
router.post("/cleanup-duplicates", requireTenant, authenticateToken, requirePermission("admin_roles:view"), async (req: any, res: any) => {
  try {
    const tenantId = req.tenantObjectId;
    const { dryRun } = req.body;

    const log: string[] = [];
    const report: any = { movedUsers: 0, deletedRoles: 0, details: [] };
