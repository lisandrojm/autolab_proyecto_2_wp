 ...
 */
async function alimentarDiccionarioActividades(sucursales: Array<{ actividades: Array<{ codigo: string; descripcion: string }> }>): Promise<number> {
  try {
    const porCodigo = new Map<string, string>();
    for (const s of sucursales) {
      for (const a of s.actividades) {
        if (!a.codigo) continue;
        if (!porCodigo.get(a.codigo)) porCodigo.set(a.codigo, (a.descripcion || "").trim());
      }
    }
    if (porCodigo.size === 0) return 0;

    const existentes = new Set((await ArcaActividad.find({ externalId: { $in: [...porCodigo.keys()] } }).select("externalId").lean()).map((a: any) => String(a.externalId)));
