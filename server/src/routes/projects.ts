...
async function filtroBusquedaProyecto(q: string): Promise<Record<string, unknown>> {
  ...
  const centros = await CentroCosto.find({ $or: [{ codAuxiliar: regex }, { descAuxiliar: regex }, { name: regex }, { "data.nombre": regex }] })
    ...
    .lean();
  const ids = (centros as any[]).map((c) => Number(c?.idAuxiliar ?? c?.data?.id)).filter((n) => Number.isFinite(n));
...
/**
 * EL CENTRO DE COSTO DE CADA PROYECTO, RESUELTO POR EL SERVER.
 *
 * Lo necesita cualquier listado que dibuje el badge del centro: sin esto la pantalla se queda
 * esperando a `cargarCentrosCosto` —los 2204 centros del catálogo de Tango— para traducir un número,
 * y hasta que llega muestra «ID: 46» en cada tarjeta. Acá es una consulta por página de proyectos.
 *
 * Estaba escrito adentro de `GET /projects` y el listado por cliente no lo tenía, que es justamente
 * la pantalla donde se veían los ids: el mismo badge resolvía distinto según de qué lista saliera.
 */
async function resolverCentroCostoDeProyectos(projects: any[]) {
 ...
 */
const sedesDelProyecto = (meta: any): number[] => {
  const varias = Array.isArray(meta?.sedeIds) ? meta.sedeIds.map(Number).filter((n: number) => Number.isFinite(n) && n > 0) : [];
  if (varias.length > 0) return [...new Set<number>(varias)];
...
const sedesPedidas = (meta: any): { sedeIds: number[]; sedeId: number | undefined } | undefined => {
  if (Array.isArray(meta?.sedeIds)) {
    const ids = sedesDelProyecto({ sedeIds: meta.sedeIds });
    return { sedeIds: ids, sedeId: ids[0] };
  }
  if (meta?.sedeId !== undefined) return { sedeIds: meta.sedeId ? [Number(meta.sedeId)] : [], sedeId: meta.sedeId || undefined };
  return undefined;
};

/** Resuelve a nombre las sedes de varios proyectos de una: `metadataResolutions.sedes` (y `.sede`, la principal). */
async function resolverSedes(projects: any[]): Promise<void> {
  ...
  const sedeMap = new Map(sedes.map((s: any) => [Number(s.data.id), s]));
  for (const p of projects) {
    const resueltas = sedesDelProyecto(p.metadata).map((id) => sedeMap.get(id)).filter(Boolean);
    if (resueltas.length === 0) continue;
    if (!p.metadataResolutions) p.metadataResolutions = {};
...
router.get("/projects", requireTenant, authenticateToken, requireAnyRole, async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
    ...
    */
    ...
    await resolverSedes(projects as any[]);
  ...
  try {
    ...
    */
    if (data.valoracionId) {
      ...
      project.valoracionId = (await valoracionParaMargen(req.tenantObjectId!, data.margen)) as any;
    ...
    project.metadata = {
      ...
      sedeId: sedesPedidas(data.metadata)?.sedeId as number,
...
*/
router.get("/projects/mis-areas-turnos", requireTenant, authenticateToken, async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
    const yo: any = await User.findById(req.user!.userId).select("metadata.id").lean();
    const idFrame = Number(yo?.metadata?.id);

    const condiciones: any[] = [{ "coordinatorAssignments.userId": new Types.ObjectId(req.user!.userId) }];
    // Sin id de FRAME no hay con qué matchear `responsableId`: se pregunta sólo por lo que coordina.
    if (Number.isFinite(idFrame)) condiciones.push({ "metadata.responsableId": idFrame });

    const proyectos: any[] = await Project.find({ tenantId: req.tenantObjectId, $or: condiciones })
      .select("name metadata.responsableId areasConfig coordinatorAssignments")
      .lean();

    let combinaciones = 0;
    const proyectosConAlgo: { _id: string; name: string }[] = [];
    for (const p of proyectos) {
      ...
      const propias = supervisa
        ? (p.areasConfig || []).reduce((n: number, ac: any) => n + (ac.shiftIds || []).length, 0)
        : (p.coordinatorAssignments || []).filter((a: any) => String(a?.userId?._id || a?.userId || "") === String(req.user!.userId) && a?.areaId && a?.shiftId).length;
      if (propias > 0) {
  ...
  try {
    ...
    if (project.metadata) {
      ...
      const sedesResueltas = sedesDelProyecto(project.metadata)
        .map((id) => (sedes as any[]).find((s: any) => Number(s.data?.id) === id))
        .filter(Boolean);
...
router.patch("/projects/:projectId", requireTenant, authenticateToken, requireAnyRole, async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
    ...
    if (updateData.metadata) {
      ...
      if (pedidas) {
        currentProject.metadata.sedeId = pedidas.sedeId as any;
    ...
    */
    const pidieronValoracion = (req.body as Record<string, unknown>).valoracionId !== undefined;
...
const revisarValoracion = async (args: { valoracionProyecto: unknown; rolFrameId: unknown; categoriaSatId: unknown; motivo?: unknown }) => {
  ...
  if (Number.isFinite(rolId) && Number.isFinite(catId)) {
    const rol: any = await RoleFrame.findOne({ "data.rol.id": rolId }).select("name data.categoriasSat").lean();
    const asociada = (rol?.data?.categoriasSat || []).find((c: any) => Number(c?.id) === catId);
  ...
  return {
    rechazo: null as null | Record<string, unknown>,
    aGuardar: (porUserId: string) => {
