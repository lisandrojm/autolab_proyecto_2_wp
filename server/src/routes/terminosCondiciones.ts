...
async function aceptacionesPorTerminos(tenantId: Types.ObjectId, ids: Types.ObjectId[]): Promise<Map<string, number>> {
  ...
  return new Map(filas.map((f) => [String(f._id), f.n as number]));
}

const leerCuerpo = (body: any): { titulo: string; contenido: string; vigente?: boolean } | { error: string } => {
  const titulo = String(body?.titulo ?? "").trim();
  const contenido = String(body?.contenido ?? "");
  if (!titulo) return { error: "El título es obligatorio." };
  if (!htmlHasText(contenido)) return { error: "El texto de los términos no puede quedar vacío." };
...
async function dejarUnicoVigente(tenantId: Types.ObjectId, id: Types.ObjectId) {
...
router.get("/", async (req: Req, res: Response) => {
  try {
    const docs: any[] = await TerminosCondiciones.find({ tenantId: req.tenantObjectId }).select("-historial").sort({ vigente: -1, updatedAt: -1 }).lean();
...
*/
router.get("/:id/version/:version", async (req: Req, res: Response) => {
  try {
    const doc: any = await TerminosCondiciones.findOne({ _id: req.params.id, tenantId: req.tenantObjectId }).lean();
    if (!doc) {
      res.status(404).json({ error: "Esos términos y condiciones ya no existen." });
      return;
    }
    const version = Number(req.params.version);
    const texto = doc.version === version ? doc : (doc.historial || []).find((h: any) => h.version === version);
    if (!texto) {
...
router.post("/", async (req: Req, res: Response) => {
  try {
    ...
    if (doc.vigente) await dejarUnicoVigente(req.tenantObjectId!, doc._id as Types.ObjectId);
...
*/
router.put("/:id", async (req: Req, res: Response) => {
  try {
    const cuerpo = leerCuerpo(req.body);
    if ("error" in cuerpo) {
      res.status(400).json({ error: cuerpo.error });
    ...
    if (cuerpo.vigente !== undefined) doc.vigente = cuerpo.vigente;
...
router.delete("/:id", async (req: Req, res: Response) => {
  try {
    ...
    const aceptaron = (await aceptacionesPorTerminos(req.tenantObjectId!, [doc._id as Types.ObjectId])).get(String(doc._id)) || 0;
    if (aceptaron > 0) {
