...
async function calculateDaysRequestedBackend(
  ...
  let userContractTypeId: number | null = null;
  ...
  if (config && userContractTypeId !== null) {
    rule = config.contractRules?.find((r: any) => r.contractId === userContractTypeId);
  }
  ...
  const validateDate = (date: Date): boolean => {
    ...
    if (rule) {
      if (isSat && !rule.saturday) return false;
      if (isSun && !rule.sunday) return false;
      if (isHol && !rule.holiday) return false;
    } else {
  ...
  try {
    ...
    if (data.categoryId) {
      ...
      if (category.categoryType === "fecha" && (!data.daysRequested || data.daysRequested === 0)) {
        data.daysRequested = await calculateDaysRequestedBackend(
          req.tenantObjectId!,
          userId,
          category,
          data.dynamicValue
    ...
    if (data.categoryId) {
      ...
      if (category?.categoryType === "datos_personales") {
        const enabled = ((category.config as any)?.camposEditables as string[]) || [];
