...
const idsDeTurno = (c: any): string[] => [...new Set([...(c.areaShiftAssignments || []).flatMap((a: any) => (a?.shiftIds || []).map(String)), ...(c.shiftId ? [String(c.shiftId)] : [])])].filter((id) => Types.ObjectId.isValid(id));

router.get("/", ...acceso, async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
    const hoy = hoyArgentina();
    const proyectos: any[] = await Project.find({ tenantId: req.tenantObjectId }).select("name clientId").populate("clientId", "name").lean();
    const nombreProyecto = new Map(proyectos.map((p) => [String(p._id), p.clientId?.name ? `${p.clientId.name} | ${p.name}` : p.name]));
    const ups: any[] = await UserProject.find({ projectId: { $in: proyectos.map((p) => p._id) } })
    ...
    for (const up of ups) {
      (up.contracts || []).forEach((c: any, indice: number) => {
        if (Array.isArray(c.dias_semana) && c.dias_semana.length > 0) return;
        const baja = fechaISO(c.fecha_baja_contrato);
        if (baja && baja < hoy) return; // terminado: ya no importa
        filas.push({ up, c, indice });
      });
    }

    const turnos: any[] = await Shift.find({ _id: { $in: [...new Set(filas.flatMap((f) => idsDeTurno(f.c)))] } }).select("name days").lean();
    const turnoDe = new Map(turnos.map((t) => [String(t._id), t]));
    const personas: any[] = await User.find({ _id: { $in: [...new Set(filas.map((f) => String(f.up.userId)))] } }).select("firstName lastName metadata.fullName").lean();
    ...
    const contratos = filas
      .map(({ up, c, indice }) => {
        ...
        const sugeridos = [...new Set(deTurno.flatMap((t: any) => (t.days || []).map(Number)))].sort((a, b) => a - b);
        return {
          ...
          turno: deTurno.map((t: any) => t.name).join(", "),
...
router.put("/", ...acceso, async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
    const { userProjectId, indice, fechaAlta } = req.body || {};
    const dias: number[] = [...new Set<number>((Array.isArray(req.body?.dias_semana) ? req.body.dias_semana : []).map(Number).filter((d: number) => Number.isInteger(d) && d >= 0 && d <= 6))].sort((a, b) => a - b);
    ...
    const up: any = await UserProject.findById(userProjectId).select("projectId contracts.fecha_alta_contrato").lean();
