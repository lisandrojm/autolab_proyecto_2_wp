...
const idsDe = (x: unknown): string[] => (Array.isArray(x) ? x.map(String).filter((id) => Types.ObjectId.isValid(id)).slice(0, 500) : []);

// ── Escritorio ──────────────────────────────────────────────────────────────

// POST y no GET: una página de usuarios son decenas de ids.
router.post("/resumen", ...escritorio, async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
...
router.get("/usuario/:userId", ...escritorio, async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
...
router.post("/", ...escritorio, async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
    ...
    if (origen === "solicitud_renovacion") {
      const solicitud: any = Types.ObjectId.isValid(String(solicitudId)) ? await User.findOne({ _id: solicitudId, tenantId: req.tenantObjectId }).select("metadata.projectIds metadata.solicitudUserId").lean() : null;
...
const aCargo = async (req: AuthenticatedRequest & TenantRequest, res: any): Promise<boolean> => {
  if (await tieneAlCargoElProyecto(req.tenantObjectId!, req.user!.userId, req.params.projectId)) return true;
...
router.get("/equipo/:projectId/resumen", ...equipos, async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
    if (!(await aCargo(req, res))) return;
...
router.get("/equipo/:projectId/usuario/:userId", ...equipos, async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
    if (!(await aCargo(req, res))) return;
...
router.post("/equipo/:projectId", ...equipos, async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
    if (!(await aCargo(req, res))) return;
