 ...
 */
router.post("/nombres/validar", async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
    if (!isAdmin(req)) {
    ...
    const idsPedidos: string[] = Array.isArray(req.body?.userIds) ? req.body.userIds.filter((x: any) => Types.ObjectId.isValid(String(x))).map(String) : [];
    ...
    if (idsPedidos.length > 0) filtro._id = { $in: idsPedidos };
    if (!revalidar) filtro["metadata.nombreValidadoArcaAt"] = { $exists: false };

    const candidatos: any[] = await User.find(filtro).select("_id metadata.cuit").lean();
...
router.post("/consulta-padron/bulk", async (req: AuthenticatedRequest & TenantRequest, res) => {
  try {
    ...
    const resultados: {
      ...
      faultCode?: string;
      faultString?: string;
      /** Presente SOLO si el nombre guardado no coincidía con el de ARCA y se reemplazó. */
