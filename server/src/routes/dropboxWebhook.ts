...
dropboxWebhookRoutes.post("/webhook", async (req, res) => {
  ...
  const cuentas: string[] = Array.isArray((req.body as any)?.list_folder?.accounts) ? (req.body as any).list_folder.accounts : [];
  ...
  try {
    ...
    */
    ...
    res.status(200).end();
    for (const t of autorizados) programarEscaneo(String(t._id));
  } catch (e: any) {
