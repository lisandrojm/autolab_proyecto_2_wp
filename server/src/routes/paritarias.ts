...
router.put("/fuentes/:id", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    */
    ...
    const actualizada = await FuenteParitaria.findByIdAndUpdate(
      req.params.id,
      cambioPatron ? { $set: { ...data, ultimoError: "" }, $unset: { ultimaRevision: 1, ultimoResultado: 1, enlacesUltimaExitosa: 1 } } : { $set: data },
      { new: true },
    );
 ...
 */
router.get("/fuentes/:id/publicaciones", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    /*
    ...
    */
    ...
    const filtro: any = { fuente: req.params.id };
 ...
 */
router.get("/publicaciones/:id/texto", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const p: any = await PublicacionParitaria.findById(req.params.id).select("textoEnlace archivo.nombreOriginal extraccion").lean();
    if (!p) return res.status(404).json({ error: "Publicación no encontrada" });
    if (!p.extraccion?.extraidoEl) return res.status(409).json({ error: "Esta publicación todavía no tiene texto extraído. Corré: npm run paritarias-texto" });
    res.json({
 ...
 */
router.get("/publicaciones/:id/archivo", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const p = await PublicacionParitaria.findById(req.params.id).lean();
    if (!p) return res.status(404).json({ error: "Publicación no encontrada" });
    const a = (p as any).archivo;
    if (!a?.ruta) {
      ...
      return res.status(404).json({ error: (p as any).archivoError || "Esta publicación no tiene el PDF guardado. Se detectó antes de que el sistema guardara los archivos: se puede rebajar con el script de respaldo." });
    }
    if (!(await existeArchivo(a.ruta))) return res.status(410).json({ error: "El registro tiene un archivo pero el archivo no está en el disco." });
  ...
  try {
    ...
    */
    ...
    /*
      ...
      mano y deliberado.
    */
    for (const p of pubs as any[]) if (await borrarArchivo(p.archivo?.ruta)) archivos++;
 ...
 */
router.get("/estado", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    */
    let codigosDeLaEmpresa: Set<string> | null = null;
    ...
    if (empresaId) {
      ...
      codigosDeLaEmpresa = new Set((convenios as any[]).map((c) => String(c.externalId || "").trim()).filter(Boolean));
    }
    const leImporta = (convenios: string[] | undefined): boolean => !codigosDeLaEmpresa || (convenios || []).some((c) => codigosDeLaEmpresa!.has(String(c).trim()));
    ...
    */
    ...
    res.json({
      ...
      declarado: Object.fromEntries(
        (declarados as any[]).map((c) => [String(c._id), { estado: c.fuenteEstadoDeclarado, nota: c.fuenteNota || "", revisadaPor: c.fuenteRevisadaPor || "", revisadaEl: c.fuenteRevisadaEl ?? null }]),
      ),
