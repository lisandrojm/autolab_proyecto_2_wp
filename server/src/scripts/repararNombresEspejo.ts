...
const tieneRotos = (s: string) => s.includes("\uFFFD");
...
async function run() {
  ...
  const t: any = await (Tenant as any).findOne({ "integrations.dropbox.appKey": { $exists: true } }).lean();
  const cfg = t ? getTenantDropboxConfig(t) : null;
  if (!cfg) throw new Error("Ningún tenant tiene Dropbox conectado.");
  const tenantId = String(t._id);

  // El destino correcto de cada archivo sale de Mongo, que lo tiene bien.
  const pubs: any[] = await PublicacionParitaria.find({ "dropbox.path": { $regex: "^" + base.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") } })
    .select("dropbox.path")
    .lean();
  const porClave = new Map<string, string>();
  for (const p of pubs) porClave.set(clave(p.dropbox.path), p.dropbox.path);
  ...
  */
  ...
  for (const e of entidades.entries) {
    if (e.tag !== "folder" || !tieneRotos(e.name)) continue;
    ...
    if (!DRY_RUN) await moveEntry(tenantId, cfg, e.path, destino);
    carpetas++;
  }
    ...
    for (const anio of (await listFolder(tenantId, cfg, ent.path)).entries) {
      ...
      for (const f of (await listFolder(tenantId, cfg, anio.path)).entries) {
        ...
        */
        ...
        if (destino === f.path) continue; // ya está bien: idempotente.
        console.log(`${DRY_RUN ? "·" : "✔"}  ${f.path}\n       → ${destino}`);
        if (!DRY_RUN) await moveEntry(tenantId, cfg, f.path, destino);
        archivos++;
      }
    }
  }
