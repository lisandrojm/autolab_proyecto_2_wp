...
const ORIGINAL = {
  ...
  allowedProjectIds: [] as Types.ObjectId[],
};
...
async function main() {
  ...
  */
  ...
  if (porNombre) {
    console.log(`=  Ya existe un motivo llamado "${ORIGINAL.name}" (${(porNombre as any)._id}). No se toca.`);
    await mongoose.disconnect();
    return;
  }
  if (porId) {
    console.log(`!  El id original está ocupado por "${(porId as any).name}". Habría que decidir a mano: no se toca nada.`);
    await mongoose.disconnect();
    process.exit(1);
  }
  ...
  if (efectos.length === 0) console.log("      (sin efectos en la semilla)");
