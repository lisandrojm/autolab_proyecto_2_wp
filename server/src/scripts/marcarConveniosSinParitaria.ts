...
const SIN_PARITARIA: Array<{ codigo: string; porque: string }> = [{ codigo: "9999/99", porque: "no es un CCT: es «excluido de convenio». Sin sindicato no hay quién publique acuerdos." }];
...
interface Respaldo {
  _id: string;
  externalId: string;
  fuenteEstadoDeclarado?: string;
  fuenteNota?: string;
  fuenteRevisadaPor?: string;
  fuenteRevisadaEl?: Date | null;
}
...
async function run() {
  ...
  for (const { codigo, porque } of SIN_PARITARIA) {
    ...
    */
    ...
    if (vigilante) {
      console.log(`⛔ ${codigo}: «${(vigilante as any).nombre}» lo tiene asignado como fuente. No se marca: revisalo a mano.`);
      continue;
    }
  ...
  if (parecidos.length > 0) {
    ...
    for (const p of parecidos as any[]) console.log(`     ${p.externalId} — ${p.name}`);
  }
