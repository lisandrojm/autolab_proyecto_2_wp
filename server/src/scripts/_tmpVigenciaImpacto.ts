...
const vig = (baja: any, hoy: string) => !baja || String(baja).substring(0, 10) >= hoy;
...
async function main() {
  ...
  const indeterminadoPorNombre = new Map<string, boolean>();
  ...
  const p: any = await Project.findOne({ name: { $regex: "426_LN", $options: "i" } }).select("_id name").lean();
  ...
  for (const m of members) {
    const up: any = ((m as any).metadata?.projects || []).find((x: any) => x && String(x.projectId) === String(projectId));
    ...
    if (antes) vigAntes++;
    if (despues) vigDespues++;
    if (!antes && despues) {
      const indet = indeterminadoPorNombre.get(String(activo.nombre_contrato || "").toLowerCase()) ?? false;
      if (!activo.fecha_baja_contrato) sinBajaPeroUltimoVencido++;
      if (cambian.length < 12) {
        cambian.push(
          `${(m as any).firstName} ${(m as any).lastName} — contratos=${contracts.length} | último="${ultimo.nombre_contrato}" baja=${String(ultimo.fecha_baja_contrato || "—").substring(0, 10)} → activo="${activo.nombre_contrato}" baja=${String(activo.fecha_baja_contrato || "—").substring(0, 10)}${indet ? " (tipo indeterminado)" : ""}`,
        );
      }
    }
  }
