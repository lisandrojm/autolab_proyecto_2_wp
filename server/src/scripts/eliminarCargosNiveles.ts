...
async function run() {
  ...
  const limpiezas: Array<{ col: string; filtro: any; unset: Record<string, ""> }> = [
  ...
  for (const { col, filtro, unset } of limpiezas) {
    if (!existentes.includes(col)) { console.log(`· ${col}: no existe, se saltea`); continue; }
    const n = await db.collection(col).countDocuments(filtro);
    const campos = Object.keys(unset).join(", ");
    if (n === 0) { console.log(`· ${col}: 0 doc(s) con ${campos} — nada que hacer`); continue; }
    if (DRY_RUN) { console.log(`· ${col}: ${n} doc(s) perderían ${campos}`); continue; }
  ...
  for (const col of ["positions", "levels"]) {
    if (!existentes.includes(col)) { console.log(`· colección ${col}: no existe`); continue; }
    const n = await db.collection(col).countDocuments();
    if (DRY_RUN) { console.log(`· colección ${col}: se eliminaría con sus ${n} doc(s)`); continue; }
  ...
  if (nRoles === 0) console.log(`· roles: ninguno tiene ${permisos.join(" / ")}`);
  else if (DRY_RUN) console.log(`· roles: ${nRoles} rol(es) perderían ${permisos.join(" / ")}`);
  else {
    const r = await db.collection("roles").updateMany({ permissions: { $in: permisos } }, { $pull: { permissions: { $in: permisos } } } as any);
