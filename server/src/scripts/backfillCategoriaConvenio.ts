...
async function run() {
  ...
  if (iTabla < 0 || iPadded < 0 || iPadre < 0) throw new Error("Al CSV le faltan columnas (tabla / codigo_padded / filtro_padre)");
  ...
  for (const f of filas) {
    ...
    if (previo && previo !== cct) conflictivos.add(cod);
  ...
  for (const c of cats as any[]) {
    ...
    completadas++;
    if (!DRY_RUN) await db.collection("categorias-sat").updateOne({ _id: c._id }, { $set: { "data.convenio": cct, updatedAt: new Date() } });
  }
  ...
  const usados = new Set(cats.map((c: any) => c.data?.convenio).filter(Boolean) as string[]);
  if (!DRY_RUN) for (const [, cct] of convenioPorCategoria) usados.add(cct);
