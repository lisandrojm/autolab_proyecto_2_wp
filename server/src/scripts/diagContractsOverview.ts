...
const kb = (o: unknown) => `${(JSON.stringify(o).length / 1024).toFixed(0)} KB`;
...
async function main() {
  ...
  const projects: any[] = await Project.find({ tenantId: tenant._id }).select("_id name clientId").lean();
  const projectIds = projects.map((p) => p._id);
  console.log(`proyectos del tenant: ${projects.length}  (${ms(tProj)})`);

  // --- FASE 1: el barrido que hace el endpoint en cada request.
  const tF1 = Date.now();
  const memberships: any[] = await UserProject.aggregate([
  ...
  const plan: any = await UserProject.collection
    .aggregate([{ $match: { projectId: { $in: projectIds } } }, { $project: { _id: 1 } }], { explain: true } as any)
    .next()
    .catch(() => null);
  ...
  const idsPorIndice = new Map<number, any[]>();
