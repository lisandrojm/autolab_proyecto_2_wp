...
async function run() {
  ...
  if (!conexion) {
    ...
    if (!DRY_RUN) await marcarPendientes(motivo);
  ...
  if (!alcance.ok) {
    ...
    if (!DRY_RUN) await marcarPendientes(alcance.motivo);
    await mongoose.disconnect();
    return;
  }

  const entidades = new Map<string, string>();
  for (const f of await FuenteParitaria.find({}).select("entidad").lean()) entidades.set(String((f as any)._id), (f as any).entidad || "");
  ...
  for (const p of pendientes) {
    ...
    if (!p.archivo?.ruta || !(await existeArchivo(p.archivo.ruta))) {
      ...
      if (!DRY_RUN) await PublicacionParitaria.updateOne({ _id: p._id }, { $set: { dropbox: { path: "", estado: "pendiente", motivo: `No se espejó: ${razon}.` } } });
      continue;
    }

    const ex: any = (p as any).extraccion || {};
    const nombre = nombreLegible({
      ...
      conveniosMencionados: (ex.conveniosMencionados || []).map((c: any) => c.valor),
...
async function marcarPendientes(motivo: string) {
