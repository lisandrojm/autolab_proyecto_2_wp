...
const CAMPOS =
  "projectId nombre_rol_frame rol_frame_id contracts.fecha_alta_contrato contracts.fecha_baja_contrato contracts.fecha_carga contracts.nombre_contrato contracts.tipo_contrato contracts.nombre_rol_frame";
...
async function main() {
  ...
  const vinculos: any[] = await UserProject.aggregate([
    { $match: { userId: { $in: usuarios.map((u: any) => u._id) } } },
    {
      $project: {
  ...
  const contratos = vinculos.reduce((n: number, v: any) => n + (v.contracts?.length || 0), 0);
