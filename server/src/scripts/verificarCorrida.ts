...
import { normalizarParte, Evento, DatosDePersona } from "../services/liquidacion/normalizar.js";
import { codificarEvento, Linea, Exclusion, ReglasGlobales } from "../services/liquidacion/codificar.js";
...
async function main() {
  ...
  const semillaPorClave = new Map<string, any[]>(Object.entries(MAPEO_SEMILLA).map(([nombre, efectos]) => [claveDeMotivo(nombre), efectos]));
  const noLiquidaPorClave = new Set(MOTIVOS_QUE_NO_LIQUIDAN.map(claveDeMotivo));

  const efectosPorNombre = new Map<string, any[]>();
  for (const m of motivos as any[]) {
    const semilla = semillaPorClave.get(claveDeMotivo(m.name)) || [];
    efectosPorNombre.set(
      claveDeMotivo(m.name),
      semilla.map((e) => ({ ...e, soloRegimen: e.soloRegimen ?? null, empresaId: null, vigenteDesde: "2020-01-01", vigenteHasta: null })),
    );
  }
  ...
  for (const f of padron.filas) {
    const datos: DatosDePersona = { apellidoYNombre: f.apellidoYNombre, legajo: f.legajo, empresaId: f.empresaId, ccCodigo: f.ccCodigo, regimen: f.regimen };
    if (!porPersonaYProyecto.has(`${f.userId}|${f.projectId}`)) porPersonaYProyecto.set(`${f.userId}|${f.projectId}`, datos);
    if (!porPersona.has(f.userId)) porPersona.set(f.userId, datos);
  }
  const datosDe = (userId: string, projectId: string | null) => (projectId ? porPersonaYProyecto.get(`${userId}|${projectId}`) : undefined) || porPersona.get(userId);

  const partes: any[] = await Request.find({ tenantId, date: { $gte: desde, $lte: hasta }, "attendance.0": { $exists: true } })
  ...
  padron.filas.forEach((f) => {
    if (f.empresaId && f.empresaNombre) nombreEmpresa.set(f.empresaId, f.empresaNombre);
    if (f.ccCodigo && f.ccNombre) nombreCC.set(f.ccCodigo, f.ccNombre);
  });
  ...
  const porConcepto = new Map<string, { filas: number; par1: number; par2: number }>();
  agregadas.forEach((l) => {
  ...
  const sinEfecto = new Map<string, number>();
  exclusiones
