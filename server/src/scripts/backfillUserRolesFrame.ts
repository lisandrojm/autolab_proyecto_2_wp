...
async function backfillUserRolesFrame() {
  ...
  try {
    ...
    // ── Usuarios candidatos ────────────────────────────────────────────
    const filter: any = { tenantId, "metadata.id": { $exists: true } };
    if (!force) {
      filter.$or = [
    ...
    let totalRefs = 0;
    const createdRoleFrames = new Map<string, string>(); // id -> nombre

    for (const user of users) {
      processed++;
      if (processed % 50 === 0) console.log(`   ...${processed}/${users.length}`);

      const empleadoId = Number((user.metadata as any)?.id);
      ...
      if (dryRun) {
        ...
        console.log(
          `   [DRY] ${user.email} (emp ${empleadoId}) -> ${frameRoles.map((r) => `${r.nombre}#${r.id}`).join(", ")}`,
        );
        continue;
      }

      const { refs, created } = await resolveRoleFrameRefs(frameRoles);
      for (const c of created) createdRoleFrames.set(String(c.id), c.nombre);
    ...
    if (!dryRun) console.log(`RoleFrame creados:          ${createdRoleFrames.size}`);
    if (createdRoleFrames.size > 0) {
      console.log("RoleFrame creados (id -> nombre):");
      for (const [id, nombre] of createdRoleFrames) console.log(`   ${id} -> ${nombre}`);
    }
    console.log("───────────────────────────────────");
  } finally {
    await disconnectDB();
