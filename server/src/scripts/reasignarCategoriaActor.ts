 ...
 */
const DECISIONES: Array<{ proyecto: string; rol: string; n: number; convenio: string; codigoArca: string }> = [
...
const clave = (proyecto: string, rol: string) => `${proyecto}||${rol}`;
...
async function run() {
  ...
  for (const d of DECISIONES) {
    ...
    const cat: any = await Categoria.findOne({ convenio: d.convenio, codigoArca: d.codigoArca }).lean();
    if (!cat) {
  ...
  for (const [k, c] of destinos) console.log(`  ${k.padEnd(20)} data.id=${String(c.legacyId).padStart(4)}  ${c.nombre}`);
  if (destinos.size === 0) console.log("  (ninguna: faltan decisiones)");

  // ── 2. Los contratos que hoy apuntan a «Actor».
  const ups: any[] = await UserProject.find({ "contracts.categoria_sat_id": CATEGORIA_ACTOR_LEGACY_ID }).select("nombre_proyecto userId projectId contracts").lean();

  const porClave = new Map<string, { up: any; idx: number; c: any }[]>();
  const fueraDelMapeo: string[] = [];
  let total = 0;
  for (const up of ups) {
    for (let i = 0; i < (up.contracts || []).length; i++) {
      const c = up.contracts[i];
      if (Number(c?.categoria_sat_id) !== CATEGORIA_ACTOR_LEGACY_ID) continue;
      total++;
  ...
  for (const d of DECISIONES) {
    ...
    if (encontrados === 0) problemas.push(`SIN MATCH: la fila «${d.proyecto} · ${d.rol}» no encontró ningún contrato — ¿está bien escrito el proyecto?`);
    else if (encontrados !== d.n) console.log(`      ojo: el conteo cambió desde el relevamiento (${d.n} → ${encontrados}).`);
  }

  console.log(`\nTotal con categoría ${CATEGORIA_ACTOR_LEGACY_ID}: ${total} (esperados ${TOTAL_ESPERADO})`);
  if (total !== TOTAL_ESPERADO) problemas.push(`TOTAL DISTINTO: hay ${total} contratos con la categoría «Actor», no ${TOTAL_ESPERADO}.`);
  ...
  if (fueraDelMapeo.length > 0) {
    ...
    for (const u of unicos) console.log(`  ${u}  ×${fueraDelMapeo.filter((x) => x === u).length}`);
  }
  ...
  const log: any[] = [];
  const porDoc = new Map<string, { up: any; cambios: { idx: number; antes: number; ahora: number }[] }>();
  for (const d of DECISIONES) {
    ...
    for (const { up, idx, c } of porClave.get(clave(d.proyecto, d.rol)) || []) {
      ...
      if (!porDoc.has(k)) porDoc.set(k, { up, cambios: [] });
  ...
  for (const { up, cambios } of porDoc.values()) {
    const doc: any = await UserProject.findById(up._id);
    if (!doc) continue;
    for (const { idx, ahora } of cambios) {
      const destino = [...destinos.values()].find((d) => Number(d.legacyId) === ahora)!;
      doc.contracts[idx] = { ...doc.contracts[idx].toObject(), categoria_sat_id: ahora, nombre_categoria_sat: destino.nombre } as any;
      escritos++;
    }
