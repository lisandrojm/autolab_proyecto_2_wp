...
async function run() {
  ...
  const origen: any = await ContratoFrame.findOne({ name: ORIGEN }).lean();
  const destino: any = await ContratoFrame.findOne({ name: DESTINO }).lean();
  if (!origen) throw new Error(`No existe la plantilla origen «${ORIGEN}»`);
  if (!destino) throw new Error(`No existe la plantilla destino «${DESTINO}»`);
  ...
  if (aOrigen.problemas.length > 0) {
    ...
    for (const x of aOrigen.problemas.slice(0, 5)) console.log(`   ${x.motivo}`);
  ...
  writeFileSync(
    archivo,
    JSON.stringify({ fecha: new Date().toISOString(), origen: ORIGEN, destino: DESTINO, destinoAntes: { name: destino.name, contratoId: String(destino.contratoId || ""), content: htmlDestino } }, null, 2),
  );

  await ContratoFrame.updateOne({ _id: destino._id }, { $set: { content: htmlOrigen } });

  // Verificación sobre lo que quedó escrito, releyendo.
  const fin: any = await ContratoFrame.findById(destino._id).lean();
