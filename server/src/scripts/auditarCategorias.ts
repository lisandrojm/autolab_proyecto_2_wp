...
async function run() {
  ...
  const enArca = new Map<string, string>(); // "convenio|codigo" → descripción
  if (hayCsv) {
  ...
  const cats = (await db.collection("categorias").find({}).toArray()) as any[];
  const grupos = (await db.collection("convenio-grupos").find({}).toArray()) as any[];
  ...
  const contratosDe = (c: any) => (c.legacyId != null ? uso.get(Number(c.legacyId)) || 0 : 0);
  const codigoValido = (v: any) => /^\d{6}$/.test(String(v || "")) && String(v) !== "000000";
  ...
  if (huerfanas.length === 0) ok("Ninguna.");
  ...
  if (!hayCsv) console.log("   (salteado: falta el CSV)");
  else {
    ...
    if (fuera.length === 0) ok("Todos los pares (convenio, código) existen en ARCA.");
    ...
    for (const cct of conveniosCargados.sort()) {
      ...
      } else ok(`${cct}: las ${enArcaDelCct.length} categorías de ARCA están cargadas.`);
    }
  }
  ...
  if (rotos.length === 0) ok("Todos los contratos resuelven su categoría.");
  for (const [id, n] of rotos) {
    hallazgos++;
    console.log(`   ✗ categoria_sat_id ${id} → ${n} contrato(s), sin categoría que lo resuelva`);
  }
  if (rotos.length > 0) console.log(`     → ver scripts/repararCategoriasHuerfanas.ts (crea un alias en vez de remapear, porque categoria_sat_id es parte de la clave del sync de FRAME)`);
  ...
  const roles = (await db.collection("roles_frame").find({}).toArray()) as any[];
  let mezcladas = 0;
  for (const r of roles) {
    ...
    const asoc = (r.data?.categoriasSat || []).map((cs: any) => porLegacyId.get(Number(cs?.id)) || { convenio: "", codigoArca: "", nombre: `${cs?.nombre || "?"} (id ${cs?.id} no resuelve)`, roto: true });
    const convs = [...new Set(asoc.map((c: any) => (c.roto ? "(no resuelve)" : String(c.convenio || "").trim() || "(sin convenio)")))];
    if (convs.length > 1) {
      mezcladas++;
      hallazgos++;
      console.log(`   ✗ "${r.name}" → ${convs.join(" + ")}`);
      for (const c of asoc as any[]) console.log(`       ${String(c.convenio || (c.roto ? "—" : "sin convenio")).padEnd(10)} ${c.codigoArca || "sin código"}  ${c.nombre}`);
      console.log(`     → una función mapea a categorías de UN convenio: la empleadora tiene que tenerlo habilitado`);
    }
  }
  if (mezcladas === 0) ok(`Las ${roles.length} funciones apuntan a un solo convenio.`);

  // ── 6
  titulo(6, "Convenios de categorías que ninguna empresa tiene habilitados");
  const convenios = (await db.collection("convenios").find({}).toArray()) as any[];
  const codigoPorId = new Map(convenios.map((c) => [String(c._id), String(c.externalId).trim()]));
  const habilitados = new Set<string>();
  for (const e of (await db.collection("companies").find({}).toArray()) as any[]) {
    for (const id of e.convenioIds || []) {
      const cod = codigoPorId.get(String(id));
      if (cod) habilitados.add(cod);
    }
  }
  const usados = [...new Set(cats.map((c) => String(c.convenio || "").trim()).filter(Boolean))];
  const sinEmpresa = usados.filter((c) => !habilitados.has(c));
  if (sinEmpresa.length === 0) ok("Todos los convenios con categorías están habilitados en alguna empresa.");
