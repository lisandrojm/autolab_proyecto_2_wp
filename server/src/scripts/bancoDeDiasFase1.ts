...
async function main() {
  ...
  */
  ...
  const partes: any[] = await Request.find({ tenantId, "attendance.0": { $exists: true } }).select("attendance").lean();
  ...
  const cambios: { _id: any; attendance: any[] }[] = [];
  for (const parte of partes) {
    let tocado = false;
    const nuevos = (parte.attendance || []).map((r: any) => {
      renglones++;
      if (r.typeId) {
        conTipo++;
        return r;
    ...
    if (tocado) cambios.push({ _id: parte._id, attendance: nuevos });
  }
  ...
  for (let i = 0; i < cambios.length; i += 200) {
    ...
    await Request.bulkWrite(lote.map((c) => ({ updateOne: { filter: { _id: c._id }, update: { $set: { attendance: c.attendance } } } })) as any);
  }
