...
function leerArchivo(ruta: string): Record<string, FilaPequena[]> {
  const absoluta = path.resolve(process.cwd(), ruta);
  if (!fs.existsSync(absoluta)) throw new Error(`No existe el archivo: ${absoluta}`);
  const json = JSON.parse(fs.readFileSync(absoluta, "utf8"));
  const normalizar = (filas: any[]): FilaPequena[] =>
    (filas || []).map((f) => ({
...
async function run() {
  ...
  if (!acuerdo) console.log(`   ⚠ No está cargado el acuerdo ${EXPEDIENTE}: las filas van a quedar sin vínculo al acta.\n`);

  const tramos: Array<{ nombre: string; codigo: string; vigencia: { desde: string; hasta: string }; filas: FilaPequena[] }> = [
  ...
  for (const tramo of tramos) {
    ...
    for (const f of tramo.filas.sort((a, b) => a.grupo - b.grupo)) {
      if (!Number.isFinite(f.grupo)) continue;
      const desvio = desvioJornadaAdicional(f.semana9hsLunVie, f.jornadaAdicional9hs);
      if (desvio != null) avisos.push(`${tramo.codigo} G${f.grupo}: la jornada adicional difiere en ${desvio} de la semana ÷ 5`);
      ...
      if (!DRY_RUN) {
        await EscalaPequenasEmpresas.updateOne(
          { convenio: CONVENIO, grupo: f.grupo, desde: aFecha(tramo.vigencia.desde) },
          {
            $setOnInsert: {
