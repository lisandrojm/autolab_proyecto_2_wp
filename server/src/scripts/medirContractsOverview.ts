...
async function main() {
  ...
  const projectFilter: any = { tenantId: new Types.ObjectId(tenantId) };
  if (clientId) projectFilter.clientId = new Types.ObjectId(clientId);
  ...
  if (projectIds.length === 0) { console.log("Sin proyectos."); await mongoose.disconnect(); return; }
  ...
  t = Date.now();
  const soloUltimo: any[] = await UserProject.aggregate([
    { $match: { projectId: { $in: projectIds } } },
  ...
  const userIds = [...new Set(membershipsRaw.map((m: any) => String(m.u || "")))].filter((id) => Types.ObjectId.isValid(id));
  t = Date.now();
  const [usersList, clientsList] = await Promise.all([
    ...
    Client.find({ _id: { $in: [...new Set(projectsList.map((p: any) => String(p.clientId || "")))].filter((id) => Types.ObjectId.isValid(id)) } }).select("name").lean(),
  ]);
  ...
  const ids = membershipsRaw.slice(0, 25).map((m: any) => m._id);
  t = Date.now();
  ...
  const queRigen: any[] = await UserProject.aggregate([
    { $match: { projectId: { $in: projectIds } } },
    {
      $project: {
        p: "$projectId",
  ...
  [...membershipsRaw].sort((a, b) => (b.c?.length || 0) - (a.c?.length || 0)).slice(0, 5).forEach((m: any) => console.log(`  · ${m.c?.length || 0} contratos`));
