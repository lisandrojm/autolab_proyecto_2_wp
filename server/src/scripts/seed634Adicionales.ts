...
async function run() {
  ...
  if (!acuerdo) console.log(`   ⚠ No está cargado el acuerdo ${EXPEDIENTE} (corré 634-acuerdo primero): los importes van a quedar sin vínculo al acta.\n`);
  ...
  for (const [indice, ad] of ADICIONALES.entries()) {
    ...
    if (existente?.confirmado) respetados++;
    ...
    if (!DRY_RUN) {
      ...
      for (const [desde, hasta, monto, tramo] of [
        ...
        const r = await AdicionalValorPeriodo.updateOne(
          { adicionalId, grupo: null, desde: aFecha(desde) },
          {
            $setOnInsert: {
              convenio: CONVENIO,
              hasta: aFecha(hasta),
        ...
        if (r.upsertedCount) valoresNuevos++;
      }
    }
  }
  ...
  if (respetados) console.log(`${respetados} ya estaban confirmados: se les respetó la definición.`);
...
async function revertir() {
  ...
  if (confirmados) console.log(`${confirmados} quedaron: ya estaban confirmados y borrarlos sería descartar trabajo de otra persona.`);
  console.log("");
  await mongoose.disconnect();
}
