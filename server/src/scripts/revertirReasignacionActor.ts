...
async function run() {
  ...
  if (!archivo) throw new Error("Falta el archivo de log: npm run actores:revertir -- logs/reasignacion-actor-....json");

  const uri = process.env.MONGO_URI;
  const dbName = process.env.MONGO_DB_NAME;
  if (!uri || !dbName) throw new Error("Faltan MONGO_URI / MONGO_DB_NAME (usar dotenv -e .env.production)");

  const log: any[] = JSON.parse(readFileSync(archivo, "utf8"));
  ...
  for (const [id, entradas] of porDoc) {
    ...
    for (const e of entradas) {
      ...
      if (!DRY_RUN) {
        doc.contracts[e.contractIndex] = { ...actual.toObject(), categoria_sat_id: e.antes.categoria_sat_id, nombre_categoria_sat: e.antes.nombre_categoria_sat } as any;
      }
      revertidos++;
    }
    if (!DRY_RUN) {
  ...
  if (yaCambiados.length > 0) {
    ...
    for (const y of yaCambiados.slice(0, 10)) console.log(`  · ${y}`);
    if (yaCambiados.length > 10) console.log(`  …y ${yaCambiados.length - 10} más.`);
    console.log("");
  }
