...
import { auditarFunciones, COLECCION_CONTRATOS, CategoriaLean, RolLean } from "../utils/auditoriaFuncionesFrame.js";
/**
...
async function run() {
  ...
  const roles = (await db.collection("roles_frame").find({}).toArray()) as unknown as RolLean[];
  /*
  ...
  */
  const categorias = ([...(await db.collection("categorias").find({}).toArray()), ...(await db.collection("categorias-sat").find({}).toArray())] as unknown) as CategoriaLean[];
  ...
  */
  const conteo = await db
    ...
    .toArray();
  const contratosPorRolId = new Map<number, number>(conteo.filter((c: any) => c._id != null).map((c: any) => [Number(c._id), Number(c.n)]));
  ...
  if (rotas.length === 0) {
    ...
    for (const f of rotas) {
      ...
      if (ok.length) console.log(`       le quedan válidas: ${ok.map((r) => `${r.convenio || "sin convenio"} ${r.codigoArca || "sin código"} «${r.nombreVigente || r.nombreGuardado}»`).join(", ")}`);
      console.log("");
    }
  }
  ...
  if (mezclan.length > 0) {
    ...
    for (const f of mezclan) console.log(`   · ${f.nombre}  ·  ${f.contratos} contrato(s)  ·  ${f.conveniosVigentes.join(" + ")}`);
    console.log("");
  }
  ...
  */
  ...
  const conveniosPorRolId = new Map<number, Set<string>>();
  for (const f of auditadas) {
    if (f.rolId != null) conveniosPorRolId.set(Number(f.rolId), new Set(f.conveniosVigentes));
  }
  ...
  const convenioPorCatId = new Map<number, string>();
  for (const c of categorias) {
    const id = c.legacyId ?? c.data?.id;
    if (id != null && c.convenio && !convenioPorCatId.has(Number(id))) convenioPorCatId.set(Number(id), c.convenio);
  }
  ...
  const nombrePorRolId = new Map<number, string>(auditadas.filter((f) => f.rolId != null).map((f) => [Number(f.rolId), f.nombre]));
  ...
  if (sinConvenio > 0) console.log(`   ${sinConvenio} contrato(s) más con una categoría SIN convenio resoluble (otro problema, se arregla en el catálogo)`);
  for (const [n, cant] of [...porFuncion.entries()].sort((a, b) => b[1] - a[1])) console.log(`      ${cant.toString().padStart(4)}  ${n}`);
  console.log("");
