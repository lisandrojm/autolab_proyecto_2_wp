...
async function assignImportedUsersRoleAndDNI() {
  ...
  try {
    ...
    const frameUsers = await User.find(
      { "metadata.id": { $exists: true } },
      { email: 1, tenantId: 1, "metadata.documento": 1 },
    );
    const dniByKey = new Map<string, string>(); // `${tenantId}:${email}` -> documento
    for (const u of frameUsers) {
      const doc = u.metadata?.documento?.toString().trim();
      if (doc && doc.length >= 6) dniByKey.set(`${u.tenantId}:${u.email}`, doc);
    }
    ...
    if (dryRun) console.log("   (DRY_RUN: backfill no persistido)");
  } catch (error) {
