...
interface Cambio {
  ...
  indice: number;
  fechaAlta: string;
  antes: { dias_semana: number[] | null; dias_por_semana: number | null; dias_rotativos: boolean | null };
  despues: number[];
}
...
async function run() {
  ...
  const ups: any[] = await UserProject.find({}).select("contracts").lean();
  const turnos: any[] = await Shift.find({}).select("days name").lean();
  ...
  for (const up of ups) {
    (up.contracts || []).forEach((c: any, indice: number) => {
      ...
      if (ids.length === 0) {
        sinDiasSinTurno++;
        if (vigente) sinDiasSinTurnoVigentes++;
        return;
      }
  ...
  if (turnoSinDias) console.log(`Con turno pero el turno no tiene días: ${turnoSinDias}.`);
  console.log("");
...
async function revertir(archivo: string) {
  ...
  const cambios: Cambio[] = JSON.parse(fs.readFileSync(ruta, "utf8"));
  console.log(`\nDB: ${dbName}   |   revirtiendo ${cambios.length} contrato(s) desde ${path.basename(ruta)}\n`);
  for (const x of cambios) {
    const set: any = {};
    const unset: any = {};
    for (const campo of ["dias_semana", "dias_por_semana", "dias_rotativos"] as const) {
      const clave = `contracts.${x.indice}.${campo}`;
      if (x.antes[campo] === null) unset[clave] = "";
      else set[clave] = x.antes[campo];
    }
