...
/**
 * Convierte el JSON que baja `scrapeConsola.js` en el XLSX del nomenclador de Convenios Colectivos.
 *
...
const fixEncoding = (s: string): string => String(s ?? "").replace(/Â/g, "Á").replace(/Ê/g, "É").replace(/Î/g, "Í").replace(/Ô/g, "Ó").replace(/Û/g, "Ú");
...
if (duplicados.length) console.warn(`OJO: ${duplicados.length} código/s duplicado/s, p. ej. ${duplicados.slice(0, 5).map(([c, n]) => `${c} (x${n})`).join(", ")}`);
