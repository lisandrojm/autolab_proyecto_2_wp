...
async function run() {
  ...
  for (const f of await FuenteParitaria.find({}).select("convenios").lean()) fuentes.set(String((f as any)._id), (f as any).convenios || []);
  ...
  for (const p of pubs) {
    ...
    */
    if (!p.archivo?.ruta || !(await existeArchivo(p.archivo.ruta))) {
      ...
      if (!DRY_RUN) {
        await PublicacionParitaria.updateOne(
          { _id: p._id },
          { $set: { "extraccion.estado": "error", "extraccion.motivo": `No se extrajo: ${motivo}.`, "extraccion.extraidoEl": new Date(), "extraccion.texto": "", "extraccion.paginas": 0 } },
        );
      }
      continue;
    }
    ...
    if (cotejo === "ajeno") hallazgos.push(`AJENO · ${etiqueta} → cita ${senales.conveniosMencionados.map((c) => c.valor).join(", ")}, la fuente alimenta ${deLaFuente.join(", ")}`);
    if (senales.unidadSospechosa) hallazgos.push(`UNIDAD · ${etiqueta} → «${senales.unidadSospechosa.fragmento}»`);
    if (r.estado === "vacio") hallazgos.push(`SIN TEXTO · ${etiqueta} → ${r.motivo}`);
    if (periodoOk === false) hallazgos.push(`PERÍODO · ${etiqueta} → el PDF dice «${senales.periodoMencionado?.valor}» y el enlace «${p.textoEnlace}»`);
