...
const MUDANZAS: Array<{ de: string; a: string }> = [
...
interface Respaldo {
  infoId: string;
  estado: string;
  carpetas: Array<{ dropboxCarpeta?: string; detalle?: string }>;
}
...
async function run() {
  ...
  for (const e of estados as any[]) {
    const carpetas = (e?.data?.transicionAutomatica?.carpetas || []) as Array<{ dropboxCarpeta?: string; detalle?: string }>;
    ...
    if (!DRY_RUN) await infos.updateOne({ _id: e._id }, { $set: { "data.transicionAutomatica.carpetas": nuevas } });
  }
  ...
  for (const e of estados as any[]) {
    for (const c of (e?.data?.transicionAutomatica?.carpetas || []) as any[]) {
      const p = String(c.dropboxCarpeta || "");
      if (/^\/AFIP\b/i.test(p) && !MUDANZAS.some((m) => m.de === p)) huerfanas.push(`${e.name}: ${p}`);
    }
  }
  ...
  if (cambiadas === 0) console.log("Las carpetas vigiladas ya apuntan al árbol nuevo. No hay nada que hacer.\n");
