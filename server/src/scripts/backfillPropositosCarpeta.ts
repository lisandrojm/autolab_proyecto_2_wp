...
const nombre = (c: Carpeta) => (c.dropboxCarpeta || "").split("/").filter(Boolean).pop() || "(sin ruta)";
...
async function run() {
  ...
  for (const e of estados as any[]) {
    const carpetas: Carpeta[] = e?.data?.transicionAutomatica?.carpetas || [];
    console.log(`«${e.name}»`);

    const nuevas: Carpeta[] = [];
    const asignadosEnEsteEstado = new Map<string, string>();
    ...
    if (cambia) {
      ...
      if (!DRY_RUN) await db.collection("infos").updateOne({ _id: e._id }, { $set: { "data.transicionAutomatica.carpetas": nuevas } });
    }
    console.log();
  }
  ...
  for (const e of estados as any[]) {
    for (const c of (e?.data?.transicionAutomatica?.carpetas || []) as Carpeta[]) {
      const p = esProposito(c.proposito) ? c.proposito : inferirPropositos(c)[0];
      if (p) cubiertos.add(p);
    }
  }
  ...
  const rutas = [...new Set((estados as any[]).flatMap((e) => (e?.data?.transicionAutomatica?.carpetas || []).map((c: Carpeta) => c.dropboxCarpeta)).filter(Boolean))] as string[];
  ...
  if (aEscribir === 0 && respaldos.length === 0) console.log("No hay nada que hacer.");
...
async function revertir(archivo: string) {
  ...
  await mongoose.connect(uri, { dbName });
  const db = mongoose.connection.db!;
  const respaldos: Respaldo[] = JSON.parse(fs.readFileSync(archivo, "utf8"));
