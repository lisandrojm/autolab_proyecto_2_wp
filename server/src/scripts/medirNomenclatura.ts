...
const CAMPOS_VARIABLES = ["proyecto", "apellido", "nombres", "contrato", "docName", "email", "empresa", "extra"] as const;

const pct = (n: number, total: number) => `${n} (${((n / total) * 100).toFixed(1)}%)`;
...
async function run() {
  ...
  const guardados = new Map((await db.collection("nomenclaturaarchivos").find({}).toArray()).map((f: any) => [f.tipo, f.patron]));
  const patronDe = (t: TipoNomenclatura) => guardados.get(t) || PATRON_POR_DEFECTO[t];

  const users = new Map((await db.collection("users").find({}).project({ lastName: 1, firstName: 1, email: 1, metadata: 1 }).toArray()).map((u: any) => [String(u._id), u]));
  const empresas = new Map((await db.collection("companies").find({}).project({ cuit: 1, razonSocial: 1 }).toArray()).map((c: any) => [String(c._id), c]));
  const plantillas = (await db.collection("contratos-frame").find({}).project({ name: 1 }).toArray()).map((f: any) => campoNomenclatura(f.name)).sort((a, b) => b.length - a.length);
  ...
  for (const tipo of TIPOS_NOMENCLATURA) {
    ...
    const medidos: Array<{ bytes: number; nombre: string; campos: Record<string, number> }> = [];
    for (const up of ups as any[]) {
      const u = users.get(String(up.userId));
      if (!u) continue;
      for (const c of up.contracts || []) {
        const emp: any = empresas.get(String(c.empresaContratoId || ""));
    ...
    if (exceden.length > 0) huboExceso = true;
    ...
    if (exceden.length > 0) console.log(`   SE PASAN: ${pct(exceden.length, medidos.length)} — el generador los va a recortar`);
