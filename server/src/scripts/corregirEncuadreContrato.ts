...
/**
 * Los contratos a corregir, identificados por documento y posición — no por (proyecto, rol), que acá
 * no alcanza: los dos casos son el MISMO proyecto y el MISMO rol, y solo uno se corrige.
 *
 * `catActual` es una aserción, no un dato: si el contrato ya no tiene esa categoría, el script frena.
 * Sin eso, correr esto dos veces —o después de que alguien lo arregle a mano— pisaría un encuadre
 * bueno con uno viejo, en silencio.
 */
type Correccion = {
  aplicar: boolean;
  quien: string;
...
const CORRECCIONES: Correccion[] = [
  {
    ...
    motivo:
      "Rol FRAME «Asistente de Cámara»: es técnico y va por SATSAID (0634/11). Estaba en APUNTADOR, " +
  ...
  },
  {
    ...
    */
    ...
    motivo:
      "Mismo rol «Asistente de Cámara» y mismo proyecto que el anterior, encuadrado en PERSONAJE " +
...
async function run() {
  ...
  for (const c of CORRECCIONES) {
    ...
    const destino: any = await Categoria.findOne({ convenio: c.destinoConvenio, codigoArca: c.destinoCodigo }).lean();
    if (!destino) {
    ...
    const up: any = await UserProject.findById(c.userProjectId).lean();
    if (!up) {
    ...
    if (Number(contrato.categoria_sat_id) !== c.catActual) {
      problemas.push(
        `YA CAMBIÓ: ${c.userProjectId}[${c.contractIndex}] tiene la categoría ${contrato.categoria_sat_id} («${contrato.nombre_categoria_sat}»), no la ${c.catActual} que esperaba este script. Alguien lo tocó: revisá antes de forzar.`
      );
      continue;
    }
    ...
    */
    const compat: any = await buscarCategoriaCompatPorLegacyId(Number(destino.legacyId));
    if (!compat) {
    ...
    if (sueldo_bruto === 0) console.log(`    ojo: la categoría destino no tiene escala cargada, los importes quedan en cero.`);
    console.log("");
  ...
  for (const e of plan) {
    if (!porDoc.has(e.userProjectId)) porDoc.set(e.userProjectId, []);
    porDoc.get(e.userProjectId)!.push(e);
  }

  let escritos = 0;
  for (const [id, entradas] of porDoc) {
    const doc: any = await UserProject.findById(id);
    if (!doc) continue;
