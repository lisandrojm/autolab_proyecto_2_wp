...
async function run() {
  ...
  const empresas = (await db.collection("companies").find({}).toArray()) as any[];
  const obrasSociales = (await db.collection("obras-sociales").find({}).toArray()) as any[];
  ...
  const porDataId = new Map<number, any>(obrasSociales.filter((o) => o.data?.id != null).map((o) => [Number(o.data.id), o]));
  ...
  for (const e of empresas) {
    ...
    if (e.obraSocialId !== undefined) {
      unset.obraSocialId = "";
      if (e.obraSocialDefaultId === undefined && e.obraSocialId != null) set.obraSocialDefaultId = e.obraSocialId;
      renombradas++;
    }
  ...
  if (sinMatch.length > 0) console.log(`\n⚠ Revisar:\n   ${sinMatch.join("\n   ")}`);
