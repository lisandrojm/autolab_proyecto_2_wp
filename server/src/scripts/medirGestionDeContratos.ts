...
async function main() {
  ...
  const estados: any[] = await Info.find({ type: "estado-empleado" }).lean();
  const impositivos = estados.filter((e: any) => e.data?.esImpositivo && e.name).map((e: any) => e.name);
  console.log(`Estados impositivos: ${impositivos.join(", ") || "(ninguno)"}\n`);

  const medir = async (titulo: string, filtroProyecto: any) => {
    let t = Date.now();
    const projectsList: any[] = await Project.find({ tenantId: new Types.ObjectId(tenantId), ...filtroProyecto }).select("_id name clientId contratoEmpresas releaseEmpresas").lean();
    ...
    const enBandeja = membershipsRaw.reduce((s, m) => s + (m.c || []).filter((c: any) => impositivos.includes(String(c.e || ""))).length, 0);
  ...
  if (projectId) await medir(`ACOTADO AL PROYECTO que se está mirando`, { _id: new Types.ObjectId(projectId) });
