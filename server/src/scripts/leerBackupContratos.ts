...
const main = async () => {
  ...
  for (const coleccion of ["contratos", "contratos-frame"]) {
    ...
    for (const d of faltan) console.log(`   · ${d.name}   _id=${String(d._id)}`);
    ...
    if (posteriores.length > 0) {
      console.log(`   (y ${posteriores.length} que hoy existen y no estaban en la copia: ${posteriores.map((d: any) => d.name).join(", ")})`);
    }
    console.log("");
  }
