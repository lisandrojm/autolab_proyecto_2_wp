...
interface Vigencia {
  convenio: string;
  /** `null` = no está documentado y NO se escribe. Ver el comentario de cada caso. */
  desde: string | null;
  hasta: string;
  fuente: string;
}
 ...
 */
const COLECCIONES = ["convenio-grupos", "categorias"] as const;
...
async function run() {
  ...
  for (const v of VIGENCIAS) {
    ...
    if (!v.desde) console.log(`   (sin «desde» documentado: no se escribe, y por eso tampoco se toca fechaActualizacion)`);
    ...
    for (const coleccion of COLECCIONES) {
      ...
      */
      ...
      for (const d of docs as any[]) {
        ...
        if (!DRY_RUN) {
          const set: Record<string, unknown> = { vigenciaHasta: v.hasta };
          if (v.desde) {
...
async function revertir(archivo: string) {
  ...
  for (const r of respaldos) {
    ...
    for (const campo of ["vigenciaDesde", "vigenciaHasta", "fechaActualizacion"] as const) {
      // `null` en el respaldo = el campo NO estaba. Volver a escribirlo como null no sería revertir.
      if (r[campo] === null) unset[campo] = 1;
      else set[campo] = r[campo];
    }
