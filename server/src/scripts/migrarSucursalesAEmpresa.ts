...
function leerSedesACrear(raw: string): Map<string, string> {
  const out = new Map<string, string>();
  for (const par of raw.split(",").map((s) => s.trim()).filter(Boolean)) {
    const i = par.indexOf(":");
    if (i <= 0) throw new Error(`CREAR_SEDES mal formado en "${par}". Se espera codigoSucursal:Nombre, ej. 00001:Zapiola`);
    const codigo = par.slice(0, i).trim();
    const nombre = par.slice(i + 1).trim();
    if (!codigo || !nombre) throw new Error(`CREAR_SEDES mal formado en "${par}". Se espera codigoSucursal:Nombre, ej. 00001:Zapiola`);
...
function leerMapeo(raw: string): Map<string, number> {
  const out = new Map<string, number>();
  for (const par of raw.split(",").map((s) => s.trim()).filter(Boolean)) {
    const [codigo, sedeId] = par.split(":").map((s) => s.trim());
    if (!codigo || !sedeId) throw new Error(`MAPEO mal formado en "${par}". Se espera codigoSucursal:sedeId, ej. 00002:4`);
    const id = Number(sedeId);
    if (!Number.isFinite(id)) throw new Error(`MAPEO: "${sedeId}" no es un data.id de sede válido`);
    out.set(codigo, id);
  }
  return out;
}
...
function leerActividades(ruta: string): Map<string, Array<{ codigo: string; descripcion: string }>> {
  ...
  const idx = (n: string) => {
    ...
    if (i < 0) throw new Error(`Al CSV le falta la columna "${n}"`);
    return i;
  };
  ...
  const out = new Map<string, Array<{ codigo: string; descripcion: string }>>();
  for (const f of filas) {
    if (f[iTabla] !== "ACTIVIDAD_DOMICILIO") continue;
    const sucursal = String(f[iPadre] || "").trim();
    const codigo = String(f[iPadded] || "").trim();
    if (!sucursal || !codigo) continue;
    const lista = out.get(sucursal) || [];
    // Una misma sucursal puede tener varias actividades (es el caso que motivó todo esto).
    if (!lista.some((a) => a.codigo === codigo)) lista.push({ codigo, descripcion: String(f[iDesc] || "").trim() });
    out.set(sucursal, lista);
  }
  return out;
}

type SedeDoc = { _id: unknown; name?: string; data?: { id?: number; nombre?: string; codigoSucursal?: string } };
...
async function run() {
  ...
  const actividadesPorSucursal = CSV_PATH ? leerActividades(CSV_PATH) : new Map<string, Array<{ codigo: string; descripcion: string }>>();
  ...
  const sedes = (await db.collection("infos").find({ type: "sede" }).toArray()) as unknown as SedeDoc[];
  ...
  const pares: Array<{ sedeId: number; codigo: string; origen: "legado" | "mapeo" | "creada" }> = [];

  for (const sede of sedes) {
    const codigo = String(sede.data?.codigoSucursal || "").trim();
    const sedeId = Number(sede.data?.id);
    if (!codigo || !Number.isFinite(sedeId)) continue;
  ...
  for (const [codigo, sedeId] of mapeo) {
    ...
    if (i >= 0) pares[i] = { sedeId, codigo, origen: "mapeo" };
    else pares.push({ sedeId, codigo, origen: "mapeo" });
  }
  ...
  if (aCrear.size > 0) {
    ...
    for (const [codigo, nombre] of aCrear) {
      ...
      if (!DRY_RUN) await db.collection("infos").insertOne({ ...doc, createdAt: new Date(), updatedAt: new Date() });
      porSedeIdCatalogo.set(sedeId, doc as unknown as SedeDoc);
      sedes.push(doc as unknown as SedeDoc);
  ...
  if (pares.length === 0) {
    ...
    for (const s of sedes) console.log(`  ${s.data?.id} = ${s.name}`);
    if (actividadesPorSucursal.size > 0) {
      console.log("\nSucursales del padrón que trae el CSV:");
      for (const [codigo, acts] of actividadesPorSucursal) console.log(`  ${codigo} → ${acts.map((a) => a.codigo).join(", ")}`);
    }
  ...
  const actuales: SedeEmpresa[] = Array.isArray(empresa.sedes) ? (empresa.sedes as SedeEmpresa[]) : [];
  const porSedeId = new Map<number, SedeEmpresa>(actuales.map((s) => [Number(s.sedeId), s]));
  ...
  for (const { sedeId, codigo, origen } of pares) {
    ...
    if (!existente) {
      ...
      if (existente.codigoSucursal !== codigo) console.log(`- ${etiqueta}: ya registrada con ${existente.codigoSucursal || "(vacío)"} → se actualiza a ${codigo}.`);
      if (existente.actividades.length === 0 && delCsv.length > 0) console.log(`- ${etiqueta}: se cargan ${delCsv.length} actividad(es) del CSV: ${delCsv.map((a) => a.codigo).join(", ")}.`);
      existente.codigoSucursal = codigo;
      if (existente.actividades.length === 0) existente.actividades = delCsv;
      actualizadas++;
    } else {
      sinCambios++;
