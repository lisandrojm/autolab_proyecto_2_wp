...
import { liquidacionDeReferencia, AdicionalVigente } from "./liquidacionReferencia.js";
...
const ANTIGUEDAD: AdicionalVigente = { codigo: "antiguedad", nombre: "Antigüedad", tipoCalculo: "por_anio_antiguedad", remunerativo: null, confirmado: false, monto: 10570.92, unidad: "por año" };
const COMIDAS: AdicionalVigente = { codigo: "comidas", nombre: "Comidas", tipoCalculo: "por_evento", remunerativo: null, confirmado: false, monto: 7836.14, unidad: "por comida" };
const GUARDERIA: AdicionalVigente = { codigo: "guarderia", nombre: "Guardería", tipoCalculo: "mensual", remunerativo: null, confirmado: false, monto: 111786.76 };
...
describe("liquidacionDeReferencia — grupo 1, junio 2026", () => {
  ...
  it("3 años de antigüedad y 2 comidas", () => {
    ...
    assert.deepEqual(
      l.lineas.map((x) => [x.codigo, x.cantidad, x.monto]),
      [
  ...
  it("un adicional confirmado como remunerativo sí entra en la base", () => {
    const confirmado: AdicionalVigente = { ...ANTIGUEDAD, remunerativo: true, confirmado: true };
  ...
  it("uno no remunerativo se paga y no tributa", () => {
    const noRem: AdicionalVigente = { ...COMIDAS, remunerativo: false, confirmado: true };
...
describe("cómo se cuenta cada tipo de cálculo", () => {
  ...
  it("un porcentaje se calcula sobre la base que se le diga", () => {
    const porBasico: AdicionalVigente = { codigo: "x", nombre: "Título", tipoCalculo: "porcentaje", remunerativo: true, confirmado: true, monto: null, porcentaje: 10, base: "basico" };
    const l = liquidacionDeReferencia(ESCALA_G1_JUNIO, [porBasico], { cantidades: { x: 1 } });
    assert.equal(l.lineas[0].monto, 118720.86);

    const porTotal: AdicionalVigente = { ...porBasico, base: "total" };
  ...
  it("un porcentaje sin porcentaje cargado queda afuera y se avisa", () => {
    const roto: AdicionalVigente = { codigo: "x", nombre: "Título", tipoCalculo: "porcentaje", remunerativo: true, monto: null, porcentaje: null, base: "basico" };
