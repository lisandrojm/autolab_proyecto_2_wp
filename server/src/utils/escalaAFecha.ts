...
export const aIsoFecha = (v: unknown): string => {
  if (!v) return "";
...
export const diaAnterior = (iso: string): string => {
...
export function rigeEn(periodo: ConVigencia, fecha: string): boolean {
  const desde = aIsoFecha(periodo.desde);
  if (!desde || !fecha) return false;
  if (fecha < desde) return false;
  const hasta = aIsoFecha(periodo.hasta);
  if (!hasta) return true; // sin vencimiento declarado: sigue vigente
  return fecha <= hasta;
}
 ...
 */
export function periodoVigente<T extends ConVigencia>(periodos: T[], fecha: string): T | null {
...
export function ordenarPorVigencia<T extends ConVigencia>(periodos: T[]): T[] {
 ...
 */
export function periodosSuperpuestos(periodos: ConVigencia[]): Superposicion[] {
  const legibles = periodos.filter((p) => aIsoFecha(p.desde));
  const out: Superposicion[] = [];
  for (let i = 0; i < legibles.length; i++) {
    for (let j = i + 1; j < legibles.length; j++) {
      ...
      if (!hastaDelPrimero || desdeDelSegundo <= hastaDelPrimero) out.push({ a: arrancaAntes, b: arrancaDespues, desde: desdeDelSegundo });
    }
  }
  return out;
}
 ...
 */
export function vigenciaIncoherente(periodo: ConVigencia): boolean {
  const desde = aIsoFecha(periodo.desde);
  const hasta = aIsoFecha(periodo.hasta);
