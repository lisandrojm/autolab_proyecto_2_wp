...
describe("a qué grupo pertenece un tipo de servicio", () => {
  it("por debajo de 500 es CONTINUOS", () => {
    for (const c of ["000", "001", "100", "499"]) assert.equal(grupoDeTipoServicio(c), GRUPO_CONTINUOS, c);
  });

  it("de 500 para arriba es DISCONTINUOS", () => {
    for (const c of ["500", "514", "550", "557", "560"]) assert.equal(grupoDeTipoServicio(c), GRUPO_DISCONTINUOS, c);
  });
  ...
  it("sin código no hay grupo", () => {
    for (const c of ["", "   ", null, undefined, "sin código"]) assert.equal(grupoDeTipoServicio(c), "", String(c));
  });
});
