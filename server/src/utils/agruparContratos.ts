 ...
 */
export function agruparContratosPorDocumento(pedidos: unknown): AgrupadoPorDocumento {
  const porDocumento = new Map<string, ContratoPedido[]>();
  const invalidos: ContratoPedido[] = [];

  for (const p of Array.isArray(pedidos) ? pedidos : []) {
    const item: ContratoPedido = {
      projectId: String((p as any)?.projectId || ""),
      userId: String((p as any)?.userId || ""),
      contratoId: String((p as any)?.contratoId ?? ""),
    };
...
export function partirClaveDocumento(clave: string): { projectId: string; userId: string } {
