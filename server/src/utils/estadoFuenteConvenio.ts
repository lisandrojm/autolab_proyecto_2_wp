...
/** Los cuatro estados posibles. `con_fuente` es el único que no se puede declarar. */
...
export const ESTADOS_DECLARABLES = ["sin_revisar", "sin_fuente_conocida", "no_aplica"] as const;
export type EstadoDeclarado = (typeof ESTADOS_DECLARABLES)[number];

export const esDeclarable = (v: unknown): v is EstadoDeclarado => ESTADOS_DECLARABLES.includes(String(v) as EstadoDeclarado);
 ...
 */
export const estadoFuenteDe = (tieneFuenteAsignada: boolean, declarado?: string | null): EstadoFuenteConvenio => {
  if (tieneFuenteAsignada) return "con_fuente";
 ...
 */
export const ETIQUETA_ESTADO_FUENTE: Record<EstadoFuenteConvenio, string> = {
