...
const tieneEscala = (x: any): boolean => Number(x?.sueldoBruto || 0) > 0;

const leer = (x: any, origen: "categoria" | "grupo"): EscalaSalarial => ({
 ...
 */
export const escalaDeCategoria = (categoria: any, grupo?: any): EscalaSalarial => {
  if (tieneEscala(categoria)) return leer(categoria, "categoria");
  if (tieneEscala(grupo)) return leer(grupo, "grupo");
  return { ...VACIA };
};

/** `true` si el alta se puede generar con esta categoría. Sin escala, no hay retribución que declarar. */
export const tieneEscalaResuelta = (categoria: any, grupo?: any): boolean => escalaDeCategoria(categoria, grupo).origen !== null;
