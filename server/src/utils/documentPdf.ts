...
const mimeDeImagen = (ext: string): string => MIME_POR_EXTENSION[ext] || `image/${ext}`;

function imagePathToDataUri(url?: string | null): string | null {
  if (!url) return null;
  try {
...
function buildMembreteHeader(m: MembreteInput): string {
...
function buildFirmaFooter(m: MembreteInput): string {
...
export function empresaToMembrete(empresa: any): MembreteInput {
  const e: any = empresa || {};
...
function wrapHtml(bodyHtml: string, membrete?: MembreteInput, resaltarVariables?: boolean): string {
 ...
 */
export async function buildDocPdf(content: string, data: Record<string, any>, membrete?: MembreteInput, opciones?: { resaltarVariables?: boolean }): Promise<Buffer> {
