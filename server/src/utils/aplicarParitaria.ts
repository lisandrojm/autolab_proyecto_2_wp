...
import { calcularEscalaGrupo, EscalaCalculada, redondearCentavos, TOLERANCIA_CENTAVO } from "./escalaCalculo.js";

/** Aplicar un porcentaje a un importe. 4,8 se pasa como `4.8`. */
export const aumentar = (monto: number, porcentaje: number): number => redondearCentavos(Number(monto || 0) * (1 + Number(porcentaje || 0) / 100));
 ...
 */
export function proponerEscala(filas: FilaEscalaActual[], porcentaje: number, esperado: EsperadoDelActa[] = [], tolerancia: number = TOLERANCIA_CENTAVO): FilaEscalaPropuesta[] {
  const porGrupo = new Map(esperado.map((e) => [String(e.grupo ?? ""), e]));

  return filas.map((f) => {
    const basicoPropuesto = aumentar(f.basico, porcentaje);
    const avisos: string[] = [];
    let propuesta: EscalaCalculada;
    ...
    if (esp?.basico != null) {
      ...
      if (Math.abs(delta) >= tolerancia) avisos.push(`El básico del acta (${esp.basico}) difiere en ${delta} del que da el porcentaje.`);
    }
    if (esp?.total != null) {
      const delta = redondearCentavos(Number(esp.total) - propuesta.total);
      if (Math.abs(delta) >= tolerancia) avisos.push(`El total del acta (${esp.total}) difiere en ${delta} del calculado.`);
    }
 ...
 */
export function proponerMontos(valores: MontoActual[], porcentaje: number, esperado: Array<{ clave: string; grupo?: number | null; monto: number }> = [], tolerancia: number = TOLERANCIA_CENTAVO): MontoPropuesto[] {
  const llave = (clave: string, grupo?: number | null) => `${clave}|${grupo ?? ""}`;
  const porClave = new Map(esperado.map((e) => [llave(e.clave, e.grupo), e.monto]));

  return valores.map((v) => {
    if (v.monto == null) return { ...v, montoPropuesto: null, avisos: ["Sin importe vigente: no hay de dónde sacar el aumento."] };
    ...
    if (esp != null) {
      ...
      if (Math.abs(delta) >= tolerancia) avisos.push(`El acta dice ${esp} (${delta > 0 ? "+" : ""}${delta} contra el porcentaje).`);
    }
 ...
 */
export function porcentajeAcumulado(porcentajes: number[]): number {
