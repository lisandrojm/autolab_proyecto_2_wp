...
const soloCuit = (raw: string): string => {
 ...
 */
export const normalizarEmail = (email: string): string => String(email || "").toLowerCase().replace(/[^a-z0-9]/g, "");

/** El email que trae el nombre, con su "@" de vuelta. "" si no hay ninguno reconocible. */
function leerEmail(nombre: string): string {
...
export function leerAnclas(nombreArchivo: string): AnclasNombre {
  ...
  while ((m = re.exec(nombre))) fechas.push(m[1]);
 ...
 */
export function mismoDocumento(a: AnclasNombre, b: AnclasNombre): boolean {
  if (!mismaPersona(a, b)) return false;
  ...
  if (a.documento && b.documento && a.documento !== b.documento) return false;
 ...
 */
export function mismaPersona(a: AnclasNombre, b: AnclasNombre): boolean {
  if (a.cuit && b.cuit) return a.cuit === b.cuit;
 ...
 */
function mismasFechas(a: string[], b: string[]): boolean {
 ...
 */
export function esCampoAncla(campo: string): boolean {
