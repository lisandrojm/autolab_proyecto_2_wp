...
const g = (over: Record<string, unknown> = {}) => ({ convenio: "0322/75", donde: "categoria" as const, nombre: "TIRA", sueldoBruto: 100000, vigenciaHasta: "2026-08-31", ...over });
...
describe("cómo se agrupa para mostrarlo", () => {
  it("por convenio, con la más vieja al frente", () => {
    const filas = escalasVencidas(
      [g({ convenio: "0322/75", vigenciaHasta: "2026-08-31" }), g({ convenio: "0322/75", nombre: "UNITARIO", vigenciaHasta: "2026-08-31" }), g({ convenio: "0102/90", nombre: "PROTAGONISTA A", vigenciaHasta: "2026-06-30" })],
      HOY,
    );
    const porConv = vencidasPorConvenio(filas);
    // El problema es del CCT, no de cada fila: dos escalas de 0322/75 son un solo aviso.
    assert.deepEqual(
      porConv.map((c) => [c.convenio, c.escalas]),
      [
        ["0102/90", 1],
        ["0322/75", 2],
      ],
