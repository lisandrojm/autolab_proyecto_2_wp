 ...
 */
export const resolverCategoriasCompatPorId = async (ids: any[]): Promise<CategoriaCompat[]> => {
  if (!Array.isArray(ids) || ids.length === 0) return [];

  const [cats, grupos] = await Promise.all([Categoria.find({ _id: { $in: ids } }).lean(), ConvenioGrupo.find().lean()]);
  const porGrupo = new Map(grupos.map((g: any) => [String(g._id), g]));
  const resueltas = cats.map((c: any) => aplanar(c, porGrupo.get(String(c.grupoId))));

  const encontrados = new Set(resueltas.map((c) => String(c._id)));
  const faltantes = ids.filter((id) => !encontrados.has(String(id)));
  if (faltantes.length > 0) {
    const viejas = (await CategoriaSat.find({ _id: { $in: faltantes } }).lean()) as any[];
...
export const buscarCategoriaCompatPorLegacyId = async (legacyId: number): Promise<CategoriaCompat | null> => {
  ...
  const c: any = await Categoria.findOne({ legacyId }).lean();
  if (c) {
