...
import { resolverValoracion, ValoracionParaResolver } from "./valoracionAutomatica.js";

/** El caso real de la pantalla: Plata hasta 20 % de margen, Oro del 20 % para arriba. */
const PLATA: ValoracionParaResolver = { _id: "plata", orden: 1, margenDesde: null, margenHasta: 20, esDefault: true, activo: true };
const ORO: ValoracionParaResolver = { _id: "oro", orden: 2, margenDesde: 20, margenHasta: null, esDefault: false, activo: true };
const ESCALA = [PLATA, ORO];

const nombre = (v: ValoracionParaResolver | null) => (v ? String(v._id) : null);
...
describe("resolverValoracion — el rango", () => {
  ...
  it("0 es un margen válido —trabajar sin ganancia— y entra por rango, no por default", () => {
    const conMinimo: ValoracionParaResolver[] = [
...
describe("resolverValoracion — la default", () => {
  ...
  it("un margen que no entra en ningún rango cae en la default", () => {
    const conHueco: ValoracionParaResolver[] = [
  ...
  it("una valoración SIN rango no se come todos los montos", () => {
    ...
    const conComodin: ValoracionParaResolver[] = [
  ...
  it("sin rango que aplique y sin default, null: no se inventa una", () => {
    const sinDefault: ValoracionParaResolver[] = [{ _id: "alta", orden: 1, margenDesde: 30, margenHasta: null, activo: true }];
...
describe("resolverValoracion — inactivas y orden", () => {
  it("una valoración inactiva no se elige, aunque su rango contenga el margen", () => {
    const conApagada: ValoracionParaResolver[] = [{ _id: "apagada", orden: 1, margenDesde: 0, margenHasta: 100, activo: false }, ORO];
  ...
  it("`activo` ausente cuenta como activa", () => {
    ...
    const sinCampo: ValoracionParaResolver[] = [{ _id: "vieja", orden: 1, margenDesde: 0, margenHasta: null }];
  ...
  it("con dos rangos que se tocan, gana el de `orden` más bajo", () => {
    ...
    const solapadas: ValoracionParaResolver[] = [
  ...
  it("las que no tienen `orden` van al final", () => {
    const sinOrden: ValoracionParaResolver[] = [
