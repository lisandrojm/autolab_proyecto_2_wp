...
import { CAMPOS_DE_CONDICIONES, PlantillaEquipo, IPlantillaEquipo } from "../models/PlantillaEquipo.js";
...
export function puestoEnEquipo(puesto: any, asignacion: any) {
 ...
 */
async function condicionesDesde(p: any, puesto: any, body: any, acc: Acceso): Promise<Record<string, any> | null> {
  ...
  for (const campo of CAMPOS_DE_CONDICIONES) {
    ...
    if (comparable(pedido[campo]) !== comparable(puesto[campo])) c[campo] = pedido[campo];
  }
 ...
 */
function avisosDelEquipo(p: any, equipo: any, porDiasSueltos: (contratoId: string) => boolean): Record<string, string[]> {
  const puestos = [...(p.integrantes || [])].sort((a: any, b: any) => (a.orden ?? 0) - (b.orden ?? 0));
  const choques = choquesDelEquipo(
    puestos.map((i: any, n: number) => {
...
async function contratosPorDiasSueltos(plantillas: any[]): Promise<(contratoId: string) => boolean> {
  ...
  for (const p of plantillas) {
    ...
    for (const e of p.equipos || []) for (const a of e.asignaciones || []) if (a.condiciones?.contratoId) ids.add(String(a.condiciones.contratoId));
  }
...
export async function listarPlantillas(acc: Acceso, projectId: string) {
  ...
  return lista.map((p) => ({
    ...
    equipos: (p.equipos || []).map((e: any) => ({
      _id: String(e._id),
      nombre: e.nombre,
      asignados: (e.asignaciones || []).filter((a: any) => a.userId).length,
      // Cuántos puestos tienen condiciones propias en este equipo (otro horario, otro turno…).
      propias: (e.asignaciones || []).filter((a: any) => a.condiciones && Object.keys(a.condiciones).length).length,
...
export async function obtenerPlantilla(acc: Acceso, id: string) {
  ...
  const userIds = (p.equipos || []).flatMap((e: any) => (e.asignaciones || []).flatMap((a: any) => [a.userId, a.reemplazadoDePersonaId])).filter(Boolean);
  const [personas, porDiasSueltos]: [any[], (id: string) => boolean] = await Promise.all([
...
async function copiarComo(acc: Acceso, origen: any, destino: { alcance: Acceso["alcance"]; projectId: Types.ObjectId | null; nombre: string; conEquipos: boolean }) {
  ...
  const equipos = destino.conEquipos
    ? (origen.equipos || []).map((e: any) => ({ nombre: e.nombre, asignaciones: (e.asignaciones || []).filter((a: any) => mapa.has(String(a.puestoId))).map((a: any) => ({ ...a, puestoId: mapa.get(String(a.puestoId)) })), ultimaContratacionEl: null }))
...
export async function crearEquipo(acc: Acceso, id: string, nombre: string, copiarDeId?: string) {
  ...
  p.equipos.push({ nombre: n, asignaciones: origen ? origen.asignaciones.map((a: any) => ({ puestoId: a.puestoId, userId: a.userId, condiciones: a.condiciones || null })) : [] } as any);
  await p.save();
  return obtenerPlantilla(acc, id);
}
...
export async function asignarPuesto(acc: Acceso, id: string, equipoId: string, puestoId: string, userId: string | null) {
  ...
  if (userId === null || userId === "") {
    if (actual?.condiciones) Object.assign(actual, { userId: null });
    else e.asignaciones = e.asignaciones.filter((a: any) => String(a.puestoId) !== String(puestoId));
  } else {
    ...
    if (actual) {
      if (!actual.userId) actual.userId = oid(userId);
      else if (String(actual.userId) !== String(userId)) Object.assign(actual, { reemplazadoDePersonaId: actual.userId, reemplazadoEl: new Date(), userId: oid(userId) });
...
export async function condicionesEnEquipo(acc: Acceso, id: string, equipoId: string, puestoId: string, body: any) {
  ...
  if (actual) {
    ...
    if (!condiciones && !actual.userId) e.asignaciones = e.asignaciones.filter((a: any) => a !== actual);
...
function paraPlan(p: any, equipoId: string): { plantilla: PlantillaParaPlan; integrantes: IntegranteParaPlan[] } {
  ...
  return {
    ...
    integrantes: [...(p.integrantes || [])]
      ...
      .map((puesto: any) => ({ a: asignacion.get(String(puesto._id)), i: puestoEnEquipo(puesto, asignacion.get(String(puesto._id))) }))
      .map(({ a, i }: any) => ({
        _id: String(i._id),
...
export async function planificar(tenantId: Types.ObjectId, p: any, body: any): Promise<PlanDeLote> {
  ...
  const conFechas = borrador.filas.filter((f) => !f.excluido && f.datos && (f.datos.startDate || f.datos.fechasTrabajadas.length));
  const pedidos = new Map(conFechas.map((f) => [f.integranteId, pedidoDesdeSolicitud(armarPayloadDeSolicitud(f.datos!).metadata)]));
  const numero = new Map(integrantes.map((i, n) => [i._id, n + 1]));
  const hoy = hoyArgentina();
  await Promise.all(
    conFechas.map(async (f) => {
      const pedido = pedidos.get(f.integranteId)!;
      const avisos = await superposicionesDeAlta(tenantId, f.userId, pedido);
      // La misma persona en otro puesto de ESTA contratación: todavía no existe en la base, se compara acá.
      const otrosPuestos: CompromisoExistente[] = conFechas
        .filter((o) => o.integranteId !== f.integranteId && o.userId === f.userId)
        .map((o) => {
          const x = pedidos.get(o.integranteId)!;
          return { origen: "lote", proyectoNombre: `puesto ${numero.get(o.integranteId)}`, desde: x.desde, hasta: x.hasta, fechas: x.fechas, dias: x.dias, rotativos: x.rotativos, inTime: x.inTime, outTime: x.outTime, shiftIds: x.shiftIds };
        });
      const delLote = superposiciones(pedido, otrosPuestos, hoy);
      ctx.superposiciones.set(f.integranteId, [...delLote, ...avisos].map((a) => ({ tipo: a.tipo, mensaje: a.mensaje })));
    }),
  );
 ...
 */
async function asignacionesConCambios(p: any, equipo: any, body: any) {
  const { puntuales } = leerContratacion(body);
  const asignaciones = (equipo.asignaciones || []).map((a: any) => ({ ...a }));
  for (const [puestoId, x] of Object.entries(puntuales)) {
    if (x.excluido) continue;
    const puesto = (p.integrantes || []).find((i: any) => String(i._id) === puestoId);
    if (!puesto) continue;
    let a = asignaciones.find((y: any) => String(y.puestoId) === puestoId);
    if (!a && (x.userId || x.inTime || x.outTime || x.categoriaSatId || x.dailyRate)) asignaciones.push((a = { puestoId: oid(puestoId), userId: null, condiciones: null }));
    if (!a) continue;
    if (x.userId) {
      if (a.userId && String(a.userId) !== x.userId) Object.assign(a, { reemplazadoDePersonaId: a.userId, reemplazadoEl: new Date() });
      a.userId = oid(x.userId);
    }
    const pisado: any = {};
    if (x.inTime) pisado.inTime = x.inTime;
    if (x.outTime) pisado.outTime = x.outTime;
    if (x.categoriaSatId) pisado.categoriaSatId = x.categoriaSatId;
    if (x.dailyRate) pisado.dailyRateManual = x.dailyRate;
...
export async function contratarPlantilla(acc: Acceso, id: string, body: any) {
  ...
  try {
    await sesion.withTransaction(async () => {
      ...
      const set: any = { ultimaContratacionEl: new Date(), ultimoLoteId: loteId };
      if (equipo) {
        const idx = plantillaObj.equipos.findIndex((e: any) => String(e._id) === equipoId);
        set[`equipos.${idx}.ultimaContratacionEl`] = new Date();
        if (body?.guardarEnEquipo === true) set[`equipos.${idx}.asignaciones`] = await asignacionesConCambios(p, equipo, body);
      }
