 ...
 */
function aplicarCondicionesPorPuesto(p: any, e: any, porPuesto: any, acc: Acceso) {
  if (!porPuesto || typeof porPuesto !== "object") return;
  for (const [puestoId, cond] of Object.entries(porPuesto)) {
    ...
    let a = (e.asignaciones || []).find((x: any) => String(x.puestoId) === String(puestoId));
    if (!a) e.asignaciones.push((a = { puestoId: oid(puestoId), userId: null, condiciones: null, excluido: false }));
