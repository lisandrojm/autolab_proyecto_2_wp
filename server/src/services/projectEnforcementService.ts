...
export async function enforceCoordinatorArea(project: IProject): Promise<void> {
  ...
  const allShiftIds = shifts.map((s) => s._id as Types.ObjectId);
  ...
  const existingIndex = project.areasConfig.findIndex(
    (ac) => ac.areaId.toString() === coordinatorArea._id.toString()
  );
