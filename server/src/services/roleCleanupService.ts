 ...
 */
export async function cleanupDuplicateMobileRoles(tenantId: Types.ObjectId | string, dryRun: boolean = false): Promise<any> {
  const tid = new Types.ObjectId(tenantId);
  const log: string[] = [];
  const report: any = { movedUsers: 0, deletedRoles: 0 };
