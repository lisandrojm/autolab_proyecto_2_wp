...
const generales = (extra: Record<string, unknown> = {}) => ({ idPersona: 27371836646, tipoPersona: "FISICA", tipoClave: "CUIT", estadoClave: "ACTIVO", ...extra });
...
describe("§6.2 — responsable inscripto", () => {
  it("con IVA activo y sin bloque de monotributo", () => {
    const r = derivarCondicionFiscal(
      { datosGenerales: generales(), datosRegimenGeneral: { impuesto: [{ idImpuesto: 30, descripcionImpuesto: "IVA", estadoImpuesto: "AC", periodo: 201501 }] }, errorMonotributo: { mensaje: "No cumple con las condiciones" } },
      CONSULTA,
    );
...
describe("§6.3 — CUIL de empleado: el caso más común, y NO es un error", () => {
  it("con los dos errores, es SIN_ACTIVIDAD", () => {
    const r = derivarCondicionFiscal(
      { datosGenerales: generales({ tipoClave: "CUIL" }), errorMonotributo: { mensaje: "No cumple con las condiciones para enviar datos monotributo" }, errorRegimenGeneral: { mensaje: "No cumple con las condiciones para enviar datos regimen general" } },
      CONSULTA,
    );
...
describe("formas en que ARCA devuelve lo mismo", () => {
  ...
  it("saca la letra de la categoría escrita de varias formas", () => {
    const conLetra = (desc: string) => derivarCondicionFiscal({ datosMonotributo: { categoriaMonotributo: { descripcionCategoria: desc } } }, CONSULTA).monotributo?.categoria;
