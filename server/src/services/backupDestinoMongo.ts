...
import { backupDbName, backupDbNameConFecha, selloFecha, esBaseDeCopiaConFecha, siguienteSlot, preflight, backupMeta, esBaseDeCopia, Slot, MAX_DB_BYTES, MAX_NS_BYTES, MAX_COLECCIONES } from "../utils/nombreBackup.js";
...
/*
  ...
  para poder probarse.
*/
const numeroDeEnv = (clave: string, porDefecto: number): number => {
 ...
 * fecha no entre.
 */
const estrategia = (): "fecha" | "slots" => (String(process.env.MONGO_BACKUP_ESTRATEGIA || "fecha").trim() === "slots" ? "slots" : "fecha");
...
export function nombreDeBaseCopia(baseOrigen: string, slot: Slot, fecha: Date = new Date()): string {
...
export function proximaBaseCopia(baseOrigen: string, ultimoSlotOk?: Slot | null): { base: string; bytes: number; maximo: number } {
 ...
 */
export async function clonarEnMongo(baseOrigen: string, colecciones: string[], ultimoSlotOk?: Slot | null, baseAnterior?: string | null): Promise<ResultadoDestinoMongo> {
  ...
  */
  ...
  try {
    ...
    await destino.collection(COLECCION_MANIFIESTO).insertOne(backupMeta({ baseOrigen, slot, colecciones, documentos }) as any);
    ...
    */
    const prefijo = prefijoDeBase(baseOrigen);
    const max = limites().maxDbBytes;
    const esCopiaNuestra = (n: string) => n !== baseOrigen && n !== base && (esBaseDeCopiaConFecha(n, prefijo, max) || esBaseDeCopia(n, prefijo, max));

    const aBorrar = new Set<string>();
    if (baseAnterior && esCopiaNuestra(baseAnterior)) aBorrar.add(baseAnterior);
    try {
      const { databases } = await conexion.getClient().db().admin().listDatabases();
      for (const d of databases) if (esCopiaNuestra(String(d.name))) aBorrar.add(String(d.name));
    } catch {
