...
import { FuenteParitaria, IFuenteParitaria } from "../models/FuenteParitaria.js";
...
const limpiarTexto = (html: string): string =>
  html
    .replace(/<[^>]*>/g, " ")
 ...
 */
export function extraerEnlacesPdf(html: string, urlBase: string): EnlacePdf[] {
  const encontrados = new Map<string, EnlacePdf>();
  for (const m of html.matchAll(/<a[^>]+href=["']([^"']+\.pdf)["'][^>]*>([\s\S]*?)<\/a>/gi)) {
    ...
    if (!encontrados.has(url)) encontrados.set(url, { url, texto: limpiarTexto(m[2]) });
  }
  return [...encontrados.values()];
}

/** Compila un patrón del ABM. Uno inválido no puede tirar la revisión: se trata como «no filtra». */
const compilar = (patron: string | undefined, porDefecto: RegExp | null): RegExp | null => {
 ...
 */
export function filtrarEnlaces(enlaces: EnlacePdf[], fuente: Pick<IFuenteParitaria, "patronIncluir" | "patronExcluir">): EnlacePdf[] {
  ...
  return enlaces.filter((e) => {
    ...
    if (incluir && !incluir.test(contra)) return false;
    if (excluir && excluir.test(contra)) return false;
    return true;
  });
}
 ...
 */
export async function revisarFuente(fuenteId: string): Promise<ResultadoRevision> {
  ...
  */
  ...
  const cerrar = async (r: ResultadoRevision) => {
    ...
    if (r.resultado === "ok" && r.enlaces > 0) fuente.enlacesUltimaExitosa = r.enlaces;
    await fuente.save();
    return r;
  };
  ...
  try {
    ...
    if (!res.ok) return cerrar({ ...base, resultado: "error_red", error: `La página respondió ${res.status}.` });
    html = await res.text();
  } catch (e: any) {
  ...
  */
  ...
  const conocidas = new Set((await PublicacionParitaria.find({ fuente: fuente._id }).select("url hash").lean()).map((p: any) => `${p.url}|${p.hash}`));
  ...
  */
  ...
  for (const enlace of candidatos) {
    ...
    if (conocidas.has(`${enlace.url}|${hash}`)) continue;
...
export const fuenteConProblema = (f: Pick<IFuenteParitaria, "ultimoResultado">): boolean => !!f.ultimoResultado && f.ultimoResultado !== "ok";
