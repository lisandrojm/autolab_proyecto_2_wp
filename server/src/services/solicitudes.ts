 ...
 */
export async function prepararSolicitudNueva(tenantId: Types.ObjectId, creadorId: string, metadata: any): Promise<string | null> {
  /*
  ...
  */
  ...
  if (!tieneAreaYTurno) return "La solicitud tiene que traer el área y el turno de la persona.";

  // Toda solicitud de alta nace "pendiente" (ciclo de vida tipo Pedido).
  if (!metadata.solicitudStatus) metadata.solicitudStatus = "pendiente";
...
export async function rolesPorDefecto(tenantId: Types.ObjectId, session?: ClientSession): Promise<string[]> {
  ...
  return [(defaultRole._id as any).toString()];
}
 ...
 */
export async function efectosDeSolicitudNueva(tenantId: Types.ObjectId, creadorId: string, user: any): Promise<void> {
  /*
  ...
  */
  const renovacionDe: any = user.metadata?.esRenovacion ? user.metadata?.renovacionDe : null;
  if (renovacionDe?.userProjectId && renovacionDe?.fechaBajaContrato) {
    try {
      ...
      if (up) {
        const quien: any = await User.findById(creadorId).select("firstName lastName").lean();
        await RenovacionContrato.updateOne(
          { tenantId, userProjectId: up._id, fechaBajaContrato: String(renovacionDe.fechaBajaContrato) },
          {
            $set: {
              userId: up.userId,
  ...
  */
  ...
  const proyectos = [...((user.metadata?.projectIds || []) as any[]), ...((user.projectIds || []) as any[])];
  await notificar({
    tenantId,
