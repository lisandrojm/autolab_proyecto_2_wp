...
function extraerFault(fault: any): { faultCode?: string; faultString?: string } {
...
export async function consultarPadron(tenantId: string, cfg: TenantAfipConfig, cuitConsultado: string, opts?: { tipo?: "padron" | "servicio_test" }): Promise<ResultadoPadron> {
 ...
 */
export async function consultarPadronFiscal(tenantId: string, cfg: TenantAfipConfig, cuitConsultado: string): Promise<{ personaReturn?: any; motivo?: string }> {
  const cuit = normalizarCuit(cuitConsultado);
  if (!cuit) return { motivo: `CUIT inválido: "${cuitConsultado}"` };
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:a5="http://a5.soap.ws.server.puc.sr/">
  ...
  <soapenv:Body>
    ...
    if (!personaReturn) return { motivo: "La respuesta no trae personaReturn" };
 ...
 */
export async function verificarServicioPadron(tenantId: string, cfg: TenantAfipConfig): Promise<ResultadoVerificacionPadron> {
