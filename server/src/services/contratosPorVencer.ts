 ...
 */
export const variantesDMY = (desde: string, dias: number): string[] => {
  const formas = new Set<string>();
  for (let i = 0; i <= dias; i++) {
    const [y, m, d] = sumarDias(desde, i).split("-");
    for (const sep of ["/", "-"]) for (const mm of [m, String(Number(m))]) for (const dd of [d, String(Number(d))]) formas.add(`${dd}${sep}${mm}${sep}${y}`);
  }
  return [...formas];
};
...
const cache = new Map<string, { hasta: number; promesa: Promise<ContratoPorVencer[]> }>();

export function olvidarContratosPorVencer(): void {
  cache.clear();
}
...
async function calcular(tenantId: Types.ObjectId | string, userId: string, hoy: string): Promise<ContratoPorVencer[]> {
  ...
  const [{ proyectos: supervisados }, conCoordinacion] = await Promise.all([
    ...
    Project.find({ tenantId: tenant, "coordinatorAssignments.userId": yo }).select("_id coordinatorAssignments").lean() as Promise<any[]>,
  ]);
  ...
  */
  ...
  const [decisiones, personas, proyectos, areas, turnos, roles] = await Promise.all([
    RenovacionContrato.find({ tenantId: tenant, userProjectId: { $in: candidatos.map((x) => x.up._id) } }).lean() as Promise<any[]>,
...
function avisarSiTardo(t0: number, t1: number, t2: number, t3: number, asignaciones: number, porVencer: number): void {
  if (t3 - t0 < 1000) return;
