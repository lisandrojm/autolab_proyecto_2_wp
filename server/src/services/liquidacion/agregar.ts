 ...
 */
const claveDe = (l: Linea) => [l.empresaId || "", l.ccCodigo || "", l.regimen || "", l.legajo || "", l.conceptoCodigo].join("|");

export function agregarLineas(lineas: Linea[]): LineaAgregada[] {
  const porClave = new Map<string, LineaAgregada & { fechas: Set<string> }>();
  ...
  for (const l of lineas) {
    ...
    if (!actual.origenes.includes(l.origen)) actual.origenes.push(l.origen);
  }
  ...
  return [...porClave.values()]
    ...
    .sort(
      (a, b) =>
        String(a.empresaId).localeCompare(String(b.empresaId)) ||
 ...
 */
export function hojaDe(linea: LineaAgregada, nombreEmpresa: string, nombreCC?: string | null): string {
  ...
  */
  ...
  if (codigo && descripcion.startsWith(codigo)) {
    ...
    while (descripcion.length > 0 && SEPARADORES.includes(descripcion[0])) descripcion = descripcion.slice(1);
  }
