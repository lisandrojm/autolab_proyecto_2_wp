 ...
 */
function nombreDeHojaValido(nombre: string, usados: Set<string>): string {
 ...
 */
export async function generarImportMemosoft(corrida: ILiquidacionCorrida): Promise<Buffer> {
  ...
  if (porHoja.size === 0) libro.addWorksheet("Sin datos").addRow(COLUMNAS_IMPORT);
...
const COLUMNAS_PLANILLA: { encabezado: string; ancho: number; de: (f: FilaDePlanilla) => any }[] = [
  { encabezado: "Legajo", ancho: 10, de: (f) => f.legajo || "" },
  { encabezado: "Apellido y nombre", ancho: 32, de: (f) => f.apellidoYNombre },
  { encabezado: "Empresa", ancho: 16, de: (f) => (f as any).empresaNombre || "" },
 ...
 */
export async function generarPlanillaDeControl(filas: FilaDePlanilla[], periodo: string): Promise<Buffer> {
  const libro = new ExcelJS.Workbook();
  libro.creator = "WeProdu";
  libro.created = new Date();
  ...
  const ordenadas = [...filas].sort(
    (a, b) => a.fecha.localeCompare(b.fecha) || String(a.legajo).localeCompare(String(b.legajo)) || a.aplicaA.localeCompare(b.aplicaA),
  );
  ordenadas.forEach((f) => hoja.addRow(COLUMNAS_PLANILLA.map((c) => c.de(f))));

  if (ordenadas.length > 0) hoja.autoFilter = { from: { row: 1, column: 1 }, to: { row: 1, column: COLUMNAS_PLANILLA.length } };
 ...
 */
export async function generarAnexoDeExcepciones(corrida: ILiquidacionCorrida): Promise<Buffer> {
  const libro = new ExcelJS.Workbook();
  libro.creator = "WeProdu";
  libro.created = new Date();
  ...
  const ordenadas = [...(corrida.excepciones || [])].sort(
    (a, b) => Number(b.bloqueante) - Number(a.bloqueante) || a.motivo.localeCompare(b.motivo) || String(a.apellidoYNombre).localeCompare(String(b.apellidoYNombre)),
  );
  ...
  for (const e of ordenadas) {
    ...
    if (e.bloqueante) fila.getCell(1).font = { bold: true, color: { argb: "FFB00020" } };
  }

  if (ordenadas.length > 0) hoja.autoFilter = { from: { row: 1, column: 1 }, to: { row: 1, column: 6 } };
  return Buffer.from(await libro.xlsx.writeBuffer());
}
