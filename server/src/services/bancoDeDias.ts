...
import { LeaveLedger, ILeaveLedger, LedgerKind } from "../models/LeaveLedger.js";
import { LeaveAccount } from "../models/LeaveAccount.js";
import { RequestConfig, ILeaveEffect } from "../models/RequestConfig.js";
...
export const periodoDe = (fecha: Date, modo: "calendario" | "aniversario_ingreso" = "calendario"): string => {
  // `aniversario_ingreso` necesita la fecha de ingreso de la persona: lo resuelve quien devenga.
  if (modo === "aniversario_ingreso") return `${fecha.getUTCFullYear()}-${fecha.getUTCFullYear() + 1}`;
  return String(fecha.getUTCFullYear());
};

const redondear = (valor: number, modo: ILeaveEffect["rounding"]): number => {
  if (modo === "up") return Math.ceil(valor);
  if (modo === "down") return Math.floor(valor);
  if (modo === "half") return Math.round(valor);
  return valor;
};

/** El día de un parte, como fecha UTC a medianoche: `Request.date` es `"YYYY-MM-DD"`. */
const fechaDelParte = (parte: any): Date => {
 ...
 */
async function contextoDelDia(tenantId: Types.ObjectId, parte: any): Promise<{ esFeriado: boolean; esDiaNoLaborable: boolean }> {
  ...
  */
  ...
  if (parte?.projectId) {
    const proyecto: any = await Project.findById(parte.projectId).select("workSchedule").lean();
    const dias: number[] | undefined = proyecto?.workSchedule?.days;
    if (Array.isArray(dias) && dias.length > 0) esDiaNoLaborable = !dias.includes(fecha.getUTCDay());
  }
...
const cumpleCondicion = (efecto: ILeaveEffect, ctx: { esFeriado: boolean; esDiaNoLaborable: boolean }): boolean => {
  const c = efecto.condition;
  if (!c) return true;
  if (c.esFeriado !== undefined && c.esFeriado !== ctx.esFeriado) return false;
  if (c.esDiaNoLaborable !== undefined && c.esDiaNoLaborable !== ctx.esDiaNoLaborable) return false;
  return true;
};

/** Cuánto mueve un efecto para un renglón de asistencia. `0` = no se emite movimiento. */
const cantidadDe = (efecto: ILeaveEffect, renglon: any): number => {
  let base = 0;
  if (efecto.basis === "fijo") base = Number(efecto.cantidadFija) || 0;
  else if (efecto.basis === "hora") base = (Number(renglon?.overtimeHours) || 0) + (Number(renglon?.replacementOvertimeHours) || 0);
  else base = 1; // un día
...
export async function applyEffects(
  parte: any,
  accion: AccionSobreElParte,
  opciones: { tenantId: Types.ObjectId; createdBy?: Types.ObjectId | string } = {} as any,
): Promise<ResultadoDeEfectos> {
  const tenantId = opciones.tenantId || parte?.tenantId;
  const resultado: ResultadoDeEfectos = { movimientosCreados: 0, reversasCreadas: 0, yaEstaba: false };
  if (!tenantId || !parte?._id) return resultado;
  ...
  */
  ...
  const revertidos = new Set(existentes.filter((m: any) => m.reversalOf).map((m: any) => String(m.reversalOf)));
  const vivos = existentes.filter((m: any) => !m.reversalOf && !revertidos.has(String(m._id)));

  if (accion === "create" && vivos.length > 0 && vivos.every((m: any) => Number(m.source?.refVersion) === versionActual)) {
  ...
  if (accion !== "create" && vivos.length > 0) {
    const reversas = vivos.map((m: any) => ({
      tenantId,
      userId: m.userId,
      ...
      amount: m.amount,
      source: { kind: "reversa" as LedgerKind, refId, refVersion: versionActual },
      reversalOf: m._id,
  ...
  if (accion === "delete") return resultado;

  // 2. Generar lo que corresponde a la versión de ahora.
  const renglones: any[] = Array.isArray(parte.attendance) ? parte.attendance : [];
  const idsTipo = [...new Set(renglones.map((r: any) => String(r?.typeId || "")).filter((id) => Types.ObjectId.isValid(id)))];
  if (idsTipo.length === 0) return resultado;

  const tipos: any[] = await RequestConfig.find({ tenantId, _id: { $in: idsTipo } }).lean();
  const tipoPorId = new Map(tipos.map((t: any) => [String(t._id), t]));

  const idsCuenta = [...new Set(tipos.flatMap((t: any) => (t.effects || []).map((e: any) => String(e.accountId))))];
  const cuentas: any[] = idsCuenta.length > 0 ? await LeaveAccount.find({ tenantId, _id: { $in: idsCuenta }, isActive: true }).lean() : [];
  const cuentaPorId = new Map(cuentas.map((c: any) => [String(c._id), c]));

  const ctx = await contextoDelDia(tenantId, parte);
  const fecha = fechaDelParte(parte);

  const nuevos: Partial<ILeaveLedger>[] = [];
  for (const renglon of renglones) {
    const tipo = tipoPorId.get(String(renglon?.typeId || ""));
    if (!tipo || !renglon?.employeeId) continue;

    for (const efecto of (tipo.effects || []) as ILeaveEffect[]) {
      ...
      if (!cuenta) continue;
      if (!cumpleCondicion(efecto, ctx)) continue;
 ...
 */
export async function saldosDe(tenantId: Types.ObjectId, userIds: (string | Types.ObjectId)[], periodo?: string) {
  const ids = userIds.map((id) => new Types.ObjectId(String(id)));
  if (ids.length === 0) return new Map<string, Map<string, { acreditado: number; consumido: number; saldo: number }>>();

  const match: any = { tenantId, userId: { $in: ids } };
  if (periodo) match.periodo = periodo;

  const filas: any[] = await LeaveLedger.aggregate([
    { $match: match },
    {
      $group: {
  ...
  const porUsuario = new Map<string, Map<string, { acreditado: number; consumido: number; saldo: number }>>();
