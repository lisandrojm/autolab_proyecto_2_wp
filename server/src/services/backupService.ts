...
export async function generarArchivos(): Promise<{ archivos: ArchivoBackup[]; documentos: number }> {
  const db = mongoose.connection.db;
  if (!db) throw new Error("No hay conexión a MongoDB: no se puede generar el backup.");
  ...
  const archivos: ArchivoBackup[] = [];
  for (const nombre of nombres) archivos.push(await volcarColeccion(nombre));
...
const nombresDeColecciones = (archivos: ArchivoBackup[]): string[] => archivos.filter((f) => f.nombre !== "_backup.json").map((f) => f.nombre.replace(/\.json$/, ""));

/** Usa la subida simple, o la de sesión si el archivo pasa el tope del endpoint simple. */
async function subir(tenantId: string, cfg: any, ruta: string, contenido: Buffer) {
  if (contenido.length <= TOPE_SUBIDA_SIMPLE) return uploadFile(tenantId, cfg, ruta, contenido);
...
async function asegurarCarpeta(tenantId: string, cfg: any, ruta: string): Promise<void> {
  try {
 ...
 */
export async function correrBackup(disparador: "cron" | "manual" = "cron"): Promise<ResultadoBackup | null> {
  if (corriendo) throw new Error("Ya hay un backup en curso.");
  corriendo = true;
  try {
    const tenants = await Tenant.find({ "integrations.dropbox.refreshTokenEnc": { $exists: true } }).sort({ createdAt: 1 });
    const tenant = tenants.find((t: any) => !!getTenantDropboxConfig(t));
    if (!tenant) {
      console.warn("[BACKUP] Ningún tenant tiene Dropbox conectado: no hay dónde guardar el backup.");
      return null;
    }
    if (tenants.length > 1) console.warn(`[BACKUP] Hay ${tenants.length} tenants con Dropbox; se usa el primero (${tenant._id}).`);
    ...
    */
    ...
    if (!dropbox.ok) throw new Error(`La copia no quedó en Dropbox: ${dropbox.error}`);
    ...
    */
    ...
    const cambios: Record<string, unknown> = { "integrations.backup.ultimoBackupAt": new Date(), "integrations.backup.ultimoError": "" };
