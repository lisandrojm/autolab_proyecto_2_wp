...
export const rutaAbsoluta = (ruta: string): string => path.join(RAIZ, ruta);
 ...
 */
export const sanearNombre = (base: string, espacio: "_" | " " = "_"): string => {
  let limpio = "";
  for (const ch of base) {
    const codigo = ch.codePointAt(0) ?? 0;
    if (codigo < 32) limpio += "_";
    else if (ch === " ") limpio += espacio;
    else limpio += PROHIBIDOS.includes(ch) ? "_" : ch;
  }
  // Sin puntos al principio: ningún nombre sugerido empieza con «..».
  return limpio.replace(/^[.]+/, "");
};

export const nombreDesdeUrl = (url: string): string => {
  ...
  if (sinPuntos.toLowerCase().endsWith(".pdf")) return sinPuntos.slice(0, MAX_NOMBRE) || "documento.pdf";
 ...
 */
export const guardarPdf = async (fuenteId: string, hash: string, bytes: Buffer, url: string, contentType?: string | null): Promise<ArchivoPublicacion> => {
  ...
  if (!/^[0-9a-f]{64}$/.test(hash)) throw new Error(`Hash inválido para el archivo: ${hash}`);
  if (!/^[0-9a-fA-F]{24}$/.test(String(fuenteId))) throw new Error(`Id de fuente inválido: ${fuenteId}`);
...
export const existeArchivo = async (ruta?: string): Promise<boolean> => {
 ...
 */
export const borrarArchivo = async (ruta?: string): Promise<boolean> => {
  if (!ruta) return false;
  try {
