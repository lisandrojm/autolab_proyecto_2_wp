...
export function resolveScheduleDays(type, days) {
...
export function holidayToDateStr(d) {
    if (typeof d === "string")
        return d.slice(0, 10);
 ...
 */
export function expandExpectedDates(opts) {
    ...
    return eachDateStr(from, effectiveTo).filter((dateStr) => {
        if (!allowed.has(weekdayOf(dateStr)))
            return false;
        if (EXCLUDE_HOLIDAYS && holidays && holidays.has(dateStr))
            return false;
        return true;
    });
}
