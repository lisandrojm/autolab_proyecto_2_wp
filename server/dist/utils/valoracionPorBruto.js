 ...
 */
const brutoValido = (b) => typeof b === "number" && Number.isFinite(b) && b > 0;
export function valorarPorBruto(categorias, niveles) {
    ...
    */
    ...
    if (escala.length === 0) {
        for (const c of categorias)
            resultado.set(c.id, { valoracionId: null, motivo: "no hay valoraciones activas" });
        return resultado;
    }
    const porConvenio = new Map();
    for (const c of categorias)
        porConvenio.set(c.convenio || "", [...(porConvenio.get(c.convenio || "") || []), c]);
    for (const grupo of porConvenio.values()) {
        for (const c of grupo)
            if (!brutoValido(c.bruto))
                resultado.set(c.id, { valoracionId: null, motivo: "no tiene bruto en el catálogo" });
        // De la más barata a la más cara; entre dos que cobran igual, por nombre (y por id si tampoco hay).
        const ordenadas = grupo.filter((c) => brutoValido(c.bruto)).sort((a, b) => a.bruto - b.bruto || (a.nombre || "").localeCompare(b.nombre || "") || a.id.localeCompare(b.id));
        if (ordenadas.length < 2) {
            for (const c of ordenadas)
                resultado.set(c.id, { valoracionId: null, motivo: "es la única de su convenio: no hay qué elegir" });
            continue;
        }
