 ...
 */
export const TIPOS_NOMBRE_SE_LEE_DE_VUELTA = [...TIPOS_NOMENCLATURA];
const G = {
    proyecto: "Proyecto",
    persona: "Persona",
    documento: "Documento",
...
export const largoEnBytes = (s) => new TextEncoder().encode(s).length;
...
export function recortarNombre(nombre, reservar = 4) {
    ...
    if (largoEnBytes(recortado) <= tope)
        return recortado;
    ...
    while (chars.length > 0 && largoEnBytes(chars.join("")) > tope)
        chars.pop();
 ...
 */
const aplicarTope = (nombre, valor) => {
    const tope = TOPES_CAMPO[nombre];
    if (!tope)
        return valor;
    const chars = [...valor];
    if (chars.length <= tope)
        return valor;
...
export function renderNomenclatura(patron, datos) {
    const reemplazado = String(patron || "").replace(/\{\{\s*([\w]+)\s*\}\}/g, (_m, nombre) => VARIABLES_COMPUESTAS.has(nombre) ? String(datos[nombre] ?? datos[canonica(nombre)] ?? "").trim() : aplicarTope(nombre, campoNomenclatura(datos[nombre] ?? datos[canonica(nombre)])));
    return reemplazado
        .split("_")
        .map((s) => s.trim())
        .filter((s) => s !== "")
