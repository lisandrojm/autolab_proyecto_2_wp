 ...
 */
export const resolvio = (r) => r.valor !== null;
/* ───────────────────────────── Empresa ───────────────────────────── */
/**
 ...
 * de la SRL.
 */
export const normalizarRazonSocial = (texto) => String(texto || "")
    .toUpperCase()
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
    .replace(/[._,-]/g, " ")
 ...
 */
const marcaDe = (razonSocial) => normalizarRazonSocial(razonSocial).split(" ")[0] || "";
/**
 ...
 * por contrato.
 */
export function empresaDelContrato(contrato, empresas) {
    const delCampo = contrato.empresaContratoId ? String(contrato.empresaContratoId) : "";
    if (delCampo)
        return { valor: delCampo, origen: "campo" };
...
const regimenPorJornadas = (jornadas) => {
    if (jornadas === JORNADAS_MENSUAL)
        return "mensual";
    if (jornadas === JORNADAS_JORNALERO)
        return "jornalero";
    return null;
};
/**
 ...
 */
export function regimenDelContrato(contrato) {
    ...
    if (porTipo)
        return { valor: porTipo, origen: "tipo_contrato" };
    if (porJornadas)
        return { valor: porJornadas, origen: "jornadas" };
    return {
        valor: null,
 ...
 */
export function contratoVigenteEn(contrato, desde, hasta) {
...
export function limitesDelPeriodo(periodo) {
    const [anio, mes] = String(periodo).split("-").map(Number);
    if (!anio || !mes || mes < 1 || mes > 12)
        throw new Error(`Período inválido: "${periodo}". Va como AAAA-MM.`);
