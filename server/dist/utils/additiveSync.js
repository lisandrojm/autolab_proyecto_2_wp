 ...
 */
export function buildAdditiveSet(frameData, existingDoc, whitelist) {
    const set = {};
 ...
 */
export function findNewContracts(frameContracts, existingContracts) {
    ...
    for (const [key, count] of existingKeyCount.entries()) {
        if (count > 1) {
            collisionWarnings.push(`[ADDITIVE SYNC] Contract key collision in existing contracts (count=${count}): ${key}`);
        }
    }
 ...
 *              isSystem, __v.
 */
export const USER_FRAME_WHITELIST = [
    "email",
    "firstName",
    "lastName",
    "name",
    ...
    "metadata.sinBancoMotivo",
    "metadata.sinBancoDetalle",
 ...
 */
export const PROJECT_FRAME_WHITELIST = [
    "name",
    "description",
    "status",
    "startDate",
 ...
 */
export const USERPROJECT_FRAME_WHITELIST = [
    "nombre_proyecto",
    "nombre_rol_frame",
];
