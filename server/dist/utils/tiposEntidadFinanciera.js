...
export const ROTULOS_CBU = ["CBU", "CVU", "CBU/CVU"];
...
export const aTipo = (d) => ({
    ...
    rotuloCbu: ROTULOS_CBU.includes(d.data?.rotuloCbu) ? d.data.rotuloCbu : "CBU/CVU",
    orden: Number(d.data?.orden) || 0,
});
/** «Compañía Financiera» → «compania_financiera»: sin acentos, minúsculas y guiones bajos. */
export const claveDesdeNombre = (nombre) => nombre
    .normalize("NFD")
 ...
 */
async function asegurarTiposIniciales() {
    if (sembrado)
        return;
    const hay = await Info.countDocuments({ type: TIPO_INFO });
    if (hay === 0) {
        await Info.bulkWrite(INICIALES.map((t, i) => ({
            updateOne: {
