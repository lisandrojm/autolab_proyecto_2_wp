...
const ES_EMPRESA = /(\bS\.?\s?A\.?(\s?I\.?\s?C\.?\s?F?\.?)?$|\bS\.?R\.?L\.?$|\bSRL$|\bSA$|\bLTDA?\.?$|\bCOOPERATIVA\b|\bSOCIEDAD DEL ESTADO\b|\bENTE AUTARQUICO\b|\bADMINISTRACION PORTUARIA\b|\bEMPRESA (DISTRIBUIDORA|DE TRANSPORTE|OPERADORA)\b|\bCENTRAL PUERTO\b|\bCAJA DE PREVISION\b)/;
...
const sinParentesis = (s) => s.replace(/\([^)]*\)/g, " ").replace(/\s+/g, " ").trim();
/** Quita los prefijos de sigla/número, que pueden venir encadenados. */
const sinPrefijo = (s) => {
    let t = limpiar(s);
    for (let i = 0; i < 3 && PREFIJO.test(t); i++)
        t = limpiar(t.replace(PREFIJO, ""));
    return t;
};
const pareceEmpresa = (s) => ES_EMPRESA.test(sinPrefijo(s).toUpperCase());
const pareceEntidad = (s) => ARRANQUE_ENTIDAD.test(sinPrefijo(s).toUpperCase());
/**
 ...
 */
export const ladoDelGremio = (signatario) => {
    const s = sinParentesis(String(signatario || ""));
    if (!s)
        return { lado: "", regla: "vacío" };
    /*
    ...
    */
    ...
    if (barra > 0) {
        ...
        if (pareceEmpresa(izq) && !pareceEmpresa(der))
            return { lado: der, regla: "/ → derecha (la izquierda es una empresa)" };
        if (pareceEmpresa(der) && !pareceEmpresa(izq))
            return { lado: izq, regla: "/ → izquierda (la derecha es una empresa)" };
        if (pareceEntidad(izq) && !pareceEntidad(der))
            return { lado: izq, regla: "/ → izquierda (solo ella arranca como entidad)" };
        if (pareceEntidad(der) && !pareceEntidad(izq))
            return { lado: der, regla: "/ → derecha (solo ella arranca como entidad)" };
    ...
    */
    ...
    if (conY > 0) {
        ...
        if (pareceEmpresa(derecha) || pareceEntidad(derecha))
            return { lado: s.slice(0, conY), regla: "Y → izquierda (la derecha es la contraparte)" };
    }
 ...
 */
const partirPorComa = (s) => s.split(",").reduce((acc, parte, i) => {
    const t = parte.trim();
    if (!t)
        return acc;
    if (i === 0 || ARRANQUE_ENTIDAD.test(t.toUpperCase()))
        acc.push(t);
    else
        acc[acc.length - 1] += ", " + t;
    return acc;
...
export const gremiosDe = (signatario) => {
 ...
 */
export const claveEntidad = (nombre) => ABREVIATURAS.reduce((s, [de, a]) => s.replace(de, a), ` ${String(nombre || "").toUpperCase()} `)
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
    .replace(/[^A-Z0-9 ]/g, " ")
    .split(/\s+/)
...
export const canonizar = (claves) => {
    ...
    for (const c of unicas) {
        let d = destino.get(c);
        for (let i = 0; i < 5 && destino.get(d) && destino.get(d) !== d; i++)
            d = destino.get(d);
        destino.set(c, d);
    }
    return destino;
