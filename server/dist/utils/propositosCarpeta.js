...
export const VALORES_PROPOSITO = PROPOSITOS.map((p) => p.valor);
export const esProposito = (v) => VALORES_PROPOSITO.includes(String(v));
export const definicionDe = (v) => PROPOSITOS.find((p) => p.valor === v);
/** Cómo se llama un propósito en pantalla. Cae al valor crudo antes que mostrar vacío. */
export const etiquetaProposito = (v) => definicionDe(v)?.etiqueta || v;
/** El texto contra el que corren los patrones: la nota libre más el nombre visible de la carpeta. */
export const textoDeCarpeta = (c) => `${c.detalle || ""} ${(c.dropboxCarpeta || "").split("/").filter(Boolean).pop() || ""}`;
/**
 ...
 */
export const inferirPropositos = (c) => {
