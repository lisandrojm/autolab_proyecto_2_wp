...
import { elegirCarpeta, elegirSinCuit } from "./estadoCarpetas.js";
...
const SIN_MIGRAR = CONFIGURADO.map((e) => ({ name: e.name, carpetas: e.carpetas.map(({ dropboxCarpeta, detalle }) => ({ dropboxCarpeta, detalle })) }));
describe("resolución por propósito", () => {
    ...
    it("un propósito sin carpeta no resuelve, y no inventa una parecida", () => {
        const soloOutbox = [{ name: "X", carpetas: [{ dropboxCarpeta: "/HelloSign/Outbox", proposito: "outbox" }] }];
...
describe("fallback por patrón (carpetas sin migrar)", () => {
    ...
    it("los seis siguen resolviendo sin migrar: el backfill no es un requisito para operar", () => {
        for (const p of ["alta_temprana", "constancia_cuit", "sin_cuit", "outbox", "pendbox", "firmados"]) {
    ...
    it("una carpeta con OTRO propósito NO se presta por parecido de nombre", () => {
        ...
        */
        const confusa = [{ name: "X", carpetas: [{ dropboxCarpeta: "/HelloSign/Outbox", proposito: "firmados" }] }];
...
describe("carpeta renombrada — el fallo silencioso que motivó el cambio", () => {
    const renombrada = (proposito) => [{ name: "Envio de documentacion", carpetas: [{ dropboxCarpeta: "/WEPRODU/ARCA/Acuses", ...(proposito ? { proposito } : {}) }] }];
    ...
    it("«Requested signatures» traducida a «Firmados» rompe el patrón y no el propósito", () => {
        const traducida = [{ name: "Disponible", carpetas: [{ dropboxCarpeta: "/HelloSign/Firmados" }] }];
        assert.equal(elegirCarpeta(traducida, "firmados").carpeta, null);
        const conProposito = [{ name: "Disponible", carpetas: [{ dropboxCarpeta: "/HelloSign/Firmados", proposito: "firmados" }] }];
...
describe("«Sin CUIT»: la deducción también pasa por propósito", () => {
    ...
    it("si no está, se deduce como hermana de la de Constancia — resuelta POR PROPÓSITO", () => {
        ...
        const sinSinCuit = [{ name: "E", carpetas: [{ dropboxCarpeta: "/WEPRODU/ARCA/Acuse fiscal", proposito: "constancia_cuit" }] }];
