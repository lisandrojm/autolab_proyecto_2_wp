...
import { armarPayloadDeSolicitud } from "./solicitudDeContratacion.js";
/** COPIA CONGELADA del armado original. No se toca: es la referencia. */
function payloadComoAntes(formData, c, timestamp) {
    ...
    return {
        ...
        metadata: {
            ...
            esRenovacion: renovacion ? true : editingUser?.metadata?.esRenovacion || undefined,
            renovacionDe: renovacion ? { userProjectId: renovacion.userProjectId, fechaBajaContrato: renovacion.fechaBajaContrato } : editingUser?.metadata?.renovacionDe || undefined,
            isSolicitud: true,
        },
    };
}
/** Lo que el formulario le pasa hoy a `armarPayloadDeSolicitud` (mismo mapeo que `UserRegistrationModal`). */
function datosDelFormulario(formData, c) {
    return {
        fullName: formData.fullName,
        projectIds: formData.projectIds,
        ...
        areaShiftAssignments: formData.areaShiftAssignments,
        esRenovacion: c.renovacion ? true : c.editingUser?.metadata?.esRenovacion || undefined,
        renovacionDe: c.renovacion ? { userProjectId: c.renovacion.userProjectId, fechaBajaContrato: c.renovacion.fechaBajaContrato } : c.editingUser?.metadata?.renovacionDe || undefined,
    };
}
...
const ctx = { selectedUser: { _id: "u1" }, esServicios: false, indeterminado: false, porDiasSueltos: false, jornadasCalculadas: 22, ajuste: false, contratoElegido: { name: "Plazo fijo 5x7" }, renovacion: null, editingUser: null };
const casos = [
