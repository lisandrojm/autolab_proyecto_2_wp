...
function leerDomicilios(ruta) {
    ...
    if (iTabla < 0 || iPadded < 0 || iDesc < 0)
        throw new Error("Al CSV le faltan columnas (tabla / codigo_padded / descripcion)");
    const out = new Map();
    for (const f of filas) {
        if (f[iTabla] !== "SUCURSAL_DOMICILIO")
            continue;
...
async function run() {
    ...
    const domicilios = CSV_PATH ? leerDomicilios(CSV_PATH) : new Map();
    ...
    const nombreSede = (id) => sedes.find((s) => Number(s.data?.id) === Number(id))?.name || `Sede ${id}`;
    ...
    const porCodigo = new Map(existentes.map((s) => [String(s.codigo), s]));
    for (const empresa of empresas) {
        const registros = Array.isArray(empresa.sedes) ? empresa.sedes : [];
        const sucursalIds = (empresa.sucursalIds || []).map((x) => String(x));
        ...
        for (const reg of registros) {
            ...
            if (!sucursal) {
                ...
                if (!DRY_RUN)
                    await db.collection("arca-sucursales").insertOne(sucursal);
                porCodigo.set(codigo, sucursal);
            }
            else {
                console.log(`  - ${codigo}: ya existe la sucursal "${sucursal.domicilio}", se reusa.`);
            }
            if (!sucursalIds.includes(String(sucursal._id)))
                sucursalIds.push(String(sucursal._id));
        }
        console.log(`  → queda con ${sucursalIds.length} sucursal(es) asignada(s).`);
        if (!DRY_RUN) {
            const update = { $set: { sucursalIds: sucursalIds.map((id) => new mongoose.Types.ObjectId(id)) } };
            if (LIMPIAR)
                update.$unset = { sedes: "" };
