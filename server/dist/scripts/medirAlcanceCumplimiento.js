...
async function main() {
    ...
    const roles = await Role.find({ tenantId }).select("name permissions").lean();
    const conPermiso = roles.filter((r) => (r.permissions || []).includes(MOBILE_ACTIVITY_COMPLIANCE));
    console.log(`Roles con "${MOBILE_ACTIVITY_COMPLIANCE}": ${conPermiso.map((r) => r.name).join(", ") || "(ninguno)"}`);
    const proyectos = await Project.find({ tenantId }).select("name metadata.responsableId coordinatorAssignments").lean();
    ...
    const asignadosPorProyecto = new Map();
    ...
    const nombres = new Map((await User.find({ _id: { $in: [...todosLosAsignados] } }).select("firstName lastName").lean()).map((u) => [
        String(u._id),
    ...
    */
    ...
    const usuarios = await User.find({ tenantId, isSystem: { $ne: true } })
        ...
        .lean();
    const candidatos = usuarios.filter((u) => (u.roles || []).some((r) => idsConPermiso.has(String(r))));
    console.log(`
PERSONAS QUE PUEDEN ENTRAR A CUMPLIMIENTO: ${candidatos.length}`);
    for (const u of candidatos) {
        const nombresDeSusRoles = (u.roles || []).map((r) => nombrePorRolId.get(String(r)) || String(r));
        const esAdmin = nombresDeSusRoles.some((n) => ["admin", "superadmin"].includes(String(n).toLowerCase()));
        const idFrame = Number(u.metadata?.id);
        const comoResponsable = Number.isFinite(idFrame) ? proyectos.filter((p) => Number(p.metadata?.responsableId) === idFrame) : [];
        const comoCoordinador = proyectos.filter((p) => asignadosPorProyecto.get(String(p._id)).has(String(u._id)));
        const comoMiembro = proyectos.filter((p) => (u.projectIds || []).some((id) => String(id) === String(p._id)));
        ...
        const ahora = [...new Map([...comoResponsable, ...comoCoordinador, ...comoMiembro].map((p) => [String(p._id), p])).values()];
        const gente = (lista) => {
            const s = new Set();
            lista.forEach((p) => asignadosPorProyecto.get(String(p._id)).forEach((x) => s.add(x)));
            return s.size;
        };
        console.log(`   ${((u.lastName || "") + ", " + (u.firstName || "")).padEnd(34)}` +
