...
const CAMPOS_CATEGORIA = ["alcance", "riesgo"];
const db = () => {
    const conn = mongoose.connection.db;
    if (!conn)
        throw new Error("Sin conexión a la base.");
    return conn;
};
/** `valoracionId` pudo guardarse como ObjectId o como string: se busca de las dos formas. */
const bronceEnCualquierFormato = () => {
    const comoTexto = [BRONCE_ID];
    if (mongoose.Types.ObjectId.isValid(BRONCE_ID))
        comoTexto.push(new mongoose.Types.ObjectId(BRONCE_ID));
    return { $in: comoTexto };
};
async function limpiarAlcancePuesto() {
    ...
    try {
        ...
        const enCatalogo = await db().collection("valoraciones").countDocuments({ _id: bronce });
        ...
        if (enCatalogo === 0) {
            ...
            if (nFunciones === 0)
                console.log("  roles_frame            : 0 categorías apuntan a Bronce");
            else if (!aplicar)
                console.log(`  roles_frame            : ${nFunciones} funciones con categorías en Bronce (pasarían a null)`);
            else {
                const r = await db()
                    .collection("roles_frame")
            ...
            if (nProyectos === 0)
                console.log("  projects               : 0 proyectos en Bronce");
            else if (!aplicar)
                console.log(`  projects               : ${nProyectos} proyectos en Bronce (pasarían a null)`);
            else {
            ...
            if (nContratos === 0)
                console.log("  users_&_projects       : 0 contratos en Bronce");
            else if (!aplicar)
                console.log(`  users_&_projects       : ${nContratos} documentos con contratos en Bronce (pasarían a null)`);
            else {
                const r = await db()
        ...
        if (hallazgosBronce > 0)
            console.log(`  ⚠️  Referencias a Bronce encontradas: ${hallazgosBronce}. Ningún documento se borró.`);
    }
    finally {
        await disconnectDB();
    }
