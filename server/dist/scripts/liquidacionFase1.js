...
const HORAS_EXTRA = { codigo50: "0015", codigo100: "0016", param: "par1", unidad: "cantidad" };
async function main() {
    ...
    const conceptos = await MemosoftConcepto.find({ tenantId }).lean();
    ...
    */
    ...
    fuente.forEach((c) => porCodigo.set(c.codigo, c));
    console.log(`Catálogo: ${fuente.length} conceptos${conceptos.length ? "" : " (de la semilla: todavía no se cargó en la base)"}`);
    console.log("");
    const motivos = await RequestConfig.find({ tenantId }).select("name memosoftEffects").sort({ order: 1 }).lean();
    const respaldo = { fecha: new Date().toISOString(), tenantId: String(tenantId), motivos: [], horasExtraAnterior: undefined };
    let problemas = 0;
    const aGuardar = [];
    for (const motivo of motivos) {
        ...
        const yaTiene = (motivo.memosoftEffects || []).filter((e) => !e.vigenteHasta).length;
        if (yaTiene > 0) {
        ...
        const malos = nuevos.map((e) => validarEfecto(e, porCodigo.get(e.conceptoCodigo))).filter(Boolean);
        if (malos.length) {
    ...
    for (const x of aGuardar) {
        ...
        doc.memosoftEffects = reemplazarVigentes((doc.memosoftEffects || []), x.lista, desde);
        await doc.save();
    }
