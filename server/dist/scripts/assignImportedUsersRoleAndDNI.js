...
async function assignImportedUsersRoleAndDNI() {
    ...
    try {
        ...
        if (scope === "all") {
            ...
            console.log(`📅 Última importación: ${new Date(latestRun.createdAt).toLocaleString("es-AR")} ` +
        ...
        const getRolPorDefecto = async (tenantId) => {
        ...
        for (const user of users) {
            let changed = false;
            const changes = [];
            const rolPorDefectoId = await getRolPorDefecto(user.tenantId);
