...
async function run() {
    ...
    const empresas = await Company.find({}).select("razonSocial firmanteNombre firmanteEmail").lean();
    const sinEmail = empresas.filter((e) => !e.firmanteEmail);
    console.log("── Email del firmante ──");
    for (const e of empresas)
        console.log(`   ${e.firmanteEmail ? "✔" : "✖"} ${String(e.razonSocial).padEnd(16)} ${e.firmanteNombre || "(sin firmante)"} → ${e.firmanteEmail || "FALTA"}`);
    ...
    const plantillas = await ContratoFrame.find({}).select("name content").sort({ name: 1 }).lean();
    const log = [];
    let tocadas = 0;
    let rechazadas = 0;
    ...
    for (const p of plantillas) {
        ...
        */
        ...
        if (nuevos > 0) {
            ...
            for (const x of despues.problemas.slice(0, 3))
    ...
    const fin = await ContratoFrame.find({}).select("name content").lean();
