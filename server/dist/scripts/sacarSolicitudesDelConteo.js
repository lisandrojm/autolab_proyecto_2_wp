...
async function run() {
    ...
    const abiertas = await User.find({ "metadata.isSolicitud": true }).select("_id tenantId metadata.activo metadata.solicitudStatus").lean();
    const aprobadasDePaso = [];
    const aprobadas = await User.find({ "metadata.isSolicitud": { $ne: true }, "metadata.solicitudStatus": "aprobada", "metadata.solicitudUserId": { $exists: true, $ne: null } })
        .select("_id tenantId metadata.activo metadata.solicitudUserId")
        .lean();
    for (const a of aprobadas) {
        if (String(a.metadata?.solicitudUserId) === String(a._id))
            continue;
        if (await UserProject.exists({ userId: a._id }))
            continue;
    ...
    const respaldo = { usuarios: [], tenants: [] };
    ...
    const cuenta = (motivo) => objetivo.filter((o) => o.motivo.startsWith(motivo)).length;
    ...
    for (const [tenantId, ids] of porTenant) {
        const t = await Tenant.findById(tenantId).select("name userIds usage").lean();
        if (!t)
            continue;
        const antes = (t.userIds || []).map(String);
        const sacar = new Set(ids.map(String));
        const despues = antes.filter((id) => !sacar.has(id));
        ...
        if (!DRY_RUN)
            await Tenant.updateOne({ _id: tenantId }, { $set: { userIds: despues.map((id) => new Types.ObjectId(id)), "usage.users.current": despues.length } });
    }
