 ...
 */
const partirDescripcion = (descripcion) => {
...
async function run() {
    ...
    if ([iTabla, iPadded, iDesc, iPadre].some((i) => i < 0))
        throw new Error("Al CSV le faltan columnas (tabla / codigo_padded / descripcion / filtro_padre)");
    ...
    for (const f of filas) {
        ...
        if (!cct || !/^\d{6}$/.test(codigo))
            continue;
        if (!porConvenio.has(cct))
            porConvenio.set(cct, []);
    ...
    if (desconocidos.length > 0)
        throw new Error(`El CSV no tiene categorías para: ${desconocidos.join(", ")}. Convenios disponibles: ${[...porConvenio.keys()].join(", ")}`);
    ...
    for (const cct of objetivo) {
        ...
        const sinGrupoPropio = [];
        const asignaciones = [];
        for (const it of items) {
            const { nombre, grupo, nombreGrupo } = partirDescripcion(it.descripcion);
            if (grupo === null)
                sinGrupoPropio.push({ codigo: it.codigo, nombre });
            else
                asignaciones.push({ codigo: it.codigo, nombre, descripcion: it.descripcion, grupo, nombreGrupo });
        }
        ...
        const nombresPorNumero = new Map();
        for (const a of asignaciones.filter((x) => x.nombreGrupo)) {
            if (!nombresPorNumero.has(a.grupo))
                nombresPorNumero.set(a.grupo, new Set());
            nombresPorNumero.get(a.grupo).add(a.nombreGrupo);
        }
        for (const [numero, nombres] of nombresPorNumero) {
            if (nombres.size > 1)
                throw new Error(`${cct}: el grupo ${numero} aparece con dos nombres distintos (${[...nombres].join(" / ")}). Revisá el CSV antes de importar.`);
        }
        ...
        // ── Grupos
        const gruposExistentes = new Map((await db.collection("convenio-grupos").find({ convenio: cct }).toArray()).map((g) => [Number(g.numero), g]));
        const numerosNecesarios = [...new Set(asignaciones.map((a) => a.grupo))].sort((a, b) => a - b);
        for (const numero of numerosNecesarios) {
            if (gruposExistentes.has(numero))
                continue;
            ...
            if (!DRY_RUN)
                await db.collection("convenio-grupos").insertOne(doc);
        ...
        // ── Categorías
        const yaCargadas = new Set((await db.collection("categorias").find({ convenio: cct }).project({ codigoArca: 1 }).toArray()).map((c) => String(c.codigoArca)));
        let creadas = 0;
        for (const a of asignaciones.sort((x, y) => x.grupo - y.grupo || x.codigo.localeCompare(y.codigo))) {
            if (yaCargadas.has(a.codigo))
                continue;
