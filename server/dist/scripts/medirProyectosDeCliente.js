...
const pesos = [];
async function medir(paso, fn, detalle = () => "") {
    const t0 = Date.now();
    const r = await fn();
...
async function main() {
    ...
    const client = await medir("Client.findOne (sin lean ni select)", () => Client.findOne({ _id: clientObjectId }).exec());
    if (!client) {
    ...
    if (userId) {
        const usuario = await medir("alcanceDeResponsable · User.findById", () => User.findById(userId).select("metadata.id").lean().exec());
        const idFrame = Number(usuario?.metadata?.id);
        if (Number.isFinite(idFrame)) {
            await medir("alcanceDeResponsable · Project.find(responsableId)", () => Project.find({ tenantId, "metadata.responsableId": idFrame }).select("_id clientId").lean().exec(), (r) => `${r.length} proyectos`);
        }
    }
    const projects = await medir("Project.find + 4 populate + select(-objectives -workSchedule -teamConfig)", () => Project.find(filter)
        .sort({ createdAt: -1 })
        .skip(0)
        .limit(12)
    ...
    const sedeIds = [...new Set(projects.map((p) => p.metadata?.sedeId).filter(Boolean))].map(Number);
    if (sedeIds.length > 0)
        await medir("Info.find (sedes)", () => Info.find({ type: "sede", "data.id": { $in: sedeIds } }).lean().exec(), (r) => `${r.length} sedes`);
    const ccIds = [...new Set(projects.map((p) => p.metadata?.centroCostoId).filter(Boolean))].map(Number);
    if (ccIds.length > 0)
        await medir("CentroCosto.find (sin select)", () => CentroCosto.find({ $or: [{ idAuxiliar: { $in: ccIds } }, { "data.id": { $in: ccIds } }] }).lean().exec(), (r) => `${r.length} centros`);
    const ids = projects.map((p) => p._id);
    if (ids.length > 0)
        await medir("UserProject.aggregate (contador de personas)", () => UserProject.aggregate([{ $match: { projectId: { $in: ids } } }, { $group: { _id: "$projectId", count: { $sum: 1 } } }]).exec());
    ...
    projects.forEach((p) => {
        ...
        const campos = Object.entries(p)
            .map(([k, v]) => [k, Buffer.byteLength(JSON.stringify(v ?? null)) / 1024])
            .filter(([, k]) => k > 1)
            .sort((a, b) => b[1] - a[1])
