...
async function run() {
    ...
    for (const r of REMAPEOS) {
        ...
        */
        ...
        const antes = rol.data?.categoriasSat || [];
        console.log(`   ${r.funcion}  (rol ${r.rolId})`);
        console.log(`      motivo: ${r.porque}`);
        console.log(`      antes:  ${antes.length ? antes.map((c) => `${c.id} «${c.nombre}»`).join(", ") : "(ninguna)"}`);
        console.log(`      queda:  ${nuevas.map((c) => `${c.id} «${c.nombre}»`).join(", ")}`);
        respaldo.push({ _id: String(rol._id), name: rol.name, categoriasSat: antes });
        if (!DRY_RUN) {
    ...
    for (const nombre of PARA_DECIDIR) {
        ...
        const rolId = rol.data?.rol?.id;
        const ups = await db.collection(COLECCION_CONTRATOS).find({ "contracts.rol_frame_id": rolId }).project({ contracts: 1 }).toArray();
        const n = ups.reduce((a, up) => a + (up.contracts || []).filter((c) => Number(c.rol_frame_id) === Number(rolId)).length, 0);
        const cats = (rol.data?.categoriasSat || []).map((c) => `${c.id} «${c.nombre}»`).join(", ") || "(ninguna)";
