...
async function run() {
    ...
    const cats = await Categoria.find({}).lean();
    const porLegacy = new Map();
    for (const c of cats)
        if (c.legacyId != null)
            porLegacy.set(Number(c.legacyId), c);
    // ── Funciones FRAME: nombre → convenios que sus categorías contemplan.
    const roles = await RoleFrame.find({}).lean();
    const conveniosDeFuncion = new Map();
    ...
    const ups = await UserProject.find({ "contracts.0": { $exists: true } })
        .select("nombre_proyecto userId projectId contracts")
        .lean();
    const sospechosos = [];
    const sinFuncionMapeada = new Map();
    const usoPorLegacy = new Map();
    let conCategoria = 0;
    for (const up of ups) {
        for (let i = 0; i < (up.contracts || []).length; i++) {
            ...
            if (fn.convenios.has(String(cat.convenio || "")))
                continue;
            sospechosos.push({
    ...
    for (const [k, n] of sinMapeo.slice(0, 15))
        console.log(`   ${String(n).padStart(4)}  ${k}`);
    if (sinMapeo.length > 15)
        console.log(`   … y ${sinMapeo.length - 15} más`);
    ...
    const grupos = await ConvenioGrupo.find({ convenio: CONVENIO_GRUPOS }).sort({ numero: 1 }).lean();
