...
async function run() {
    ...
    let proximo = Math.max(Number(maxNuevo[0]?.legacyId || 0), Number(maxViejo[0]?.data?.id || 0)) + 1;
    console.log(`Sin legacyId: ${sinId.length}`);
    console.log(`Se numeran desde ${proximo} (max en categorias: ${maxNuevo[0]?.legacyId ?? 0} · max en categorias-sat: ${maxViejo[0]?.data?.id ?? 0})\n`);
    const porConvenio = new Map();
    const ops = sinId.map((c) => {
        const id = proximo++;
    ...
    for (const [convenio, cuantas] of [...porConvenio.entries()].sort())
        console.log(`  ${convenio}  →  ${cuantas}`);
    console.log("\nPrimeras 5:");
    sinId.slice(0, 5).forEach((c, i) => console.log(`  legacyId ${Number(maxNuevo[0]?.legacyId || 0) + 1 + i}  ${c.convenio}  ${c.codigoArca}  ${c.nombre}`));
    if (DRY_RUN) {
