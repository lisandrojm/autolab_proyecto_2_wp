...
async function run() {
    ...
    for (const cat of cats) {
        ...
        if (norm(oficial) === buscado)
            iguales++;
        else
            distintas.push({ convenio: cat.convenio, codigo: cat.codigoArca, local: cat.nombre, oficial, contratos: usos.get(cat.legacyId) || 0 });
    }
...
`);
    ...
    for (const codigo of ["035358", "035340", "035346"]) {
        const local = cats.find((c) => c.codigoArca === codigo && c.convenio === "0634/11");
