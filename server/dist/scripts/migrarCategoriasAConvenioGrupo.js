...
const ESCALA = ["sueldoBasico", "sueldoAdicional", "presentismo", "sueldoBruto", "neto"];
async function run() {
    ...
    for (const numero of [...porGrupo.keys()].sort((a, b) => a - b)) {
        ...
        for (const campo of ESCALA) {
            ...
            if (valores.length > 1)
                conflictos.push(`Grupo ${numero} · ${campo}: ${valores.join(" / ")}`);
        ...
        if (!DRY_RUN)
            await db.collection("convenio-grupos").insertOne(grupo);
        gruposCreados++;
        for (const c of items) {
            ...
            if (esExcluido) {
                ...
                if (n < RANGO_SAT.min || n > RANGO_SAT.max)
                    fueraDeRango.push(`Nº${numero} "${nombre}" → ${codigoArca}`);
            }
            if (!DRY_RUN) {
    ...
    for (const [k, v] of porConvenio)
        console.log(`  ${k.padEnd(14)} ${v} categoría(s)${k === CONVENIO_SAT ? ` ${v === 106 ? "✓ coincide con las 106 de ARCA" : `⚠ ARCA tiene 106, acá hay ${v}`}` : ""}`);
