...
const fixEncoding = (s) => String(s ?? "").replace(/Â/g, "Á").replace(/Ê/g, "É").replace(/Î/g, "Í").replace(/Ô/g, "Ó").replace(/Û/g, "Ú");
...
for (const f of finales)
    vistos.set(f.codigo, (vistos.get(f.codigo) || 0) + 1);
...
if (!excluirCentinelas && centinelas.length)
    console.log(`Centinelas incluidos: ${centinelas.map((c) => c.codigo).join(", ")} — usá --sin-centinelas para dejarlos afuera.`);
if (duplicados.length)
    console.warn(`OJO: ${duplicados.length} código/s duplicado/s, p. ej. ${duplicados.slice(0, 5).map(([c, n]) => `${c} (x${n})`).join(", ")}`);
const sinDatos = finales.filter((r) => !r.codigo || !r.actividad).length;
if (sinDatos)
    console.warn(`OJO: ${sinDatos} fila/s sin código o sin actividad.`);
