...
async function run() {
    ...
    for (const up of ups)
        for (const c of (up.contracts || []))
            if (c?.categoria_sat_id != null)
                usos.set(c.categoria_sat_id, (usos.get(c.categoria_sat_id) || 0) + 1);
    const grupoPorId = new Map(grupos.map((g) => [String(g._id), g]));
    const grupoPorNumero = new Map(grupos.map((g) => [`${g.convenio}|${g.numero}`, g]));
    const cambios = [];
    const sinMatch = [];
    let yaCorrectas = 0;
    for (const cat of cats) {
        if (!cat.convenio || !cat.codigoArca)
            continue;
    ...
    const intactas = cats.filter((c) => c.convenio && c.codigoArca && !cambios.some((x) => String(x._id) === String(c._id)));
    ...
    if (sinMatch.length > 0) {
        ...
        for (const s of sinMatch)
            console.log(`   ${s.convenio}  ${s.codigo}  "${s.nombre}"  (${s.contratos} contratos)`);
        console.log("");
    }
    ...
    for (const c of cambios) {
        ...
        if (c.grupoA != null && c.grupoA !== c.grupoDe) {
            ...
            if (!grupo) {
                ...
                const r = await db.collection("convenio-grupos").insertOne(nuevo);
