...
const main = async () => {
    ...
    for (const coleccion of COLECCIONES) {
        ...
        if (APLICAR)
            console.log(`   → insertados ${ok} de ${faltantes.length}`);
        console.log("");
    }
    ...
    for (const e of estados) {
        const muertos = (e.data?.contratoFrameIds || []).map(String).filter((id) => !idsFrame.has(id));
        if (muertos.length > 0) {
            console.log(`Estado «${e.name}»: quedan ${muertos.length} referencias colgadas`);
