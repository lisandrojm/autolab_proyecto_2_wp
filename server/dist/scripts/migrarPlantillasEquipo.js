...
const GRUPOS = [["contratoId", "nombreContrato", "tipoImpositivo"], ["areaId", "shiftId"], ["inTime", "outTime"], ["diasSemana", "diasPorSemana", "diasRotativos"]];
...
const vigente = (p, e) => new Map((p.integrantes || [])
    .filter((i) => !(e.asignaciones || []).some((a) => String(a.puestoId) === String(i._id) && a.excluido))
    .map((i) => {
    const a = (e.asignaciones || []).find((x) => String(x.puestoId) === String(i._id));
...
function migrarEquipo(p, e) {
    const cambios = [];
    const usados = (p.integrantes || []).filter((i) => !(e.asignaciones || []).some((a) => String(a.puestoId) === String(i._id) && a.excluido));
    const asignacion = (i) => (e.asignaciones || []).find((a) => String(a.puestoId) === String(i._id));
    // 1. Las condiciones del equipo (si ya las tiene, está migrado: no se tocan).
    if (!e.condiciones && usados.length) {
        const efectivos = usados.map((i) => puestoEnEquipo(i, asignacion(i), e));
        const condiciones = {};
        for (const grupo of GRUPOS) {
            ...
            if (grupo.some((c) => comparable(ganador.valor[c]) !== ""))
                Object.assign(condiciones, ganador.valor);
        }
        ...
        for (const [n, i] of usados.entries()) {
            ...
            for (const c of CAMPOS_DE_CONDICIONES) {
                const esDeEquipo = CAMPOS_DE_EQUIPO.includes(c);
                if (!esDeEquipo) {
                    if (c in viejas)
                        nuevas[c] = viejas[c];
                    continue;
                }
                if (comparable(antes[c]) !== comparable(base[c]))
                    nuevas[c] = antes[c] ?? null;
            }
            if ("escalaAlFijar" in viejas && ("dailyRateManual" in nuevas || "categoriaSatId" in nuevas))
                nuevas.escalaAlFijar = viejas.escalaAlFijar;
            const hay = Object.keys(nuevas).length > 0;
            if (!a && hay)
                e.asignaciones.push((a = { puestoId: i._id, userId: null, condiciones: null, excluido: false }));
            if (a) {
                const antesTxt = JSON.stringify(a.condiciones || null);
                a.condiciones = hay ? nuevas : null;
                if (JSON.stringify(a.condiciones) !== antesTxt)
                    cambios.push(`puesto ${n + 1}: diferencia ${hay ? Object.keys(nuevas).join(", ") : "ninguna"}`);
            }
        }
    }
...
async function run() {
    ...
    for (const original of plantillas) {
        ...
        const antes = new Map((original.equipos || []).map((e) => [String(e._id), vigente(original, e)]));
        const personasAntes = (original.equipos || []).reduce((s, e) => s + (e.asignaciones || []).filter((a) => a.userId).length, 0);
        const cambiosPorEquipo = (p.equipos || []).map((e) => ({ equipo: e.nombre, cambios: migrarEquipo(p, e) })).filter((x) => x.cambios.length);
        if (!cambiosPorEquipo.length)
            continue;
        ...
        for (const e of p.equipos || []) {
            ...
            for (const [puestoId, campos] of a) {
                ...
                for (const [c, v] of Object.entries(campos))
                    if (despues[c] !== v)
                        throw new Error(`«${p.nombre}» / «${e.nombre}» / puesto ${puestoId}: ${c} era «${v}» y quedaría «${despues[c]}». No se escribió nada.`);
            }
        }
        const personasDespues = (p.equipos || []).reduce((s, e) => s + (e.asignaciones || []).filter((a) => a.userId).length, 0);
        if (personasAntes !== personasDespues)
            throw new Error(`«${p.nombre}»: ${personasAntes} personas asignadas antes y ${personasDespues} después. No se escribió nada.`);
        tocadas++;
    ...
    if (process.env.SALIDA_DRY)
        fs.writeFileSync(process.env.SALIDA_DRY, JSON.stringify(salida.map((x) => ({ _id: String(x.original._id), nombre: x.original.nombre, equipos: x.nuevosEquipos })), null, 2));
    if (!DRY_RUN && salida.length) {
        ...
        for (const x of salida) {
            ...
            const equipos = JSON.parse(JSON.stringify(x.nuevosEquipos), (k, v) => typeof v === "string" && /^[0-9a-f]{24}$/.test(v) && /(^_id$|Id$)/.test(k) ? new mongoose.Types.ObjectId(v) : typeof v === "string" && /El$/.test(k) && /^\d{4}-\d{2}-\d{2}T/.test(v) ? new Date(v) : v);
