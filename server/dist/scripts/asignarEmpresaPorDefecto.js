...
async function main() {
    ...
    const buscada = empresas.find((e) => String(e.razonSocial).toLowerCase().includes(razonPedida.toLowerCase()));
    if (!buscada) {
        console.error(`No hay ninguna empresa que coincida con "${razonPedida}". Las que hay: ${empresas.map((e) => e.razonSocial).join(", ")}`);
        await mongoose.disconnect();
        process.exit(1);
    }
    console.log(`Empresa elegida: ${buscada.razonSocial}   (${buscada._id})\n`);
    const vinculos = await UserProject.find({}).select("contracts.nombre_contrato contracts.empresaContratoId").lean();
    const porNombre = new Map();
    const cambios = [];
    for (const v of vinculos) {
        (v.contracts || []).forEach((c, i) => {
            if (c.empresaContratoId)
                return; // Ya resuelto: no se toca.
    ...
    [...porNombre.entries()]
        ...
        const menciona = empresas
            .filter((e) => nombre.toUpperCase().includes(String(e.razonSocial).split(" ")[0].toUpperCase()) && String(e._id) !== String(buscada._id))
            .map((e) => e.razonSocial);
