...
async function run() {
    ...
    const empresas = await Company.find({}).select("razonSocial firmanteNombre firmanteEmail representanteLegalNombre representanteLegalEmail").lean();
    ...
    // ── 2. Las plantillas.
    const plantillas = await ContratoFrame.find({}).select("name content").lean();
    ...
    for (const p of conLiteral)
        console.log(`      ${String((String(p.content).match(LITERAL) || []).length).padStart(2)} × ${p.name}`);
    if (sinEmail.length > 0 && !FORZAR) {
        ...
        if (!DRY_RUN && aSembrar.length > 0) {
            for (const e of aSembrar)
                await Company.updateOne({ _id: e._id }, { $set: { firmanteEmail: e.email } });
