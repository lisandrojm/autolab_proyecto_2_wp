...
async function run() {
    ...
    if (!type)
        throw new Error("Falta la variable TYPE (ej: TYPE=obra-social)");
    const target = process.env.TARGET || MAPPING[type];
    if (!target)
        throw new Error(`No hay colección destino para TYPE="${type}". Pasá TARGET=...`);
    ...
    if (dryRun) {
        ...
        if (bulkOps.length > 20)
            console.log(`  … y ${bulkOps.length - 20} más`);
