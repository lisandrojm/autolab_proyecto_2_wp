...
async function main() {
    ...
    console.log(`\nTenants por cantidad de usuarios: ${porTenant.map((t) => `${t._id} (${t.n})`).join(", ")}`);
    const tenantId = porTenant[0]._id;
    /* ── Empresas y centros de costo ── */
    linea("EMPRESAS Y CENTROS DE COSTO");
    const empresas = await Company.find({}).select("name cuit").lean();
    console.log(`Empresas (empleadoras): ${empresas.length}`);
    empresas.forEach((e) => console.log(`   · ${e.name}   cuit=${e.cuit || "—"}   ${e._id}`));
    ...
    ccPorEmpresa.forEach((c) => console.log(`   · ${c._id || "(sin empresa)"}: ${c.n}`));
    ...
    const motivos = await RequestConfig.find({ tenantId }).select("name isActive requiresReplacement effects order").sort({ order: 1 }).lean();
    ...
    if (usoMotivos.length === 0)
        console.log("   (no hay partes cargados en ese período)");
    usoMotivos.forEach((u) => console.log(`   · estado=${u._id.estado || "—"}   motivo="${u._id.motivo || "(vacío)"}"   ${u.n} renglones`));
    ...
    const usuarios = await User.find({ tenantId, isSystem: { $ne: true } })
        ...
        .lean();
    const conLegajo = usuarios.filter((u) => String(u.metadata?.numeroLegajoTango || "").trim());
    const activos = usuarios.filter((u) => u.metadata?.activo !== false);
    ...
    repetidos.slice(0, 15).forEach(([v, us]) => console.log(`   · "${v}" → ${us.map((u) => `${u.lastName}, ${u.firstName}`).join(" | ")}`));
    if (repetidos.length > 15)
        console.log(`   … y ${repetidos.length - 15} más`);
    ...
    porEmpresa.forEach((e) => console.log(`   · ${e._id}: ${e.n} contratos, ${e.personas} personas`));
