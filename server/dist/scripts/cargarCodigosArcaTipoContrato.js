...
async function run() {
    ...
    const tipos = await Contrato.find({}).lean();
    ...
    for (const a of ASIGNACIONES) {
        ...
        const set = {};
        if (a.modalidad !== null)
            set["data.afipModalidadContrato"] = a.modalidad;
        if (a.tipoServicio !== null)
            set["data.afipTipoServicio"] = a.tipoServicio;
        if (a.liquidacion !== null)
            set["data.afipModalidadLiquidacion"] = a.liquidacion;
        if (a.generaAlta !== null)
            set["data.generaAlta"] = a.generaAlta;
        console.log(String(a.nombre).slice(0, 52).padEnd(54) +
        ...
        if (!DRY_RUN && Object.keys(set).length > 0)
            await Contrato.updateOne({ _id: t._id }, { $set: set });
    }
