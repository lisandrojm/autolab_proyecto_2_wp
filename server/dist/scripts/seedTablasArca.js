...
async function run() {
    ...
    const col = (nombre) => {
        ...
        if (i < 0)
            throw new Error(`Al CSV le falta la columna "${nombre}". Tiene: ${encabezado.join(", ")}`);
        return i;
    };
    ...
    for (const tabla of TABLAS) {
        ...
        for (const [codigo, desc] of porCodigo) {
            ...
            if (!previo) {
                nuevos++;
                if (!DRY_RUN)
                    await coleccion.insertOne({ ...doc, createdAt: new Date(), updatedAt: new Date() });
            }
            else if (previo.name !== desc) {
                actualizados++;
                if (!DRY_RUN)
                    await coleccion.updateOne({ _id: previo._id }, { $set: { ...doc, updatedAt: new Date() } });
            }
            else {
                iguales++;
            }
