 ...
 */
async function junioDesdeCategoriasSat() {
    ...
    for (const [grupo, lista] of [...porGrupo.entries()].sort((a, b) => a[0] - b[0])) {
        const distintos = (campo) => [...new Set(lista.map((x) => Number(x?.[campo] || 0)))];
...
const abrilDesdeJunio = (junio) => junio.map((f) => ({
    grupo: f.grupo,
...
function leerAnexo(ruta) {
    const absoluta = path.resolve(process.cwd(), ruta);
    if (!fs.existsSync(absoluta))
        throw new Error(`No existe el archivo del Anexo A: ${absoluta}`);
...
async function run() {
    ...
    if (!junio.length)
        throw new Error("No se pudo reconstruir ningún grupo de junio. Pasá ARCHIVO=<anexo.json>.");
    ...
    if (!controlOk)
        console.log("   ⚠ Si no coincide, revisá la fuente antes de escribir: el resto de los grupos sale del mismo camino.\n");
    const acuerdo = await AcuerdoParitario.findOne({ expediente: EXPEDIENTE }).select("_id");
    if (!acuerdo)
        console.log(`   ⚠ No está cargado el acuerdo ${EXPEDIENTE} (corré 634-acuerdo primero): los períodos van a quedar sin vínculo al acta.`);
