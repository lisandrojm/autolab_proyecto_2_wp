...
async function run() {
    ...
    if (yaExiste) {
        console.log(`  El alias legacyId ${LEGACY_HUERFANO} ya existe (${yaExiste.nombre}). Nada que hacer.`);
    }
    else {
        const destino = (await db.collection("categorias").findOne({ legacyId: LEGACY_DESTINO }));
        if (!destino)
            throw new Error(`No existe la categoría legacyId ${LEGACY_DESTINO}: correr primero migrarCategoriasAConvenioGrupo.ts`);
        ...
        if (!DRY_RUN)
            await db.collection("categorias").insertOne(alias);
    }
    // ─────────────────────────────────────────────────────────────────────────────
    ...
    const porLegacyId = new Map(categorias.filter((c) => c.legacyId != null).map((c) => [Number(c.legacyId), c]));
    const ups = (await db.collection("users_&_projects").find({ "contracts.nombre_categoria_sat": "Sin categoria" }).toArray());
    let corregidos = 0;
    const sinResolver = new Map();
    for (const up of ups) {
        let cambio = false;
        for (const c of up.contracts || []) {
            if (c.nombre_categoria_sat !== "Sin categoria")
                continue;
        ...
        if (cambio && !DRY_RUN)
            await db.collection("users_&_projects").updateOne({ _id: up._id }, { $set: { contracts: up.contracts } });
    }
...
async function contarContratosCon(db, legacyId) {
    const ups = (await db.collection("users_&_projects").find({ "contracts.categoria_sat_id": legacyId }).project({ contracts: 1 }).toArray());
    return ups.reduce((acc, up) => acc + (up.contracts || []).filter((c) => Number(c.categoria_sat_id) === legacyId).length, 0);
}
run().catch(async (e) => {
    console.error("\nFALLÓ:", e.message, "\n");
