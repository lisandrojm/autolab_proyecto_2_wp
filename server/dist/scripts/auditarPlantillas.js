...
async function run() {
    ...
    const tipos = await Contrato.find({}).select("name").lean();
    const porTipoId = new Map(tipos.map((t) => [String(t._id), t.name]));
    const plantillas = await ContratoFrame.find({}).select("name content contratoId").sort({ name: 1 }).lean();
    ...
    for (const p of plantillas) {
        ...
        for (const x of r.problemas.slice(0, 12))
            console.log(`      ${x.motivo}\n         …${x.fragmento}…`);
        if (r.problemas.length > 12)
            console.log(`      … y ${r.problemas.length - 12} más`);
    }
    ...
    if (tiposSinPlantilla.length > 0)
        console.log(`tipos sin plantilla: ${tiposSinPlantilla.map((t) => t.name).join(", ")}`);
