...
async function run() {
    ...
    const antes = auditarPunteros(ups, [...categorias, ...sat], nombrePorRolId);
    ...
    for (const g of antes.porCategoria) {
        ...
        for (const r of g.roles)
            console.log(`      ${String(r.contratos).padStart(4)}  ${r.nombre}`);
    }
    console.log("");
    ...
    for (const r of REASIGNACIONES) {
        const destino = categorias.find((c) => Number(c.legacyId) === r.a);
        if (!destino) {
        ...
        const grupo = grupos.find((g) => String(g._id) === String(destino.grupoId));
        /*
    ...
    const despues = DRY_RUN ? antes : auditarPunteros((await db.collection(COLECCION_CONTRATOS).find({}).project({ userId: 1, nombre_proyecto: 1, contracts: 1 }).toArray()), [...categorias, ...sat], nombrePorRolId);
    ...
    if (pendientes.length === 0)
        console.log("   (ninguno)\n");
    const users = new Map((await db.collection("users").find({}).project({ firstName: 1, lastName: 1, metadata: 1 }).toArray()).map((u) => [String(u._id), u]));
