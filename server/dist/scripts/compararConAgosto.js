...
async function main() {
    ...
    const corrida = await correrLiquidacion(tenantId, "2026-08", new Types.ObjectId(), { persistir: false });
    /* ── 1. Qué conceptos emite cada lado ── */
    titulo("QUÉ CONCEPTOS EMITE CADA LADO");
    const mios = new Map();
    corrida.lineas.forEach((l) => {
    ...
    corrida.lineas
        .filter((l) => l.conceptoCodigo === "0000" && l.legajo)
        .forEach((l) => porLegajo.set(String(l.legajo), (porLegajo.get(String(l.legajo)) || 0) + l.par2));
    let iguales = 0;
    let deMenos = 0;
    let faltantes = 0;
    ...
    if (detalle.length > 40)
        console.log(`   … y ${detalle.length - 40} más`);
