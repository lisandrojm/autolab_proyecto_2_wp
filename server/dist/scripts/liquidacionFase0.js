...
import { empresaDelContrato, resolvio } from "../utils/liquidacion/contratos.js";
const CARPETA_RESPALDOS = path.resolve(process.cwd(), "migraciones-respaldo");
const titulo = (t) => console.log(`\n${"─".repeat(78)}\n${t}\n${"─".repeat(78)}`);
async function main() {
    const revertir = process.argv.includes("--revertir");
    if (revertir)
        return deshacer(process.argv[process.argv.indexOf("--revertir") + 1]);
    ...
    const respaldo = { fecha: new Date().toISOString(), tenantId: String(tenantId), conceptosCreados: [], empresasEnContratos: [], legajos: [] };
    ...
    const existe = new Set(yaCargados.map((c) => `${c.empresaId}|${c.codigo}`));
    let aCrear = 0;
    const nuevos = [];
    for (const empresa of conocidas) {
        for (const c of CONCEPTOS_SEMILLA) {
            if (existe.has(`${empresa.id}|${c.codigo}`))
                continue;
            aCrear++;
            nuevos.push({
                tenantId,
    ...
    if (aplicar && nuevos.length) {
        ...
        respaldo.conceptosCreados = creados.map((c) => String(c._id));
        console.log(`Creados: ${creados.length}`);
    }
    /* ─────────────── 2. La empresa dentro del contrato ─────────────── */
    titulo("2. EMPRESA EN LOS CONTRATOS");
    const vinculos = await UserProject.find({}).select("userId contracts.nombre_contrato contracts.empresaContratoId contracts.fecha_alta_contrato contracts.fecha_baja_contrato").lean();
    /* Se agrupa por nombre distinto para poder revisar 16 decisiones en vez de 7.462. */
    const porNombre = new Map();
    const cambios = [];
    for (const v of vinculos) {
        (v.contracts || []).forEach((c, i) => {
            if (c.empresaContratoId)
                return; // Ya lo tiene: no se pisa nunca.
            ...
            if (resolvio(r))
                cambios.push({ userProjectId: String(v._id), indice: i, anterior: null, empresaId: r.valor });
        });
    }
    ...
    if (aplicar && cambios.length) {
        ...
        for (let i = 0; i < cambios.length; i += 200) {
            ...
            await UserProject.bulkWrite(lote.map((c) => ({
                updateOne: {
    ...
    */
    ...
    const usuarios = await User.find({ tenantId, isSystem: { $ne: true }, "metadata.numeroLegajoTango": { $nin: [null, ""] } })
    ...
    for (const u of usuarios) {
        ...
        const empresasDeLaPersona = [...(empresaPorUsuario.get(String(u._id)) || new Set())];
    ...
    if (aplicar && aEscribir.length) {
        ...
        for (let i = 0; i < aEscribir.length; i += 200) {
            ...
            await User.bulkWrite(lote.map((x) => ({ updateOne: { filter: { _id: new Types.ObjectId(x.userId) }, update: { $set: { "metadata.legajosPorEmpresa": x.lista } } } })));
        }
...
async function deshacer(archivo) {
    ...
    for (let i = 0; i < respaldo.legajos.length; i += 200) {
        ...
        await User.bulkWrite(lote.map((x) => ({
            updateOne: {
