...
const valorFlag = (f) => args.find((a) => a.startsWith(`${f}=`))?.split("=").slice(1).join("=");
...
async function main() {
    ...
    */
    ...
    const remapeo = construirRemapeo(anteriores.map((c) => ({ idViejo: Number(c.idAuxiliar ?? c.data?.id ?? Number(c.externalId)), codigo: String(c.codAuxiliar ?? c.name ?? c.data?.nombre ?? "") })), validado.items);
    ...
    const filtroProyectos = { "metadata.centroCostoId": { $gt: 0 } };
    if (TENANT)
        filtroProyectos.tenantId = new mongoose.Types.ObjectId(TENANT);
    const proyectos = await Project.find(filtroProyectos).select("name tenantId metadata.centroCostoId metadata.centroCostoOrigen").lean();
    ...
    if (!SOLO_PROYECTOS) {
        ...
        await CentroCosto.insertMany(validado.items.map((i) => sincronizarCamposDerivados({ ...i })), { ordered: false });
