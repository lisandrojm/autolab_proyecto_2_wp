...
async function contratosPorLegacyId() {
    const ups = await UserProject.find({ "contracts.0": { $exists: true } }).select("contracts.categoria_sat_id").lean();
    const m = new Map();
    for (const up of ups) {
...
async function run() {
    ...
    const problemas = [];
    const grupos = await ConvenioGrupo.find({ convenio: CONVENIO }).sort({ numero: 1 }).lean();
    const cats = await Categoria.find({ convenio: CONVENIO }).lean();
    ...
    const porNumero = new Map(grupos.map((g) => [Number(g.numero), g]));
    ...
    for (const m of MAPEO) {
        ...
        if (!match) {
            problemas.push(`DESFASAJE: el grupo ${m.desde} («${g.nombre || ""}») no tiene la categoría ${m.codigoArca} («${m.nombre}»), que es la que lo identifica. Tiene: ${hijas.map((c) => `${c.codigoArca} ${c.nombre}`).join(", ") || "(ninguna)"}.`);
            continue;
        }
        if (hijas.length !== 1) {
    ...
    */
    ...
    for (const c of conservados)
        console.log(`   #${String(c.grupo.numero).padStart(2)} → #${String(c.nuevo).padStart(2)}   «${c.grupo.nombre || ""}»   (${c.categoria.codigoArca})`);
    ...
    if (conEscalaPropia > 0)
        console.log(`   ${conEscalaPropia} de ellas tienen escala propia, que se conserva.`);
    ...
    const gruposFin = await ConvenioGrupo.find({ convenio: CONVENIO }).sort({ numero: 1 }).lean();
    const catsFin = await Categoria.find({ convenio: CONVENIO }).lean();
