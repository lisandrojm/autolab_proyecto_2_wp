...
async function run() {
    ...
    const respaldo = { categorias: [], grupos: [] };
    for (const convenio of CONVENIOS) {
        ...
        */
        const conEscala = grupos.filter((g) => Number(g.sueldoBruto || 0) > 0);
        if (conEscala.length > 0) {
            console.log(`   ✗ ${conEscala.length} grupo(s) tienen escala cargada (${conEscala.map((g) => g.nombre || g.numero).join(", ")}). NO se toca este convenio.`);
            console.log(`     Aplanarlo perdería esos importes. Pasalos a la categoría primero.\n`);
            continue;
        }
        // 1. Las categorías se sueltan del grupo.
        const conGrupo = cats.filter((c) => c.grupoId);
        for (const c of conGrupo) {
            ...
            if (!DRY_RUN)
                await db.collection("categorias").updateOne({ _id: c._id }, { $set: { grupoId: null } });
        }
        ...
        for (const g of grupos) {
            const colgando = DRY_RUN ? cats.filter((c) => String(c.grupoId) === String(g._id) && !conGrupo.some((x) => String(x._id) === String(c._id))).length : await db.collection("categorias").countDocuments({ grupoId: g._id });
            if (colgando > 0) {
            ...
            if (!DRY_RUN)
                await db.collection("convenio-grupos").deleteOne({ _id: g._id });
        }
        console.log("");
    }
    ...
    for (const convenio of CONVENIOS) {
        ...
        const sinGrupo = cats.filter((c) => !c.grupoId).length;
        console.log(`   ${convenio}  ·  ${grupos} grupo(s)  ·  ${sinGrupo}/${cats.length} categoría(s) en «sin grupo»`);
        for (const c of cats)
            console.log(`      ${c.codigoArca}  ${c.nombre}  ·  escala ${Number(c.sueldoBruto || 0) > 0 ? `$ ${Number(c.sueldoBruto).toLocaleString("es-AR")}` : "SIN CARGAR"}`);
    }
