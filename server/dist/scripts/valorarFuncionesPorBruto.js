...
async function valorarFuncionesPorBruto() {
    ...
    try {
        ...
        const plan = await planValoracionPorBruto(tenant._id, { incluirValoradas: force });
        const nombre = new Map(plan.niveles.map((n) => [n._id, n.name]));
        const etiqueta = (id) => (id ? nombre.get(id) || "?" : "sin valorar");
        ...
        for (const f of plan.funciones) {
            ...
            for (const c of f.cambios)
                console.log(`     ${(c.convenio || "(sin convenio)").padEnd(9)} ${c.nombre.padEnd(46)} ${formatoPlata(c.bruto).padStart(14)}  ${etiqueta(c.antes)} → ${etiqueta(c.despues)}${c.motivo ? ` (${c.motivo})` : ""}`);
        }
        for (const n of plan.salteadas)
            console.log(`⏭️  ${n}: ya tiene categorías valoradas, no se toca (FORCE=true para revalorarla).`);
        console.log("\n── Resumen ──");
