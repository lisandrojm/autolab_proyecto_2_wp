...
const formatoPlata = (n) => (n == null ? "sin bruto" : `$${Math.round(n).toLocaleString("es-AR")}`);
        ...
        */
        ...
        for (const funcion of funciones) {
            ...
            for (const grupo of porConvenio.values()) {
                ...
                for (const a of grupo.filter((x) => x.bruto == null))
                    a.motivo = "sin bruto en el catálogo";
                const minimo = Math.min(...conBruto.map((a) => a.bruto));
                if (!conBruto.some((a) => a.bruto > minimo)) {
                    for (const a of conBruto)
                        a.motivo = conBruto.length === 1 ? "única del convenio: no hay qué elegir" : "todas con el mismo bruto: no hay una más barata";
                    continue;
                }
                for (const a of conBruto)
                    a.valoracion = a.bruto === minimo ? "baja" : "alta";
            }
            ...
            for (const [convenio, grupo] of porConvenio) {
                ...
                for (const a of [...grupo].sort((x, y) => (x.bruto ?? Infinity) - (y.bruto ?? Infinity))) {
                    ...
                    if (a.valoracion === "baja")
                        categoriasBaja++;
                    else if (a.valoracion === "alta")
                        categoriasAlta++;
                    else
                        sinValorar++;
                    if (a.valoracion) {
