...
const main = async () => {
    ...
    for (const c of contratos.sort((a, b) => String(a.name).localeCompare(String(b.name))))
        console.log(`   · ${c.name}`);
    console.log(`\n── PLANTILLAS (contratos-frame): ${frames.length}`);
    for (const f of frames.sort((a, b) => String(a.name).localeCompare(String(b.name)))) {
    ...
    */
    ...
    const contratoIdsUsados = new Map();
    ...
    for (const i of infosContrato.sort((a, b) => String(a.name).localeCompare(String(b.name)))) {
        console.log(`   · [${i.data?.id}] ${i.name}`);
    }
    ...
    for (const e of estados) {
        const ids = (e.data?.contratoFrameIds || []).map(String);
        const muertos = ids.filter((id) => !idsFrame.has(id));
        if (muertos.length > 0) {
            console.log(`   · ${e.name}: ${muertos.length} de ${ids.length} apuntan a la nada`);
            estadosConHuerfanos++;
        }
    }
    if (estadosConHuerfanos === 0)
        console.log("   (ninguno)");
