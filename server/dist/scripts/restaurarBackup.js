...
async function main() {
    ...
    const colecciones = manifiesto.colecciones || [];
    ...
    for (const c of colecciones) {
        ...
        if (dif !== 0)
            console.log(`   ${c.nombre.padEnd(40)} ${String(hoy).padStart(6)}   ${String(c.documentos).padStart(11)}   ${dif > 0 ? "+" : ""}${dif}`);
    }
    ...
    if (sobran.length) {
        ...
        for (const n of sobran)
            console.log(`   ${n.padEnd(40)} ${await base.collection(n).countDocuments()} documentos`);
    }
    if (!aplicar) {
        ...
        */
        ...
        for (const c of actuales) {
            ...
            for (let i = 0; i < docs.length; i += LOTE)
                await respaldo.collection(c.name).insertMany(docs.slice(i, i + LOTE));
        }
    ...
    for (const c of colecciones) {
        ...
        const docs = lineas.map((l) => EJSON.parse(l, { relaxed: false }));
