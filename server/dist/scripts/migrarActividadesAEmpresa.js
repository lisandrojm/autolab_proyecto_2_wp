...
async function run() {
    ...
    const sucursales = await ArcaSucursal.find({}).lean();
    const porId = new Map(sucursales.map((s) => [String(s._id), s]));
    const empresas = await Company.find({}).select("razonSocial sucursalIds sucursalActividades").sort({ razonSocial: 1 }).lean();
    const log = [];
    let tocadas = 0;
    for (const e of empresas) {
        ...
        const filas = asignados
            ...
            if (!suc)
                return null;
            const actividades = (suc.actividades || []).filter((a) => a?.codigo).map((a) => ({ codigo: String(a.codigo), descripcion: String(a.descripcion || "") }));
            return { sucursalId: id, actividades, _nombre: `${suc.codigo} ${suc.domicilio}` };
        })
            .filter(Boolean);
        for (const f of filas) {
            const marca = f.actividades.length === 0 ? "✖" : "→";
            console.log(`   ${marca} ${f._nombre}: ${f.actividades.length} actividad(es)${f.actividades.length ? ` (${f.actividades.map((a) => a.codigo).join(", ")})` : " — el domicilio no tiene ninguna cargada"}`);
        }
