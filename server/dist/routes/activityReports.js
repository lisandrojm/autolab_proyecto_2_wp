 ...
 */
async function proyectosDondeParticipa(tenantId, userId) {
    ...
    const todos = new Set();
    comoResponsable.forEach((id) => todos.add(String(id)));
    asignado.forEach((p) => todos.add(String(p._id)));
    (usuario?.projectIds || []).forEach((id) => todos.add(String(id)));
    return [...todos];
}
/**
...
router.put("/:id", async (req, res) => {
    try {
        ...
        */
        const report = await Request.findOneAndUpdate(filter, { ...data, $inc: { version: 1 } }, { new: true });
        if (!report) {
