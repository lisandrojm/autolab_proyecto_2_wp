...
*/
router.post("/superposiciones", requireTenant, authenticateToken, requireAnyPermission("admin_users:view", MOBILE_USERS), async (req, res) => {
    try {
        ...
        const p = pedido;
        const lista = await superposicionesDeAlta(req.tenantObjectId, String(userId), {
        ...
        */
        if (data.metadata && currentUser.metadata) {
            ...
            if (data.metadata.isSolicitud === true) {
                ...
                data.metadata.avisosSuperposicion = await superposicionesDeAlta(req.tenantObjectId, data.metadata.solicitudUserId, pedidoDesdeSolicitud(data.metadata), String(req.params.id));
            }
        }
        /*
