 ...
 */
const funcionesQueUsan = async (legacyId) => {
 ...
 */
router.get("/escalas-vencidas", authenticateToken, async (_req, res) => {
    try {
        ...
        const vencidas = escalasVencidas([...grupos.map((g) => ({ ...g, donde: "grupo" })), ...cats.map((c) => ({ ...c, donde: "categoria" }))], hoy);
 ...
 */
router.get("/contratos-huerfanos", authenticateToken, async (_req, res) => {
    try {
        ...
        const nombrePorRolId = new Map(roles.filter((r) => r.data?.rol?.id != null).map((r) => [Number(r.data.rol.id), String(r.name)]));
        // Solo el resumen: la lista completa son cientos de filas y el panel muestra el agrupado.
        const { total, porCategoria } = auditarPunteros(ups, [...cats, ...viejas], nombrePorRolId);
    ...
    try {
        ...
        */
        if (req.body.isActive !== undefined) {
            ...
            if (daDeBaja && req.body.forzar)
                console.warn(`[CATEGORIAS] Baja FORZADA de «${item.nombre}» (legacyId ${item.legacyId}) con funciones FRAME apuntándole.`);
 ...
 */
const COLUMNAS_PLANTILLA = ["convenio", "grupo", "nombreGrupo", "sueldoBasico", "sueldoAdicional", "presentismo", "sueldoBruto", "sueldoBrutoLetras", "neto", "sueldoNetoLetras", "vigenciaDesde", "vigenciaHasta"];
/** La misma plantilla para los convenios SIN grupos: en vez del número de grupo, el código de ARCA. */
const COLUMNAS_PLANTILLA_SIN_GRUPO = ["convenio", "codigoArca", "nombreCategoria", "sueldoBasico", "sueldoAdicional", "presentismo", "sueldoBruto", "sueldoBrutoLetras", "neto", "sueldoNetoLetras", "vigenciaDesde", "vigenciaHasta"];
/**
...
router.post("/importar", authenticateToken, upload.single("file"), async (req, res) => {
    try {
        ...
        */
        ...
        const sinVigencia = items.filter((i) => !i.escala.fechaActualizacion).length;
        if (sinVigencia > 0)
            partes.push(`${sinVigencia} fila(s) sin «vigenciaDesde»: se conservó la fecha anterior. Cargala para que se pueda avisar cuando la escala venza.`);
