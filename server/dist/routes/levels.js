...
router.get("/count", requireTenant, authenticateToken, requirePermission("admin_levels:view"), async (req, res) => {
    try {
...
router.get("/", requireTenant, authenticateToken, requirePermission("admin_levels:view"), async (req, res) => {
    try {
...
router.post("/", requireTenant, authenticateToken, requirePermission("admin_levels:view"), async (req, res) => {
    try {
...
router.get("/:id", requireTenant, authenticateToken, requirePermission("admin_levels:view"), async (req, res) => {
    try {
...
router.patch("/:id", requireTenant, authenticateToken, requirePermission("admin_levels:view"), async (req, res) => {
    try {
...
router.delete("/:id", requireTenant, authenticateToken, requirePermission("admin_levels:view"), async (req, res) => {
    try {
        const levelId = toObjectIdOrNull(req.params.id);
        if (!levelId) {
            res.status(400).json({ error: "Invalid level ID" });
