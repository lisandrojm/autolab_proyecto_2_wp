...
const pad = (valor, largo) => valor.replace(/\D/g, "").padStart(largo, "0").slice(-largo);
...
router.put("/:id", authenticateToken, async (req, res) => {
    try {
        ...
        const data = normalizar({ ...parsed, codigo: parsed.codigo ?? "0", actividades: parsed.actividades ?? [] });
        const update = {};
        if (parsed.codigo !== undefined)
            update.codigo = data.codigo;
        if (parsed.domicilio !== undefined)
            update.domicilio = parsed.domicilio;
        if (parsed.localidad !== undefined)
            update.localidad = parsed.localidad;
        if (parsed.codigoPostal !== undefined)
            update.codigoPostal = parsed.codigoPostal;
        if (parsed.actividades !== undefined)
            update.actividades = data.actividades;
        if (parsed.isActive !== undefined)
            update.isActive = parsed.isActive;
...
router.delete("/:id", authenticateToken, async (req, res) => {
    try {
        ...
        if (enUso.length > 0) {
            return res.status(400).json({ error: `No se puede eliminar: la usan ${enUso.map((e) => e.razonSocial).join(", ")}. Quitala de esas empresas primero.` });
        }
...
const columnasReconocidas = (row) => [ALIAS_CODIGO, ALIAS_DOMICILIO, ALIAS_LOCALIDAD, ALIAS_CP, ALIAS_ACT_CODIGO, ALIAS_ACT_DESC].filter((alias) => leer(row, alias) !== "").length;
 ...
 */
function parsearTextoAnchoFijo(texto) {
    ...
    while ((m = re.exec(cabecera)) !== null) {
        if (m[0].trim())
            columnas.push({ nombre: m[0].trim(), desde: m.index });
    }
 ...
 */
export function parsearExportSucursales(buffer) {
    ...
    rows.forEach((row, i) => {
        ...
        if (!codigoRaw && !direccion && !actCodigo)
            return; // fila vacía de relleno
        let codigo;
        if (codigoRaw) {
            const soloDigitos = codigoRaw.replace(/\D/g, "");
            if (!soloDigitos || soloDigitos.length > 5)
                return void errores.push(`Fila ${fila}: código de sucursal inválido ("${codigoRaw}")`);
        ...
        if (!actual.localidad)
            actual.localidad = leer(row, ALIAS_LOCALIDAD);
        if (!actual.codigoPostal)
            actual.codigoPostal = leer(row, ALIAS_CP).replace(/\D/g, "");
