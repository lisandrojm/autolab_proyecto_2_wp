...
const router = Router();
router.get("/metrics", authenticateToken, requirePlatform(), async (req, res) => {
    try {
        const now = new Date();
        ...
        const planDistribution = tenantsWithPlans.reduce((acc, item) => {
...
router.get("/activity", authenticateToken, requirePlatform(), async (req, res) => {
    try {
