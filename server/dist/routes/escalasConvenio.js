...
import { DatoDeEscalaInvalido, armarPeriodo, motivoDeSuperposicion, cerrarPeriodosAbiertos, espejarVigenteEnGrupo, escalaDelConvenioAFecha, adicionalesAFecha, pequenasEmpresasAFecha, desvioJornadaAdicional, esElVigente, } from "../services/escalasConvenio.js";
...
import { liquidacionDeReferencia } from "../utils/liquidacionReferencia.js";
/**
...
const fechaDelQuery = (req) => aIsoFecha(String(req.query.fecha || "")) || hoyIso();
const convenioDelQuery = (req) => String(req.query.convenio || "").trim();
const responderError = (res, error, contexto) => {
    if (error instanceof DatoDeEscalaInvalido)
        return res.status(400).json({ error: error.message });
    if (error?.code === 11000)
        return res.status(409).json({ error: "Ya existe un registro con esa vigencia." });
...
router.get("/", authenticateToken, async (req, res) => {
    try {
         ...
         */
        const porGrupo = new Map(escala.map((e) => [String(e.grupo ?? ""), e]));
        const filas = grupos.map((g) => {
            const periodo = porGrupo.get(String(g.numero));
            return {
                grupo: g.numero,
                nombre: g.nombre || "",
                vigenteEnGrupo: {
 ...
 */
router.post("/periodos", authenticateToken, async (req, res) => {
    try {
        const body = req.body || {};
        ...
        if (!desde)
            return res.status(400).json({ error: "La vigencia desde es obligatoria." });
        ...
        if (choque)
            return res.status(409).json({ error: choque });
 ...
 */
router.put("/periodos/:id", authenticateToken, async (req, res) => {
    try {
        ...
        const espejo = esElVigente(actualizado, hoyIso()) ? await espejarVigenteEnGrupo(actual.convenio, actual.grupo ?? null, hoyIso()) : null;
 ...
 */
router.delete("/periodos/:id", authenticateToken, async (req, res) => {
    try {
 ...
 */
router.post("/periodos/:id/espejar", authenticateToken, async (req, res) => {
    try {
        const periodo = await EscalaPeriodo.findById(req.params.id).lean();
        if (!periodo)
            return res.status(404).json({ error: "El período no existe." });
        const espejo = await espejarVigenteEnGrupo(periodo.convenio, periodo.grupo ?? null, hoyIso());
        res.json(espejo);
    }
    catch (error) {
...
router.get("/adicionales", authenticateToken, async (req, res) => {
    try {
        const convenio = convenioDelQuery(req);
        if (!convenio)
            return res.status(400).json({ error: "Falta el convenio." });
 ...
 */
router.post("/adicionales", authenticateToken, async (req, res) => {
    try {
        const body = req.body || {};
        const convenio = String(body.convenio || "").trim();
        ...
        if (!convenio)
            return res.status(400).json({ error: "El convenio es obligatorio." });
        if (!codigo)
            return res.status(400).json({ error: "El código es obligatorio (es la clave del adicional dentro del convenio)." });
        if (!nombre)
            return res.status(400).json({ error: "El nombre es obligatorio." });
        if (await AdicionalConvenio.findOne({ convenio, codigo }))
            return res.status(409).json({ error: `El convenio ${convenio} ya tiene un adicional con código «${codigo}».` });
...
router.put("/adicionales/:id", authenticateToken, async (req, res) => {
    try {
        const body = req.body || {};
        const set = {};
        if (body.nombre !== undefined)
            set.nombre = String(body.nombre).trim();
        if (body.tipoCalculo !== undefined)
            set.tipoCalculo = body.tipoCalculo;
        // `null` tiene significado ("no se sabe"), así que se distingue de "no vino la clave".
        if (body.remunerativo !== undefined)
            set.remunerativo = body.remunerativo === true ? true : body.remunerativo === false ? false : null;
        if (body.confirmado !== undefined)
            set.confirmado = body.confirmado === true;
        if (body.base !== undefined)
            set.base = body.base || null;
        for (const k of ["unidad", "condicion", "conceptoLiquidacion"])
            if (body[k] !== undefined)
                set[k] = String(body[k] ?? "").trim();
        if (body.codigoArca !== undefined)
            set.codigoArca = numeroOpcional(body.codigoArca);
        if (body.capitulo !== undefined)
            set.capitulo = body.capitulo === "pequenas_empresas" ? "pequenas_empresas" : "general";
        if (body.orden !== undefined)
            set.orden = Number(body.orden || 0);
        ...
        if (!actualizado)
            return res.status(404).json({ error: "El adicional no existe." });
 ...
 */
router.delete("/adicionales/:id", authenticateToken, async (req, res) => {
    try {
...
router.get("/adicionales/:id/valores", authenticateToken, async (req, res) => {
    try {
        const valores = await AdicionalValorPeriodo.find({ adicionalId: req.params.id }).lean();
        res.json(ordenarPorVigencia(valores));
    }
    catch (error) {
...
router.post("/adicionales/:id/valores", authenticateToken, async (req, res) => {
    try {
        ...
        if (hasta && hasta < desde)
            return res.status(400).json({ error: "La vigencia termina antes de empezar." });
        const grupo = numeroOpcional(body.grupo);
        if (body.cerrarAnterior === true) {
...
router.put("/valores/:id", authenticateToken, async (req, res) => {
    try {
        const body = req.body || {};
        const set = {};
        if (body.desde !== undefined) {
            const desde = aIsoFecha(String(body.desde));
            if (!desde)
                return res.status(400).json({ error: "La vigencia desde no se entiende." });
        ...
        if (body.monto !== undefined)
            set.monto = numeroOpcional(body.monto);
        if (body.porcentaje !== undefined)
            set.porcentaje = numeroOpcional(body.porcentaje);
        if (body.grupo !== undefined)
            set.grupo = numeroOpcional(body.grupo);
        if (body.nota !== undefined)
            set.nota = String(body.nota ?? "");
        if (body.origen !== undefined)
            set.origen = body.origen;
        const actualizado = await AdicionalValorPeriodo.findByIdAndUpdate(req.params.id, { $set: set }, { new: true, runValidators: true });
        if (!actualizado)
            return res.status(404).json({ error: "El importe no existe." });
        res.json(actualizado);
    }
    catch (error) {
...
router.delete("/valores/:id", authenticateToken, async (req, res) => {
    try {
...
router.get("/pequenas-empresas", authenticateToken, async (req, res) => {
    try {
        const convenio = convenioDelQuery(req);
        if (!convenio)
            return res.status(400).json({ error: "Falta el convenio." });
        if (String(req.query.historial) === "true") {
            ...
            return res.json(ordenarPorVigencia(filas));
        }
 ...
 */
router.post("/pequenas-empresas", authenticateToken, async (req, res) => {
    try {
        const body = req.body || {};
        const convenio = String(body.convenio || "").trim();
        const grupo = numeroOpcional(body.grupo);
...
router.put("/pequenas-empresas/:id", authenticateToken, async (req, res) => {
    try {
        const body = req.body || {};
        const set = {};
        for (const k of ["semana9hsLunVie", "jornadaAdicional9hs", "horaExtra50", "horaExtra100"])
            if (body[k] !== undefined)
                set[k] = redondearCentavos(Number(body[k] || 0));
        if (body.desde !== undefined) {
            const desde = aIsoFecha(String(body.desde));
            if (!desde)
                return res.status(400).json({ error: "La vigencia desde no se entiende." });
        ...
        if (!actualizado)
            return res.status(404).json({ error: "La fila no existe." });
...
router.delete("/pequenas-empresas/:id", authenticateToken, async (req, res) => {
    try {
...
router.get("/acuerdos", authenticateToken, async (req, res) => {
    try {
        const convenio = convenioDelQuery(req);
        ...
        res.json(acuerdos.map((a) => ({
            ...a,
            porcentajeAcumuladoGeneral: porcentajeAcumulado((a.tramos || []).filter((t) => t.regimen !== "alternativo" && t.acumulativo !== false).map((t) => Number(t.porcentaje || 0))),
        })));
    }
    catch (error) {
...
router.post("/acuerdos", authenticateToken, async (req, res) => {
    try {
        const body = req.body || {};
...
router.put("/acuerdos/:id", authenticateToken, async (req, res) => {
    try {
        const body = { ...(req.body || {}) };
        ...
        if (!actualizado)
            return res.status(404).json({ error: "El acuerdo no existe." });
        res.json(actualizado);
    }
    catch (error) {
...
router.delete("/acuerdos/:id", authenticateToken, async (req, res) => {
    try {
        ...
        if (borrado.archivo?.ruta)
            await borrarArchivo(borrado.archivo.ruta);
 ...
 */
router.post("/acuerdos/:id/archivo", authenticateToken, upload.single("archivo"), async (req, res) => {
    try {
        ...
        if (!esPdf)
            return res.status(400).json({ error: "El acta tiene que ser un PDF." });
...
router.get("/acuerdos/:id/archivo", authenticateToken, async (req, res) => {
    try {
        const acuerdo = await AcuerdoParitario.findById(req.params.id).select("archivo titulo").lean();
        const ruta = acuerdo?.archivo?.ruta;
        if (!ruta)
            return res.status(404).json({ error: "El acuerdo no tiene el acta adjunta." });
        ...
        res.setHeader("Content-Type", acuerdo.archivo.contentType || "application/pdf");
        res.setHeader("Content-Disposition", `inline; filename="${(acuerdo.archivo.nombreOriginal || "acta.pdf").replace(/"/g, "")}"`);
 ...
 */
router.post("/aplicar-paritaria/preview", authenticateToken, async (req, res) => {
    try {
        const body = req.body || {};
        const convenio = String(body.convenio || "").trim();
        ...
        const filasBase = periodos.length
            ? periodos.map((p) => ({ grupo: p.grupo, basico: p.basico, adicionalPct: p.adicionalPct, presentismoPct: p.presentismoPct, netoFactor: p.netoFactor, adicionalMonto: p.adicionalMonto, presentismoMonto: p.presentismoMonto, total: p.total, neto: p.neto }))
            : grupos.map((g) => ({
                grupo: g.numero,
                basico: g.sueldoBasico,
        ...
        const pequenasPropuestas = body.incluirPequenasEmpresas === false
            ? []
            : pequenas.map((f) => ({
                grupo: f.grupo,
 ...
 */
router.post("/aplicar-paritaria", authenticateToken, async (req, res) => {
    try {
        const body = req.body || {};
        const convenio = String(body.convenio || "").trim();
        const desde = aIsoFecha(String(body.desde || ""));
        if (!convenio)
            return res.status(400).json({ error: "Falta el convenio." });
        if (!desde)
            return res.status(400).json({ error: "Falta la fecha desde la que rige el tramo." });
        const filas = Array.isArray(body.escala) ? body.escala : [];
        if (!filas.length)
            return res.status(400).json({ error: "No llegó ninguna fila de escala." });
        const hoy = hoyIso();
        const grupos = await ConvenioGrupo.find({ convenio }).select("numero").lean();
        const idPorNumero = new Map(grupos.map((g) => [Number(g.numero), String(g._id)]));
        const resultado = [];
        for (const fila of filas) {
            ...
            if (existente)
                await EscalaPeriodo.updateOne({ _id: existente._id }, { $set: datos });
            else
                await EscalaPeriodo.create(datos);
        ...
        for (const a of Array.isArray(body.adicionales) ? body.adicionales : []) {
            ...
            await AdicionalValorPeriodo.updateOne({ adicionalId: adicional._id, grupo: numeroOpcional(a.grupo) ?? null, desde: corte }, { $set: { convenio, monto: numeroOpcional(a.monto), porcentaje: numeroOpcional(a.porcentaje), acuerdoId: body.acuerdoId || null, tramo: body.tramo || "", origen: body.origen || "manual", createdBy: req.user?.userId || null } }, { upsert: true });
        ...
        for (const p of Array.isArray(body.pequenasEmpresas) ? body.pequenasEmpresas : []) {
            ...
            await EscalaPequenasEmpresas.updateOne({ convenio, grupo, desde: corte }, {
                $set: {
 ...
 */
router.post("/liquidacion-referencia", authenticateToken, async (req, res) => {
    try {
        const body = req.body || {};
        ...
        if (body.categoriaId) {
            ...
            convenio = String(cat.convenio || "");
            categoriaNombre = String(cat.nombre || "");
            if (cat.grupoId) {
                const g = await ConvenioGrupo.findById(cat.grupoId).select("numero").lean();
                grupo = g ? Number(g.numero) : null;
            }
        }
        if (!convenio)
            return res.status(400).json({ error: "Falta el convenio (o una categoría de la que sacarlo)." });
        ...
        const periodo = periodoVigente(periodos, fecha);
        const grupoActual = gruposDoc[0];
        /**
        ...
        const liquidacion = liquidacionDeReferencia(escala, adicionales, { aniosAntiguedad: Number(body.aniosAntiguedad || 0), cantidades: body.cantidades || {} });
        res.json({
            convenio,
            grupo,
