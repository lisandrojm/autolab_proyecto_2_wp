...
leaveAccountsRouter.get("/", async (req, res) => {
    try {
...
leaveAccountsRouter.post("/", async (req, res) => {
    try {
...
leaveAccountsRouter.patch("/:id", async (req, res) => {
    try {
 ...
 */
leaveBalancesRouter.get("/users", async (req, res) => {
    try {
        const page = Math.max(1, Number(req.query.page) || 1);
        const limit = Math.min(100, Math.max(1, Number(req.query.limit) || 25));
        const periodo = req.query.periodo ? String(req.query.periodo) : undefined;
        const filtro = { tenantId: req.tenantObjectId, isSystem: { $ne: true } };
        if (req.query.search) {
        ...
        const saldos = await saldosDe(req.tenantObjectId, usuarios.map((u) => u._id), periodo);
        res.json({
            rows: usuarios.map((u) => ({
                userId: String(u._id),
...
leaveBalancesRouter.get("/", async (req, res) => {
    try {
        const userId = String(req.query.userId || "");
        if (!Types.ObjectId.isValid(userId))
            return res.status(400).json({ error: "Falta indicar la persona." });
        const periodo = req.query.periodo ? String(req.query.periodo) : undefined;
...
leaveLedgerRouter.get("/", async (req, res) => {
    try {
        const page = Math.max(1, Number(req.query.page) || 1);
        const limit = Math.min(200, Math.max(1, Number(req.query.limit) || 50));
        const filtro = { tenantId: req.tenantObjectId };
        if (Types.ObjectId.isValid(String(req.query.userId)))
            filtro.userId = new Types.ObjectId(String(req.query.userId));
        if (Types.ObjectId.isValid(String(req.query.accountId)))
            filtro.accountId = new Types.ObjectId(String(req.query.accountId));
 ...
 */
leaveLedgerRouter.post("/adjust", async (req, res) => {
    try {
        ...
        const movimiento = await LeaveLedger.create({
            ...
            date: fecha,
            periodo: data.periodo || periodoDe(fecha, cuenta.accrual?.periodo),
