...
router.put("/vacations/:id/approve", async (req, res) => {
    try {
...
router.put("/vacations/:id/reject", async (req, res) => {
    try {
...
router.put("/vacations/:id/pre-approve", async (req, res) => {
    try {
        ...
        if (templateId && templateId.toString().trim() !== "") {
            ...
            if (template) {
                ...
                const pdfResult = await generateVacationPDF(vacation, template, user, req.tenantObjectId.toString(), tenant.name, vacation.vacationNumber);
                if (pdfResult.success) {
                    ...
                    // We still save the status update, but we want the user to know PDF failed
                }
            }
            else {
                console.warn("[PRE-APPROVE] PDF template not found or inactive:", templateId);
            }
        }
        await vacation.save();
        res.json(vacation);
...
router.put("/vacations/:id/deliver", async (req, res) => {
    try {
        const vacation = await Vacation.findOne({
            _id: req.params.id,
            tenantId: req.tenantObjectId,
...
router.put("/vacations/:id/send-signature", async (req, res) => {
    try {
        const vacation = await Vacation.findOne({
            _id: req.params.id,
            tenantId: req.tenantObjectId,
...
router.put("/vacations/:id/mark-signed", async (req, res) => {
    try {
...
router.post("/vacations/:id/regenerate-pdf", async (req, res) => {
    try {
        const vacation = await Vacation.findOne({
            _id: req.params.id,
            tenantId: req.tenantObjectId,
...
router.delete("/documents/:id", async (req, res) => {
    try {
        ...
        order.documents = order.documents.filter((d) => d._id.toString() !== req.params.id);
        await order.save();
