...
router.get("/simplificacion", async (req, res) => {
    try {
        const tenant = await Tenant.findById(req.tenantObjectId).select("integrations.arcaSimplificacion").lean();
...
router.post("/simplificacion", async (req, res) => {
    try {
        if (!isAdmin(req)) {
...
router.delete("/simplificacion", async (req, res) => {
    try {
        if (!isAdmin(req)) {
...
router.get("/simplificacion/logs", async (req, res) => {
    try {
        if (!isAdmin(req)) {
 ...
 */
router.post("/padron/consultar", async (req, res) => {
    try {
        if (!isAdmin(req)) {
...
router.get("/constancia-link", async (req, res) => {
    try {
        const { projectId, userId, contractIndex } = req.query;
        ...
        const path = up?.contracts?.[idx]?.constanciaAfipDropboxPath;
        if (!path) {
 ...
 */
const resolverCarpetaConstanciaCuit = () => resolverCarpetaPorProposito("constancia_cuit");
...
router.post("/consulta-padron/bulk", async (req, res) => {
    try {
        const tenant = await Tenant.findById(req.tenantObjectId).lean();
        const cfg = getTenantAfipConfig(tenant);
        if (!cfg) {
        ...
        let targets = [];
        try {
            targets = z.array(padronTargetSchema).max(5000).parse(req.body?.targets);
        }
        catch {
        ...
        const docsCache = new Map(); // `${projectId}|${userId}` → documento UserProject
        const resultados = [];
        ...
        for (let i = 0; i < cuits.length; i += CONCURRENCIA) {
            ...
            await Promise.all(lote.map(async (cuit) => {
                ...
                try {
                    ...
                    for (const t of matches) {
                        ...
                        if (t.contractIndex < 0 || t.contractIndex >= up.contracts.length)
                            continue;
                            ...
                            if (!dropboxCfg) {
                                ...
                                try {
                                    ...
                                    const contenido = Buffer.from(JSON.stringify({
                                        ...
                                        persona: { userId: t.userId, nombre: user?.firstName, apellido: user?.lastName },
                        ...
                        up.contracts[t.contractIndex] = {
                            ...up.contracts[t.contractIndex].toObject(),
 ...
 * y el Release.
 */
router.post("/habilitar-firma", async (req, res) => {
    try {
        ...
        */
        ...
        let targets = [];
        try {
            targets = z.array(padronTargetSchema).max(1000).parse(req.body?.targets);
        }
        catch {
            res.status(400).json({ error: "El listado de contratos (targets) es inválido." });
            return;
        ...
        const projectOk = new Set(projects.map((p) => String(p._id)));
        const userById = new Map(users.map((u) => [String(u._id), u]));
        const habilitados = [];
        const omitidos = [];
        for (const t of targets) {
            const user = userById.get(t.userId);
            const nombre = `${user?.firstName || ""} ${user?.lastName || ""}`.trim() || t.userId;
            ...
            const cuitDigitos = normalizarCuit(user?.metadata?.cuit);
            ...
            const contenido = Buffer.from(JSON.stringify({
                ...
                persona: { userId: t.userId, nombre: user?.firstName, apellido: user?.lastName, email: user?.email },
                proyecto: { id: t.projectId, nombre: up.nombre_proyecto },
                contrato: { index: t.contractIndex, fechaAlta: contract?.fecha_alta_contrato, fechaBaja: contract?.fecha_baja_contrato },
            }, null, 2));
            try {
...
async function imagenAPdf(buffer) {
