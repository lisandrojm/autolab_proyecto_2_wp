...
router.get("/projects/:projectId/miembros/:userId/contrato", requireTenant, authenticateToken, async (req, res) => {
    try {
        ...
        const proyecto = await Project.findOne({ _id: projectId, tenantId }).select("name").lean();
        if (!proyecto) {
            res.status(404).json({ error: "Proyecto no encontrado." });
            return;
        }
        const roles = (req.user?.roles || []).map((r) => String(r).toLowerCase());
        const primario = String(req.user?.primaryRole || "").toLowerCase();
        ...
        const [asignacion, persona] = await Promise.all([
        ...
        const c = getContratoActivo(asignacion?.contracts || [], hoy);
        if (!c) {
            res.json({ contrato: null });
            return;
        }
        // Área y turnos del contrato: la lista nueva, o el par viejo `areaId`/`shiftId` si es un contrato anterior.
        const asignaciones = (c.areaShiftAssignments || []).map((a) => ({ areaId: String(a.areaId || ""), shiftIds: (a.shiftIds || []).map(String) })).filter((a) => idValido(a.areaId));
        if (asignaciones.length === 0 && idValido(c.areaId))
            asignaciones.push({ areaId: String(c.areaId), shiftIds: idValido(c.shiftId) ? [String(c.shiftId)] : [] });
        ...
        const codigoConvenio = categoria?.convenio || categoriaSat?.data?.convenio || "";
        const convenio = codigoConvenio ? await Convenio.findOne({ externalId: codigoConvenio }).select("name").lean() : null;
        const areaDe = new Map(areas.map((a) => [String(a._id), a.name]));
        const turnoDe = new Map(turnos.map((s) => [String(s._id), s.name]));
        const nombreDe = (u) => u?.metadata?.fullName || `${u?.firstName || ""} ${u?.lastName || ""}`.trim();
        res.json({
            contrato: {
