...
router.get('/', async (req, res) => {
    try {
        ...
        const userContractNames = new Set();
        const userSchedules = new Set();
        const userProjectDates = new Set();
        if (user?.metadata?.projects && Array.isArray(user.metadata.projects)) {
                ...
                if (Array.isArray(up.contracts)) {
                    for (const c of up.contracts) {
                        ...
                        if (c.nombre_contrato)
                            userContractNames.add(c.nombre_contrato);
        ...
        try {
            if (user?.metadata?.projects && Array.isArray(user.metadata.projects)) {
                ...
                const userContractTypeIds = new Set();
                for (const up of user.metadata.projects) {
                    if (!up || typeof up !== 'object')
                        continue;
                    if (Array.isArray(up.contracts)) {
    ...
    try {
        ...
        if (override) {
            if (override.totalAnnual !== undefined)
                displayTotal = override.totalAnnual;
            if (override.taken !== undefined)
                displayUsed = override.taken;
            if (override.pending !== undefined)
                displayPending = override.pending;
            if (override.available !== undefined)
                displayAvailable = override.available;
        }
        res.json({
            daysWorked,
            vacations: {
