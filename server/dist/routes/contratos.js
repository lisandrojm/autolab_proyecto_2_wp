...
*/
const leerLimitesDeJornada = (horas, dias) => {
    ...
    if (h !== null && (!Number.isFinite(h) || h < 1 || h > 24))
        return { horasPorJornada: null, diasPorSemana: null, error: "Las horas por jornada tienen que estar entre 1 y 24." };
    if (d !== null && (!Number.isInteger(d) || d < 1 || d > 7))
        return { horasPorJornada: null, diasPorSemana: null, error: "Los días por semana tienen que ser un número entero entre 1 y 7." };
...
router.put("/:id", authenticateToken, async (req, res) => {
    try {
        ...
        if (horasPorJornada !== undefined)
            item.data.horasPorJornada = limites.horasPorJornada;
        if (diasPorSemana !== undefined)
            item.data.diasPorSemana = limites.diasPorSemana;
        if (esTiempoIndeterminado !== undefined)
            item.data.esTiempoIndeterminado = esTiempoIndeterminado === "true" || esTiempoIndeterminado === true;
        if (modoFechas !== undefined)
