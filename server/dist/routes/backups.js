...
router.get("/copias", async (req, res) => {
    ...
    try {
        ...
        const copias = await Promise.all(carpetas.map(async (c) => {
            try {
                ...
                return {
                    ...
                    bytes: Array.isArray(m.colecciones) ? m.colecciones.reduce((a, x) => a + (Number(x.bytes) || 0), 0) : 0,
                    completa: true,
                };
            }
            catch {
 ...
 */
router.get("/copias/archivos", async (req, res) => {
    if (!isAdmin(req)) {
    ...
    try {
        ...
        const archivos = entries
            .filter((e) => e.tag === "file")
            .map((e) => ({ nombre: e.name, path: e.path, bytes: Number(e.size) || 0 }))
...
router.get("/copias/ver", async (req, res) => {
    if (!isAdmin(req)) {
        res.status(403).json({ error: "Solo un administrador puede ver el contenido de una copia." });
        return;
    }
...
router.get("/copias/link", async (req, res) => {
    if (!isAdmin(req)) {
        res.status(403).json({ error: "Solo un administrador puede descargar una copia." });
        return;
    }
