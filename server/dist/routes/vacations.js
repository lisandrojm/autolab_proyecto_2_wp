                    ...
                    for (const aId of finalAreas) {
                        for (const posId of finalPositions) {
                            for (const lId of finalLevels) {
                                ...
                                if (aId)
                                    otherUsersQuery.areaId = aId;
                                if (posId)
                                    otherUsersQuery.positionId = posId;
                                if (lId)
                                    otherUsersQuery.levelId = lId;
                                if (rule.roleFrameId) {
                                    const rf = await RoleFrame.findById(rule.roleFrameId);
                                    if (rf) {
                                        const values = [];
        ...
        if (templateId) {
            try {
                ...
                if (template) {
                    ...
                    const result = await generateVacationPDF(vacation, template, user, tenantId.toString(), tenantName, vacation.vacationNumber);
