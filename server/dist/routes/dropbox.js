...
function dropboxError(res, error) {
    ...
    if (Buffer.isBuffer(dbx) || dbx instanceof ArrayBuffer) {
        const txt = Buffer.from(dbx).toString("utf8");
        try {
            dbx = JSON.parse(txt);
        }
        catch {
...
router.post("/download-zip", async (req, res) => {
    try {
        ...
        */
        const crudos = Array.isArray(req.body?.items)
            ? req.body.items.map((i) => ({ path: String(i?.path || ""), id: i?.id ? String(i.id) : undefined }))
            : (Array.isArray(req.body?.paths) ? req.body.paths : []).map((p) => ({ path: String(p || "") }));
        // Normaliza y deduplica por path.
        const porPath = new Map();
        for (const it of crudos)
            if (it.path)
                porPath.set(it.path, it);
