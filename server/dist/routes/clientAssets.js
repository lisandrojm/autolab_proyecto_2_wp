...
const captureIdentifiers = (req, _res, next) => {
...
// ---------------------------
const fileFilterImages = (_req, file, cb) => {
    const allowed = ["image/png", "image/jpeg", "image/jpg", "image/gif", "image/webp", "image/svg+xml", "image/bmp", "image/tiff", "image/x-icon"];
    if (allowed.includes(file.mimetype))
        cb(null, true);
    else
        cb(new Error("Solo se permiten imágenes en formatos: PNG, JPG, GIF, WebP, SVG, BMP, TIFF"));
};
const fileFilterAllTypes = (_req, file, cb) => {
    const prohibited = [
...
// ---------------------------
function registerScopeRoutes(scope, upload) {
    ...
    router.post(`/${scope}/upload`, requireTenant, authenticateToken, captureIdentifiers, (req, res, next) => {
...
router.post("/brandkit-documents/upload", requireTenant, authenticateToken, captureIdentifiers, (req, res, next) => {
    ...
    next();
}, uploadBrandkitDocuments.single("file"), (err, req, res, next) => {
    if (err) {
