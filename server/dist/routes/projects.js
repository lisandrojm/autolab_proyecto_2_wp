...
router.post("/projects/constancias/bulk-upload", requireTenant, authenticateToken, requireAnyRole, uploadConstancias, async (req, res) => {
    try {
        ...
        try {
            ...
            targets = z.array(constanciaTargetSchema).max(5000).parse(parsed);
        }
        catch {
        ...
        const nombreDe = (u) => [u?.metadata?.nombre || u?.firstName, u?.metadata?.apellido || u?.lastName].filter(Boolean).join(" ").trim() || "Sin nombre";
        const targetsPorCuit = new Map();
        for (const t of targets) {
            const user = userById.get(t.userId);
            if (!user || !projectNameById.has(t.projectId))
                continue;
        ...
        for (const file of files) {
            ...
            for (const t of matches) {
                ...
                const anterior = up.contracts[t.contractIndex]?.altaDocumentoUrl;
                if (anterior && typeof anterior === "string") {
