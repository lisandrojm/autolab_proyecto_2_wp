...
async function filtroBusquedaProyecto(q) {
    ...
    const o = [{ name: regex }];
    if (ids.length > 0)
        o.push({ "metadata.centroCostoId": { $in: ids } });
    return { $or: o };
}
/**
 ...
 */
function sumarAlFiltro(filter, condicion) {
    if (filter.$or) {
 ...
 */
async function resolverCentroCostoDeProyectos(projects) {
        ...
        if (req.query.slim === "true") {
            ...
            */
            ...
            for (const p of proyectos) {
                p.coordinatorAssignments = (p.coordinatorAssignments || []).filter((a) => String(typeof a.userId === "object" ? a.userId?._id : a.userId) === yo);
            }
            res.json({
                projects: proyectos,
...
router.get("/projects/:projectId", requireTenant, authenticateToken, requireAnyRole, async (req, res) => {
    try {
         ...
         */
        if (!soloIdsDeEquipo) {
            const miembros = (project.assignedUsers || []).filter((u) => u && typeof u === "object");
            if (miembros.length > 0) {
                ...
                const vinculos = await UserProject.find({
                    ...
                    userId: { $in: miembros.map((u) => u._id) },
                })
                    ...
                    .lean();
                const porUsuario = new Map(vinculos.map((v) => [String(v.userId), v]));
...
router.get("/projects/:projectId/area-shift-members", requireTenant, authenticateToken, requireAnyRole, async (req, res) => {
    try {
        ...
        for (const member of members) {
            ...
            if (!todos && shiftIdsDelMiembro.length === 0)
                continue;
...
*/
router.patch("/projects/:projectId/team-config/:userId/areas", requireTenant, authenticateToken, requireAnyRole, async (req, res) => {
    try {
        ...
        const entrada = config.find((c) => String(c.userId) === String(userId));
        if (entrada) {
...
router.post("/projects/:projectId/assign-member", requireTenant, authenticateToken, requireAnyRole, async (req, res) => {
    try {
        ...
        */
        ...
        if (faltanDias)
            return res.status(422).json({ error: `Faltan los días que trabaja: ${faltanDias}.` });
        ...
        */
        delete enrichedContract.solicitudId;
        if (idSolicitud)
            enrichedContract.solicitudId = new Types.ObjectId(idSolicitud);
        ...
        if (!userProject) {
            ...
            if (prevContract) {
                ...
                if (enrichedContract.solicitudId === undefined && prevContract.solicitudId)
                    enrichedContract.solicitudId = prevContract.solicitudId;
            }
            if (hasExplicitIndex) {
        ...
        */
        if (solicitudAprobada) {
            ...
            */
            const cambios = await cambiosAlAprobar(solicitudAprobada.metadata, enrichedContract, project);
            const comentario = typeof comentarioRevision === "string" ? comentarioRevision.trim().slice(0, 2000) : "";
            if (cambios.length > 0 || comentario) {
                const quienAprueba = await User.findById(req.user.userId).select("firstName lastName metadata.fullName").lean();
                await User.updateOne({ _id: solicitudAprobada._id }, { $set: { "metadata.solicitudRevision": { cambios, comentario, porNombre: nombreDePersona(quienAprueba), el: new Date() } } });
            }
            /*
            ...
            */
            ...
            await notificar({
                tenantId: req.tenantObjectId,
                destinatarios: [solicitudAprobada.metadata?.solicitudCreadaPor],
    ...
    try {
        ...
        if (actividadArca) {
            ...
            */
            ...
            const sucursalId = contrato.sucursalArcaId || empresaDelContrato?.defaultsArca?.sucursalId || (globalAplica ? sucursalGlobal : null);
            if (!sucursalId) {
