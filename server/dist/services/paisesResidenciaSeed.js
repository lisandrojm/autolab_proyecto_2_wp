 ...
 */
export async function sembrarPaisesResidenciaUnaVez() {
    const marca = await SemillaAplicada.updateOne({ clave: CLAVE }, { $setOnInsert: { clave: CLAVE, aplicadaEl: new Date() } }, { upsert: true });
    if (marca.upsertedCount === 0)
        return; // ya se aplicó, acá o en otra instancia
    try {
        ...
        */
        const paises = await Info.find({ type: "pais" }).sort({ name: 1 }).lean();
        const porId = new Map();
        for (const p of paises) {
            const id = Number(p?.data?.id);
            if (p?.data?.id == null || Number.isNaN(id) || !p?.name || porId.has(id))
                continue;
            porId.set(id, { externalId: String(id), name: p.name, activo: true, data: { id, nombre: p.name } });
        }
        const filas = [...porId.values()];
        if (filas.length > 0)
            await PaisResidencia.insertMany(filas, { ordered: false });
