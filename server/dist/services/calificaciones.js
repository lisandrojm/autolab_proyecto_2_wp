...
import { Calificacion, ESTRELLAS_VALIDAS } from "../models/Calificacion.js";
 ...
 */
export function leerCalificacion(body) {
    const estrellas = Number(body?.estrellas);
    if (!ESTRELLAS_VALIDAS.includes(estrellas))
        return { error: "La calificación tiene que ser de 1, 2, 4 o 5 estrellas." };
...
export async function nombreDeUsuario(userId) {
    const u = await User.findById(userId).select("firstName lastName metadata.fullName").lean();
...
export async function resumenDeCalificaciones(tenantId, userIds) {
    const ids = [...new Set(userIds.filter((id) => Types.ObjectId.isValid(id)))].map((id) => new Types.ObjectId(id));
    if (ids.length === 0)
        return {};
    const filas = await Calificacion.aggregate([{ $match: { tenantId, userId: { $in: ids } } }, { $group: { _id: "$userId", promedio: { $avg: "$estrellas" }, cantidad: { $sum: 1 } } }]);
    const resumen = {};
    for (const f of filas)
        resumen[String(f._id)] = { promedio: Math.round(f.promedio * 10) / 10, cantidad: f.cantidad };
    return resumen;
}
/** Todas las de una persona, la más nueva primero, con el proyecto resuelto a nombre. */
export async function historialDeCalificaciones(tenantId, userId) {
    const lista = await Calificacion.find({ tenantId, userId: new Types.ObjectId(userId) }).sort({ createdAt: -1 }).populate("projectId", "name").lean();
 ...
 */
export async function calificarFinDeContrato(datos) {
    const { contrato } = datos;
    await Calificacion.updateOne({ tenantId: datos.tenantId, origen: "fin_contrato", userProjectId: new Types.ObjectId(contrato.userProjectId), fechaBajaContrato: contrato.fechaBaja }, {
        $set: {
 ...
 * algún área/turno.
 */
export async function tieneAlCargoElProyecto(tenantId, userId, projectId) {
...
export async function esDelProyecto(tenantId, userId, projectId) {
