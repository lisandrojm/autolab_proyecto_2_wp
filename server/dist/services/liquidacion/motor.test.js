...
const ANA = { apellidoYNombre: "Aguirre, Ana", legajo: "00001", empresaId: "emp2030", ccCodigo: "426", regimen: "mensual" };
const BETO = { apellidoYNombre: "Bravo, Beto", legajo: "00002", empresaId: "emp2030", ccCodigo: "426", regimen: "jornalero" };
const padron = (userId) => (userId === "ana" ? ANA : userId === "beto" ? BETO : undefined);
const parte = (renglones, fecha = "2026-08-12") => ({
    _id: "parte1",
    date: fecha,
    projectId: "proy1",
...
const SIN_REGLAS = { horasExtra: null, jornalBase: null };
describe("liquidación · etapa 1: normalizar", () => {
    ...
    it("un reemplazo genera DOS eventos, uno por persona", () => {
        const eventos = normalizarParte(parte([{ _id: "r1", employeeId: "ana", status: "absent", absenceReason: "Enfermedad", replacementId: "beto" }]), padron);
    ...
    it("al reemplazante no se le copia el 'ausente' del titular", () => {
        ...
        const eventos = normalizarParte(parte([{ _id: "r1", employeeId: "ana", status: "absent", absenceReason: "Franco", replacementId: "beto" }]), padron);
        assert.equal(eventos[1].estado, "present");
        assert.equal(eventos[1].reemplazaA, "Aguirre, Ana");
    });
    it("cada uno se lleva SUS horas extra", () => {
        const eventos = normalizarParte(parte([{ _id: "r1", employeeId: "ana", status: "absent", replacementId: "beto", overtimeHours50: 2, replacementOvertimeHours100: 3, replacementOvertimeHours: 3 }]), padron);
...
describe("liquidación · etapa 2: codificar", () => {
    ...
    it("el jornal no se paga dos veces cuando el motivo ya lo generó", () => {
        ...
        */
        ...
        const { lineas } = codificarEvento(evento, [efecto({ conceptoCodigo: "0000", param: "par2", aplicaA: "titular" })], { horasExtra: null, jornalBase: { codigo: "0000", param: "par2", soloRegimen: "jornalero" } });
        assert.equal(lineas.length, 1);
        assert.equal(lineas[0].par2, 1);
    });
