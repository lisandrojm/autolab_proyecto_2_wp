...
const fecha = (v) => {
    ...
    if (!crudo || ["null", "undefined", "-"].includes(crudo.toLowerCase()))
        return "";
...
const id = (v) => String(v && typeof v === "object" ? v._id || "" : v || "").trim();
/** "Cocina: Mañana, Tarde · Piso: Noche" — el área y sus turnos, en un renglón comparable. */
const textoAreasTurnos = (asignaciones, areas, turnos) => (asignaciones || [])
    .map((a) => {
    const area = areas.get(id(a?.areaId)) || "";
    const suyos = (a?.shiftIds || []).map((s) => turnos.get(id(s)) || "").filter(Boolean);
    if (!area && suyos.length === 0)
        return "";
 ...
 */
export async function cambiosAlAprobar(meta, contrato, proyecto) {
    if (!meta)
        return [];
    /*
    ...
    */
    ...
    for (const a of meta.areaShiftAssignments || []) {
        if (id(a?.areaId))
            idsAreas.add(id(a.areaId));
        for (const s of a?.shiftIds || [])
            if (id(s))
                idsTurnos.add(id(s));
    }
    ...
    const idsRolesEmpresa = (meta.roles_frame || meta.rolesFrameIds || (meta.roleFrameId ? [meta.roleFrameId] : [])).map(id).filter(Boolean);
    const idsProyectos = (meta.projectIds || []).map(id).filter(Boolean);
    const soloValidos = (ids) => [...ids].filter((x) => Types.ObjectId.isValid(x));
    ...
    const areas = new Map(areasDocs.map((a) => [String(a._id), a.name]));
    const turnos = new Map(turnosDocs.map((t) => [String(t._id), t.name]));
    const nombreDePersona = (u) => (u ? (u.metadata?.fullName || `${u.firstName || ""} ${u.lastName || ""}`.trim() || "") : "");
    /*
    ...
    */
    ...
    const diasPedidos = (meta.diasSemana || []).map((d) => DIAS[Number(d)] || "").filter(Boolean).join(", ");
    const diasAprobados = (contrato?.dias_semana || []).map((d) => DIAS[Number(d)] || "").filter(Boolean).join(", ");
    /*
    ...
    */
    const eligioDeLaLista = (pedidos, aprobado) => pedidos.length > 0 && pedidos.some((x) => x === aprobado);
    const proyectosPedidos = proyectosDocs.map((p) => texto(p.name)).filter(Boolean);
    const rolesPedidos = rolesDocs.map((r) => texto(r.name)).filter(Boolean);
    ...
    const comparaciones = [
        ...
        { campo: "Tipo de contrato", pedido: texto(contratoPedido?.name) || texto(meta.nombre_contrato), aprobado: texto(contrato?.nombre_contrato) },
        ...
        { campo: "Categoría", pedido: texto(categoriaPedida?.name), aprobado: texto(contrato?.nombre_categoria_sat) },
        { campo: "Empresa del contrato", pedido: texto(empresaPedida?.razonSocial), aprobado: texto(contrato?.nombre_empresa_contrato) },
    ...
    */
    const vacio = (v) => !v || /^sin (categoria|categoría|sede|tipo|área|area|turno|rol frame)$/i.test(v);
...
async function buscarCategoriaPorId(idCategoria) {
