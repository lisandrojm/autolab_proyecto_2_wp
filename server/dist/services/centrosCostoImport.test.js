...
const item = (idAuxiliar, codAuxiliar, habilitado = "S") => ({ idAuxiliar, codAuxiliar, descAuxiliar: `${codAuxiliar}-DESC`, habilitado });
/* ---------------------------------- validarPayload ---------------------------------- */
{
...
{
    const r = validarPayload({ items: "no es una lista" });
...
{
    const r = validarPayload({ items: [{ idAuxiliar: "1", codAuxiliar: "99", descAuxiliar: "x", habilitado: "S" }] });
...
{
    const r = validarPayload({ items: [{ idAuxiliar: 1, codAuxiliar: "99", descAuxiliar: "x", habilitado: "SI" }] });
    ok(!r.ok && r.errores[0].includes("habilitado"), "habilitado que no es S ni N: falla", r.errores);
}
{
    const r = validarPayload({ items: [{ idAuxiliar: 1, codAuxiliar: "99", descAuxiliar: "x", habilitado: "n" }] });
...
{
    const r = validarPayload({ items: [item(1, "99"), { idAuxiliar: 2, codAuxiliar: "", descAuxiliar: "", habilitado: "X" }, item(3, "9")] });
