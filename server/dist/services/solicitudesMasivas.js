 ...
 */
export const cargarCatalogos = async (tenantId, 
/**
 ...
 */
proyectosVisibles = null) => {
    const filtroProyectos = { tenantId, ...(proyectosVisibles ? { _id: { $in: proyectosVisibles } } : {}) };
    const proyectosDelAlcance = await Project.find(filtroProyectos).select("name clientId areasConfig contratoEmpresas metadata.responsableId").populate("clientId", "name").lean();
    /*
    ...
    */
    ...
    const idsTurnos = new Set();
    for (const p of proyectosDelAlcance) {
        for (const ac of (p.areasConfig || [])) {
            if (ac?.areaId)
                idsAreas.add(String(ac.areaId?._id || ac.areaId));
            for (const s of (ac?.shiftIds || []))
                idsTurnos.add(String(s?._id || s));
        }
    }
    ...
    */
    const tramitePorContrato = new Map();
    for (const estado of estados) {
        const tipo = estado?.data?.tipoImpositivo;
        if (!tipo)
            continue;
        const idsPlantillas = new Set((estado?.data?.contratoFrameIds || []).map((id) => String(id)));
        for (const cf of plantillas) {
            if (idsPlantillas.has(String(cf._id)) && cf.contratoId)
                tramitePorContrato.set(String(cf.contratoId), tipo);
        }
    }
    const catalogos = {
        // Con el cliente adelante: hay proyectos con el mismo nombre en dos clientes.
        proyectos: proyectos.map((p) => ({ etiqueta: `${p.clientId?.name ? `${p.clientId.name} · ` : ""}${p.name}`, id: String(p._id) })),
        areas: areas.map((a) => ({ etiqueta: a.name, id: String(a._id) })),
        turnos: turnos.map((s) => ({ etiqueta: s.name, id: String(s._id) })),
        roles: roles.map((r) => ({ etiqueta: r.name, id: String(r._id), extra: { rolId: r.data?.rol?.id } })),
        contratos: contratos.map((c) => ({ etiqueta: c.name, id: String(c._id), extra: { esTiempoIndeterminado: !!c.data?.esTiempoIndeterminado, horasPorJornada: c.data?.horasPorJornada ?? null, diasPorSemana: c.data?.diasPorSemana ?? null, tramite: tramitePorContrato.get(String(c._id)) || "" } })),
        // El código del CCT adelante: es lo que cruza con la categoría, y hay actividades con nombres casi iguales.
        convenios: convenios.map((c) => ({ etiqueta: `${c.externalId} · ${c.name}`, id: String(c._id), extra: { cct: String(c.externalId || "").trim() } })),
        categorias: categorias.map((c) => ({ etiqueta: `${c.externalId ? `${c.externalId} · ` : ""}${c.name}`, id: String(c._id), extra: { dataId: c.data?.id, cct: String(c.data?.convenio || "").trim() } })),
        empresas: empresas.map((e) => ({ etiqueta: e.razonSocial, id: String(e._id) })),
        motivos: motivos.filter((m) => !clave(m.name).includes("horas extra")).map((m) => ({ etiqueta: m.name, id: String(m._id) })),
        siNo: [
            { etiqueta: "Sí", id: "si" },
            { etiqueta: "No", id: "no" },
        ],
    ...
    return {
        catalogos,
        proyectos: proyectos,
        turnosPorId: new Map(turnos.map((s) => [String(s._id), s])),
        contratosPorId: new Map(contratos.map((c) => [String(c._id), c])),
        tramitePorContrato,
    };
};
/**
 ...
 */
export const construirPlantilla = async (catalogos) => {
 ...
 */
export const filasDelArchivo = (buffer) => {
    ...
    if (!hoja)
        return [];
    const matriz = xlsx.utils.sheet_to_json(hoja, { header: 1, blankrows: false, raw: false, dateNF: "dd/mm/yyyy" });
    if (matriz.length === 0)
        return [];
    ...
    for (let i = 1; i < matriz.length; i++) {
        ...
        for (const c of COLUMNAS)
            valores[c.key] = String(cruda[columnaDe.get(c.key)] ?? "").trim();
        // Una fila sin nada es el resto de la planilla, no un error.
        if (Object.values(valores).every((v) => v === ""))
            continue;
...
const diasDeTexto = (texto) => {
    ...
    for (const p of partes) {
        ...
        if (!dia)
            return null;
        if (!indices.includes(dia.indice))
            indices.push(dia.indice);
    }
...
const buscarEnCatalogo = (opciones, texto) => {
    const q = clave(texto);
    if (!q)
        return null;
 ...
 */
export const validarFila = (cruda, ctx) => {
    const { valores, fila } = cruda;
    const errores = [];
    const error = (campo, motivo) => errores.push({ fila, campo, motivo });
    // ── La persona ────────────────────────────────────────────────────────────────────────────────
    const cuil = soloDigitos(valores.cuil);
    if (!cuil)
        error("CUIL o DNI", "Falta el CUIL o el DNI.");
    else if (cuil.length !== 11 && cuil.length !== 7 && cuil.length !== 8)
        error("CUIL o DNI", `«${valores.cuil}» no parece un CUIL (11 dígitos) ni un DNI (7 u 8).`);
    const existente = cuil ? ctx.usuariosPorCuil.get(cuil) : null;
    const nombre = valores.nombre.trim();
    if (!existente && !nombre)
        error("Nombre y apellido", "La persona no está en la plataforma, así que hace falta su nombre para crearla.");
    const email = valores.email.trim().toLowerCase();
    if (email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
        error("Email", `«${valores.email}» no es un email válido.`);
    // ── Dónde trabaja ─────────────────────────────────────────────────────────────────────────────
    const proyectoOpt = buscarEnCatalogo(ctx.catalogos.proyectos, valores.proyecto);
    if (!proyectoOpt)
        error("Proyecto", valores.proyecto ? `No hay ningún proyecto que se llame «${valores.proyecto}».` : "Falta el proyecto.");
    ...
    if (!areaOpt)
        error("Área", valores.area ? `No hay ningún área que se llame «${valores.area}».` : "Falta el área.");
    const turnoOpt = buscarEnCatalogo(ctx.catalogos.turnos, valores.turno);
    if (!turnoOpt)
        error("Turno", valores.turno ? `No hay ningún turno que se llame «${valores.turno}».` : "Falta el turno.");
    /*
    ...
    */
    if (proyecto && areaOpt && turnoOpt) {
        const config = (proyecto.areasConfig || []).find((ac) => String(ac.areaId?._id || ac.areaId) === areaOpt.id);
        if (!config)
            error("Área", `«${areaOpt.etiqueta}» no está configurada en ${proyectoOpt.etiqueta}.`);
        else {
            const turnosDelArea = (config.shiftIds || []).map((s) => String(s?._id || s));
            if (!turnosDelArea.includes(turnoOpt.id))
                error("Turno", `«${turnoOpt.etiqueta}» no es un turno de ${areaOpt.etiqueta} en ${proyectoOpt.etiqueta}.`);
        }
    }
    const rolOpt = buscarEnCatalogo(ctx.catalogos.roles, valores.rol);
    if (!rolOpt)
        error("Rol empresa", valores.rol ? `No hay ningún rol empresa que se llame «${valores.rol}».` : "Falta el rol empresa.");
    // ── Con qué contrato ──────────────────────────────────────────────────────────────────────────
    const contratoOpt = buscarEnCatalogo(ctx.catalogos.contratos, valores.contrato);
    if (!contratoOpt)
        error("Tipo de contrato", valores.contrato ? `No hay ningún tipo de contrato que se llame «${valores.contrato}».` : "Falta el tipo de contrato.");
    ...
    if (valores.convenio && !convenioOpt)
        error("Convenio", `No hay ningún convenio que se llame «${valores.convenio}».`);
    if (!esServicios && contratoOpt && !convenioOpt)
        error("Convenio", "Falta el convenio: es lo que define qué categorías se pueden dar de alta.");
    const categoriaOpt = buscarEnCatalogo(ctx.catalogos.categorias, valores.categoria);
    if (valores.categoria && !categoriaOpt)
        error("Categoría", `No hay ninguna categoría que se llame «${valores.categoria}».`);
    if (!esServicios && contratoOpt && !categoriaOpt)
        error("Categoría", "Falta la categoría.");
    ...
    if (valores.empresa && !empresaOpt)
        error("Empresa del contrato", `No hay ninguna empresa que se llame «${valores.empresa}».`);
    // Sin empresa en la fila: la del proyecto, si tiene una sola. Con varias, hay que decir cuál.
    const empresasDelProyecto = (proyecto?.contratoEmpresas || []).map((id) => String(id));
    const empresaId = empresaOpt?.id || (empresasDelProyecto.length === 1 ? empresasDelProyecto[0] : "");
    // ── Cuándo ────────────────────────────────────────────────────────────────────────────────────
    const desde = fechaISO(valores.desde);
    if (!desde)
        error("Desde", valores.desde ? `«${valores.desde}» no es una fecha (usá dd/mm/aaaa).` : "Falta la fecha de inicio.");
    const hasta = fechaISO(valores.hasta);
    if (valores.hasta && !hasta)
        error("Hasta", `«${valores.hasta}» no es una fecha (usá dd/mm/aaaa).`);
    if (!hasta && contratoOpt && !esIndeterminado)
        error("Hasta", `«${contratoOpt.etiqueta}» no es de tiempo indeterminado: necesita fecha de baja.`);
    if (desde && hasta && hasta < desde)
        error("Hasta", "La fecha de baja es anterior a la de alta.");
    // ── Cómo trabaja ──────────────────────────────────────────────────────────────────────────────
    const diasPorSemana = Number(soloDigitos(valores.diasPorSemana));
    const topeDias = Number(contratoOpt?.extra?.diasPorSemana) || 7;
    if (!diasPorSemana)
        error("Días por semana", "Falta cuántos días por semana trabaja.");
    else if (diasPorSemana > topeDias)
        error("Días por semana", `«${contratoOpt?.etiqueta}» admite hasta ${topeDias} días por semana.`);
    const rotativos = clave(valores.rotativos).startsWith("s");
    const dias = diasDeTexto(valores.dias);
    if (!dias)
        error("Días que trabaja", valores.dias ? `No se entiende «${valores.dias}». Escribilos separados por coma: Lu,Ma,Mi,Ju,Vi.` : "Faltan los días que trabaja.");
    else if (!rotativos && diasPorSemana && dias.length !== diasPorSemana)
        error("Días que trabaja", `Marcaste ${dias.length} día(s) y trabaja ${diasPorSemana}. Con días rotativos pueden ser más; si no, tienen que ser los mismos.`);
    else if (rotativos && diasPorSemana && dias.length < diasPorSemana)
        error("Días que trabaja", `Rota entre ${dias.length} día(s) y trabaja ${diasPorSemana}: tienen que ser al menos ${diasPorSemana}.`);
    ...
    if (valores.entrada && !entrada)
        error("Entrada", `«${valores.entrada}» no es una hora (usá HH:MM).`);
    if (valores.salida && !salida)
        error("Salida", `«${valores.salida}» no es una hora (usá HH:MM).`);
    if (!entrada || !salida)
        error("Entrada", "Falta el horario y el turno elegido tampoco lo trae: cargá entrada y salida.");
    // ── Cuánto ────────────────────────────────────────────────────────────────────────────────────
    const importe = numero(valores.importe);
    if (importe === null || importe <= 0)
        error("Importe por jornada", valores.importe ? `«${valores.importe}» no es un importe válido.` : "Falta el importe por jornada.");
    // ── Reemplazo ─────────────────────────────────────────────────────────────────────────────────
    ...
    if (cuilReemplazado && !reemplazado)
        error("Reemplaza a (CUIL)", `No hay nadie en la plataforma con el CUIL ${valores.reemplazaA}.`);
    const motivoOpt = buscarEnCatalogo(ctx.catalogos.motivos, valores.motivoReemplazo);
    if (valores.motivoReemplazo && !motivoOpt)
        error("Motivo del reemplazo", `No hay ningún motivo que se llame «${valores.motivoReemplazo}».`);
    if (cuilReemplazado && !motivoOpt)
        error("Motivo del reemplazo", "Marcaste un reemplazo: indicá por qué falta la persona reemplazada.");
    if (errores.length > 0)
        return { ok: null, errores };
    return {
        errores: [],
        ok: {
            fila,
 ...
 */
export const crearSolicitud = async (datos, tenantId, creadaPor, rolPorDefecto) => {
