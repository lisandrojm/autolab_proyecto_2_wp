...
const LIMITE = 25;
const vacio = (motivo) => ({ renombrados: [], confirmados: [], sinResolver: [], motivoSinIntentar: motivo });
export async function nombresPorPantalla(opts) {
    ...
    */
    ...
    const ups = await UserProject.find({ userId: { $in: [...porCuil.values()].map((u) => u._id) } })
        .select("userId contracts.empresaContratoId")
        .lean();
    const empresaDeUsuario = new Map();
    for (const up of ups) {
    ...
    const porEmpresa = new Map(); // empresaId → cuils
    const sinResolver = [];
    ...
    const empresas = await Company.find({ _id: { $in: [...porEmpresa.keys()] } })
    ...
    try {
        for (const [empresaId, cuilsDeLaEmpresa] of porEmpresa) {
            ...
            if (empresaCuit.length !== 11) {
                for (const c of cuilsDeLaEmpresa)
                    sinResolver.push({ cuit: c, motivo: "Su empleadora no tiene CUIT cargado: sin eso no se puede elegir en ARCA." });
                continue;
            }
            ...
            for (const item of r.items || []) {
                ...
                */
                ...
                const despues = await User.findById(u._id).select("metadata.nombreValidadoArcaAt").lean();
                if (despues?.metadata?.nombreValidadoArcaAt)
                    confirmados.push(cuil);
                else
                    sinResolver.push({ cuit: cuil, motivo: `ARCA muestra «${nombreArca}» y el apellido guardado no aparece ahí. Corregilo a mano: puede ser otro apellido, no otra forma de escribirlo.` });
            }
        }
    }
    catch (e) {
