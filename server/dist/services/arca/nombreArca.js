 ...
 */
export function partirConAncla(nombreEntero, apellidoGuardado) {
    ...
    const ancla = String(apellidoGuardado || "")
        .split(/\s+/)
        .filter(Boolean);
    if (palabras.length === 0 || ancla.length === 0 || ancla.length >= palabras.length)
        return null;
    const iguales = (a, b) => a.length === b.length && a.every((x, i) => clave(x) === clave(b[i]));
