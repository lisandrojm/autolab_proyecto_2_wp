 ...
 */
export async function condicionFiscalDeCuit(tenantId, cuit, opts = {}) {
    const tid = String(tenantId);
    if (opts.refrescar)
        olvidarCacheFiscal(tid, cuit);
    else {
    ...
    try {
        ...
        if (!cfg)
            condicion = condicionFiscalFallida("La conexión con ARCA no está configurada");
        else {
