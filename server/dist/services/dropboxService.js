...
export function getTenantDropboxConfig(tenant) {
    const d = tenant?.integrations?.dropbox;
    if (!d?.appKey || !d?.appSecretEnc || !d?.refreshTokenEnc)
        return null;
    return {
        appKey: d.appKey,
...
export function isTenantDropboxConnected(tenant) {
...
async function getAccessToken(tenantId, cfg) {
...
async function rpc(tenantId, cfg, endpoint, body) {
 ...
 */
export async function verifyAccount(tenantId, cfg) {
 ...
 * `path/not_found`.
 */
export async function listFolder(tenantId, cfg, path, full = false) {
...
export async function moveEntry(tenantId, cfg, fromPath, toPath) {
...
export async function createFolder(tenantId, cfg, path) {
...
export function isWithinRoot(cfg, path) {
    if (!path)
        return true; // vacío = root
