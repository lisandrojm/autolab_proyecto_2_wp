...
export async function listarPlantillas(acc, projectId) {
    ...
    if (acc.alcance === "personal" && !idOk(projectId))
        return [];
    const lista = await PlantillaEquipo.find({ tenantId, activo: true, ...filtroDeAlcance(acc), ...(acc.alcance === "personal" ? { projectId: oid(projectId) } : {}) })
        .sort({ nombre: 1 })
        .lean();
    return lista.map((p) => ({
        _id: String(p._id),
...
export async function crearPlantilla(acc, body) {
    ...
    if (!general && (!idOk(body?.projectId) || !(await Project.exists({ _id: oid(body.projectId), tenantId }))))
    ...
    if (general)
        Object.assign(comunes, { empresaContratoId: null, convenioId: null, areaShiftAssignments: [] });
    try {
        ...
        if (Array.isArray(body.integrantes) && body.integrantes.length)
            await agregarIntegrantes(acc, String(p._id), body.integrantes);
...
export async function usarGeneral(acc, generalId, projectId, nombre) {
    const tenantId = acc.tenantId;
    if (!idOk(projectId) || !(await Project.exists({ _id: oid(projectId), tenantId })))
    ...
    for (let n = 0; n < 20; n++) {
        ...
        try {
            const copia = await PlantillaEquipo.create({
                ...
                integrantes: (g.integrantes || []).map((i) => ({ ...i, _id: new Types.ObjectId(), userId: null, reemplazadoDePersonaId: null, reemplazadoEl: null })),
 ...
 */
export async function agregarIntegrantes(acc, id, nuevos) {
    const tenantId = acc.tenantId;
    const p = await cargar(acc, id);
    ...
    if (acc.alcance === "general" && conPersona.length)
        throw new ErrorPlantilla(400, "Una plantilla general no lleva personas: sólo puestos por rol. La gente se asigna en la copia de cada supervisor.");
 ...
 */
export async function actualizarIntegrante(acc, id, integranteId, body) {
    const tenantId = acc.tenantId;
    const p = await cargar(acc, id);
...
export async function quitarIntegrante(acc, id, integranteId) {
    const tenantId = acc.tenantId;
    const p = await cargar(acc, id);
 ...
 */
export async function reemplazarIntegrante(acc, id, integranteId, nuevoUserId) {
    const tenantId = acc.tenantId;
    const p = await cargar(acc, id);
    const i = p.integrantes.find((x) => String(x._id) === String(integranteId));
    if (!i)
        throw new ErrorPlantilla(404, "Ese integrante ya no está en la plantilla.");
    if (acc.alcance === "general")
        throw new ErrorPlantilla(400, "Una plantilla general no lleva personas.");
    if (!idOk(nuevoUserId))
