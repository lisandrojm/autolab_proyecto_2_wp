...
import { calcularEscalaGrupo, compararConActa, redondearCentavos } from "../utils/escalaCalculo.js";
 ...
 */
export function armarPeriodo(d) {
    const desde = aIsoFecha(d.desde);
    if (!desde)
        throw new DatoDeEscalaInvalido("La fecha de inicio de vigencia es obligatoria (formato AAAA-MM-DD).");
    const hasta = d.hasta ? aIsoFecha(d.hasta) : "";
    if (d.hasta && !hasta)
        throw new DatoDeEscalaInvalido("La fecha de fin de vigencia no se entiende (formato AAAA-MM-DD).");
    if (hasta && hasta < desde)
        throw new DatoDeEscalaInvalido(`La vigencia termina (${hasta}) antes de empezar (${desde}).`);
    const basico = Number(d.basico || 0);
    if (!(basico > 0))
        throw new DatoDeEscalaInvalido("El básico tiene que ser mayor que cero: una escala en cero no es una escala, es un dato sin cargar.");
    ...
    const diferencias = compararConActa(calculada, acta);
    const rige = (delActa, calculado) => (delActa == null ? calculado : redondearCentavos(Number(delActa)));
 ...
 */
export async function motivoDeSuperposicion(convenio, grupo, desde, hasta, excluirId) {
    const existentes = await EscalaPeriodo.find({ convenio, grupo: grupo ?? null }).select("desde hasta").lean();
    const otros = existentes.filter((p) => String(p._id) !== String(excluirId || ""));
 ...
 */
export async function cerrarPeriodosAbiertos(convenio, grupo, desdeDelNuevo) {
    const corte = diaAnterior(desdeDelNuevo);
    if (!corte)
        return 0;
    const r = await EscalaPeriodo.updateMany({ convenio, grupo: grupo ?? null, hasta: null, desde: { $lt: new Date(`${desdeDelNuevo}T00:00:00.000Z`) } }, { $set: { hasta: new Date(`${corte}T00:00:00.000Z`) } });
 ...
 */
export async function espejarVigenteEnGrupo(convenio, grupo, hoy) {
    if (grupo == null)
        return { grupo, espejado: false, motivo: "El convenio no usa grupos: no hay dónde espejar." };
    const periodos = await EscalaPeriodo.find({ convenio, grupo }).lean();
    const vigente = periodoVigente(periodos, hoy);
    if (!vigente)
        return { grupo, espejado: false, motivo: `Ningún período rige al ${hoy}: la escala del grupo queda como está.` };
    const destino = await ConvenioGrupo.findOne({ convenio, numero: grupo }).select("_id");
    if (!destino)
        return { grupo, espejado: false, motivo: `El convenio ${convenio} no tiene cargado el grupo ${grupo}.` };
    await ConvenioGrupo.updateOne({ _id: destino._id }, {
        $set: {
...
export async function escalaDelConvenioAFecha(convenio, fecha) {
    ...
    return out.sort((a, b) => Number(a.grupo ?? 0) - Number(b.grupo ?? 0));
}
/**
 ...
 */
export async function adicionalesAFecha(convenio, fecha, capitulo) {
    const filtro = { convenio, isActive: true };
    if (capitulo)
        filtro.capitulo = capitulo;
...
export async function pequenasEmpresasAFecha(convenio, fecha) {
    const filas = await EscalaPequenasEmpresas.find({ convenio }).lean();
    const porGrupo = new Map();
    for (const f of filas)
        porGrupo.set(f.grupo, [...(porGrupo.get(f.grupo) || []), f]);
    const out = [];
    for (const lista of porGrupo.values()) {
        const vigente = periodoVigente(lista, fecha);
        if (vigente)
 ...
 */
export function desvioJornadaAdicional(semana, jornada) {
...
export const esElVigente = (periodo, hoy) => rigeEn(periodo, hoy);
