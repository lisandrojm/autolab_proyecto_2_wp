 ...
 */
export async function ensureDefaultAreas(tenantId) {
 ...
 */
export async function ensureAllTenantsHaveDefaultAreas() {
    try {
        const { Tenant } = await import("../models/Tenant.js");
        const tenants = await Tenant.find({});
        console.log(`[AreaInit] Verifying ${tenants.length} tenants have default areas...`);
        for (const tenant of tenants) {
            await ensureDefaultAreas(new Types.ObjectId(tenant._id));
        }
