...
/*
  ...
  `CalificacionesPanel` es el contenido sin marco: la ficha del usuario lo muestra en su pestaña
  «Calificaciones»; `CalificacionesModal` lo envuelve en un modal para el resto.
*/
interface PanelProps {
  ...
  /** Quién es la persona: si cambia, el panel se recarga. */
  userKey: string;
}

interface Props extends Omit<PanelProps, "userKey"> {
  isOpen: boolean;
  onClose: () => void;
  nombre: string;
}
...
export const CalificacionesPanel: React.FC<PanelProps> = ({ cargar, calificar, onCalificada, ayuda, empezarCalificando, userKey }) => {
...
export const CalificacionesModal: React.FC<Props> = ({ isOpen, onClose, nombre, ...panel }) => {
  if (!isOpen) return null;
  return (
    <div className="fixed inset-0 z-[1000] flex items-end justify-center bg-black/50 p-0 sm:items-center sm:p-4" onClick={onClose}>
      <div className="flex max-h-[90vh] w-full max-w-lg flex-col rounded-t-2xl bg-white shadow-xl dark:bg-slate-900 sm:rounded-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-3 border-b border-slate-100 p-5 dark:border-slate-800">
          ...
          <button type="button" onClick={onClose} aria-label="Cerrar" className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800">
            <FontAwesomeIcon icon={faXmark} />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto p-5">
          <CalificacionesPanel {...panel} userKey={nombre} />
        </div>
      </div>
    </div>
  );
