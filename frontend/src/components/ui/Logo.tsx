...
export const Logo: React.FC<LogoProps> = ({ sizeClass = "text-3xl", wrapperClassName = "flex items-center cursor-pointer hover:opacity-80 transition-opacity", to = "/clients" }) => {
  const appName = import.meta.env.VITE_APP_NAME;

  return (
    <Link to={to} className={wrapperClassName}>
      <h1 className={`${sizeClass} font-bold text-gray-900 dark:text-white`}>
        <span className="text-primary-600 dark:text-primary-400">{appName}</span>
