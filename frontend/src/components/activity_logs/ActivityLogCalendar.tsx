...
export const ActivityLogCalendar: React.FC<ActivityLogCalendarProps> = ({ project, scheduleConfig, onDayClick, complianceByDate, dayLabels, controlledMonth, onMonthChange }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded shadow-sm border border-gray-200 dark:border-gray-700 p-3 sm:p-4">
      {/* Calendar Header */}
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          ...
          {scheduleConfig && (
            <span className="hidden sm:inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded text-xs font-medium bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 border border-blue-100 dark:border-blue-800">
        ...
        <div className="flex space-x-1 sm:space-x-2">
          <button onClick={prevMonth} className="p-1.5 sm:p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded transition-colors">
            <FontAwesomeIcon icon={faChevronLeft} className="text-gray-600 dark:text-gray-400 h-3 w-3 sm:h-4 sm:w-4" />
          </button>
          <button onClick={nextMonth} className="p-1.5 sm:p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded transition-colors">
      ...
      <div className="grid grid-cols-7 gap-0.5 sm:gap-1">
        ...
        {days.map((day) => {
          ...
          return (
            <div key={day.toString()} onClick={handleDayClick} className={`h-14 sm:h-20 lg:h-24 p-1 sm:p-2 rounded-md sm:rounded border ${statusColor} ${isToday ? "ring-2 ring-blue-500" : ""} relative group transition-all hover:shadow-md ${status !== "none" ? "cursor-pointer" : "cursor-default"} overflow-hidden flex flex-col justify-between`}>
      ...
      <div className="mt-4 sm:mt-6 flex flex-wrap gap-6 text-sm text-gray-600 dark:text-gray-400 justify-start">
        <div className="flex items-center gap-2.5">
          <div className="w-4 h-4 rounded bg-green-500 shadow-sm ring-2 ring-green-100 dark:ring-green-900/30"></div>
