...
export function ReleaseViewerModal({ release, isOpen, onClose }: ReleaseViewerModalProps) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [pdfUrl, setPdfUrl] = useState<string | null>(null);
  const docxContainerRef = useRef<HTMLDivElement>(null);

  const kind = getFileKind(release?.fileName);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        setLoading(true);
        setError(null);
        const blob = await releasesAPI.getFileBlob(release);
        if (cancelled) return;
  ...
  const handleDownload = () => {
    if (release) releasesAPI.download(release).catch(() => {});
  };
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={release ? `${release.name}` : "Visualizar archivo"}
      subtitle={release?.fileName}
      size="full"
      zIndex={60}
      footer={
        ...
        {!loading && !error && kind === "other" && (
          <div className="flex flex-col items-center justify-center py-20 text-center gap-3">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-300">
              Este formato no se puede previsualizar{release?.fileName ? ` (${release.fileName.split(".").pop()?.toUpperCase()})` : ""}. Descargá el archivo para verlo.
            </p>
            <button type="button" onClick={handleDownload} className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-blue-600 hover:text-blue-700 dark:text-blue-400">
              <FontAwesomeIcon icon={faDownload} className="h-4 w-4" />
              Descargar archivo
