...
import { faCheck, faTimes, faClock, faRotateLeft, faTrash, faCommentDots } from "@fortawesome/free-solid-svg-icons";
 ...
 * se pidió, para cuándo, por cuánto y en qué estado quedó. Lo único que cambia es de dónde salen las
 * filas (las de un proyecto contra todas) y si hace falta la columna Cliente / Proyecto.
 *
 * Vive acá porque la parte difícil no es la tabla, es resolver los datos: el rol frame viene en tres
 * formas distintas según quién creó la solicitud, la categoría y el trámite impositivo se resuelven
 * contra catálogos que hay que cargar, y los estados tienen su propio ciclo. Duplicar eso es
...
export const useCatalogosDeSolicitudes = () => {
  ...
  return useMemo(() => {
    ...
    const resolverCategoria = (id?: string | null) => (id ? categoriasSat.find((c) => c._id === id)?.name || "Sin categoría" : "Sin categoría");
    ...
    return { roleFrames, categoriasSat, estados, resolverRolFrame, resolverCategoria, resolverTramite };
...
interface SolicitudesTableProps {
  ...
  /** Columna Cliente / Proyecto: sobra adentro de un proyecto, hace falta en la vista global. */
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700 text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider">
              ...
              {mostrarProyectos && <th className="px-4 py-3 font-semibold">Cliente / Proyecto</th>}
              ...
              return (
                ...
                */
                <tr key={s._id} onClick={onVerDetalle ? () => onVerDetalle(s) : undefined} className={`border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors ${onVerDetalle ? "cursor-pointer" : ""} ${filaApagada ? "opacity-60 bg-gray-50/60 dark:bg-gray-900/30" : ""}`}>
                        ...
                        <div className="flex flex-col gap-1">
                          {s.proyectos.map((p) => (
                            <span key={p._id} className="text-xs text-gray-700 dark:text-gray-300 whitespace-nowrap">
                              {p.clienteNombre && <span className="text-gray-400">{p.clienteNombre} · </span>}
                              {p.name}
                            </span>
                          ))}
                        </div>
                  ...
                  <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{resolverCategoria(s.categoriaSatId)}</td>
                  {/*
                  ...
                  */}
                  ...
                  <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">{s.dailyRate ? `$${s.dailyRate.toLocaleString("es-AR")}` : "-"}</td>
