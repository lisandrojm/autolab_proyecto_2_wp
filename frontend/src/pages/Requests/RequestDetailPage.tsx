...
export const RequestDetailPage: React.FC = () => {
  ...
  const handleCancel = async () => {
    if (!request) return;
    const result = await sweetAlert.confirm("¿Cancelar solicitud?", "¿Estás seguro de que deseas cancelar esta solicitud?");
  ...
  return (
    ...
    >
      <div className="space-y-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
          <div className="flex items-start justify-between mb-6">
            <div>
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{getTypeLabel(request.typeKey)}</h2>
              <span className={`px-3 py-1 rounded text-sm font-medium ${statusBadge.bg} ${statusBadge.text}`}>{statusBadge.label}</span>
            </div>
              ...
              <div className="flex gap-2">
                {canManage && (
                  <>
                    ...
                    <button onClick={() => setRejectModalOpen(true)} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded transition-colors">
                ...
                {isOwnRequest && !canManage && (
                  <button onClick={handleCancel} className="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded transition-colors">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-gray-900 dark:text-white">{employee ? `${employee.firstName} ${employee.lastName}` : "N/A"}</p>
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Total: {request.daysCount || 0} días</p>
            </div>
            ...
            {approver && (
              <div>
                <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">{request.status === "approved" ? "Aprobado por" : "Evaluado por"}</h3>
                <p className="text-gray-900 dark:text-white">
                  {approver.firstName} {approver.lastName}
                </p>
                <p className="text-sm text-gray-600 dark:text-gray-400">{approver.email}</p>
                {request.approvedAt && <p className="text-xs text-gray-500 dark:text-gray-500 mt-1">{new Date(request.approvedAt).toLocaleString()}</p>}
              </div>
            )}
            ...
            {request.status !== "pending" && (
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  <p className="font-medium text-gray-900 dark:text-white">{request.status === "approved" ? "Aprobada" : request.status === "rejected" ? "Rechazada" : "Cancelada"}</p>
                  {(request.approvedAt || request.rejectedAt) && <p className="text-sm text-gray-600 dark:text-gray-400">{new Date(request.approvedAt || request.rejectedAt || "").toLocaleString()}</p>}
                  {approver && (
      ...
      <RequestApproveModal isOpen={approveModalOpen} onClose={() => setApproveModalOpen(false)} onConfirm={confirmApprove} request={request} />

      <RequestRejectModal isOpen={rejectModalOpen} onClose={() => setRejectModalOpen(false)} onConfirm={confirmReject} request={request} />
    </PageLayout>
  );
};
