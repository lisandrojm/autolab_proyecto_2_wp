  ...
  */
  ...
  const fetchAllRoleFrames = async () => {
    try {
      ...
      const arrayData = Array.isArray(roleFrames) ? roleFrames : roleFrames && Array.isArray((roleFrames as any).data) ? (roleFrames as any).data : [];
  ...
  const handleConfirmarCuenta = async (user: User) => {
    const result = await sweetAlert.confirm('¿Confirmar cuenta creada?', 'Confirmás que la cuenta bancaria fue creada y que los datos están cargados tanto en la plataforma como en el banco.', 'Sí, confirmar');
    if (!result.isConfirmed) return;
    try {
  ...
  const handleConfirmarCambio = async (user: User) => {
    const result = await sweetAlert.confirm('¿Confirmar cambio de datos bancarios?', 'Confirmás que el cambio de datos bancarios solicitado ya fue aplicado en el banco/FRAME.', 'Sí, confirmar');
    if (!result.isConfirmed) return;
    try {
    ...
    <PageLayout
      ...
      headerActions={
        <div className="flex items-center gap-3">
          {canManage && (
            <button onClick={openCreate} title="Nuevo usuario" aria-label="Nuevo usuario" className="inline-flex items-center gap-2 px-2 py-2 text-sm font-semibold rounded-lg bg-blue-600 text-white hover:bg-blue-700">
                ...
                <div className="space-y-6 animate-fadeIn transition-opacity duration-300">
                  <div className="flex justify-between items-start pb-4 border-b border-gray-100 dark:border-gray-800">
                    <div className="flex items-center gap-4">
                      <div className="w-16 h-16 rounded-full bg-blue-500 flex items-center justify-center text-white text-2xl font-bold shadow-md">{viewUser.firstName?.charAt(0) || viewUser.email.charAt(0).toUpperCase()}</div>
                      <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-4">
                    ...
                    {(() => {
                      const totalDaysCount =
                        (viewUser.metadata?.projects as any[])?.reduce((acc: number, p: any) => {
                      ...
                      return (
                        <div key={upIdx} className="bg-gray-50 dark:bg-gray-900/30 border border-gray-100 dark:border-gray-800 rounded-xl overflow-hidden shadow-sm">
                          ...
                          <div className="bg-white dark:bg-gray-800 px-4 py-3 border-b border-gray-100 dark:border-gray-700 flex flex-col md:flex-row md:items-center justify-between gap-2">
                            ...
                            {up.nombre_rol_frame && <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300 border border-purple-200 dark:border-purple-800 uppercase tracking-tight self-start">{up.nombre_rol_frame}</span>}
                          </div>
                          ...
                          <div className="p-4 space-y-3">
                            {up.contracts && up.contracts.length > 0 ? (
                              up.contracts.map((c: any, cIdx: number) => {
                                ...
                                const hasCoordAssignments =
                                  project?.coordinatorAssignments?.some((asm: any) => {
                                ...
                                if (project && project.coordinatorAssignments) {
                                  const myCoordinated =
                                    project.coordinatorAssignments.filter((asm: any) => {
                                      const uid = typeof asm.userId === 'object' ? asm.userId?._id : asm.userId;
                                      return String(uid) === String(viewUser?._id);
                                    }) || [];
                            ...
                            <div>
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {(() => {
                                  ...
                                  return (
                                    <button type="button" onClick={(e) => showVacationInfo(user._id, fullName, e)} className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold bg-amber-500/10 hover:bg-amber-500/20 text-amber-600 dark:text-amber-400 border border-amber-500/20 hover:border-amber-500/30 transition-all shadow-sm cursor-pointer focus:outline-none" title={dateRangeStr}>
