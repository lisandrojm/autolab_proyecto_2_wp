...
export function ManageVacationsRulesPage() {
  ...
  return (
    <PageLayout title="Configuración Global de Vacaciones" icon={faCalendar} backTo="/manage" helpKey={HELP_KEY}>
      ...
      <InfoModal isOpen={showAntiguedadInfo} onClose={() => setShowAntiguedadInfo(false)} title="Días por antigüedad" content="Define cuántos días de vacaciones corresponden según los años de antigüedad del empleado en la empresa. Por ejemplo: de 0 a 5 años = 18 días, de 6 a 10 años = 21 días." />

      <InfoModal isOpen={showMaxDiasGozadosInfo} onClose={() => setShowMaxDiasGozadosInfo(false)} title="Máximo de días gozados por año" content="Establece el límite máximo de días de vacaciones que un empleado puede tomar en un año calendario, independientemente de su saldo acumulado." />

      <InfoModal isOpen={showDiasBeneficioInfo} onClose={() => setShowDiasBeneficioInfo(false)} title="Días de beneficio adicional" content="Días extra de vacaciones otorgados como beneficio especial, sumados a los días anuales estándar." />

      <InfoModal isOpen={showArrastreInfo} onClose={() => setShowArrastreInfo(false)} title="Arrastre de días no utilizados" content="Permite que los días de vacaciones no utilizados en un año puedan trasladarse al siguiente año, con límites opcionales." />

      <InfoModal isOpen={showMaxDiasArrastreInfo} onClose={() => setShowMaxDiasArrastreInfo(false)} title="Máximo de días a arrastrar" content="Cantidad máxima de días de vacaciones no utilizados que pueden trasladarse al año siguiente." />

      <InfoModal isOpen={showVencimientoInfo} onClose={() => setShowVencimientoInfo(false)} title="Vencimiento del arrastre" content="Plazo en días desde el inicio del año para utilizar los días arrastrados del año anterior. Después de este plazo, los días no utilizados se pierden." />

      <InfoModal isOpen={showMinDiasInfo} onClose={() => setShowMinDiasInfo(false)} title="Mínimo de días por solicitud" content="Cantidad mínima de días consecutivos que debe solicitarse en cada petición de vacaciones." />

      <InfoModal isOpen={showMaxDiasCorridosInfo} onClose={() => setShowMaxDiasCorridosInfo(false)} title="Máximo de días corridos" content="Límite de días consecutivos (incluyendo fines de semana) que pueden tomarse en una sola solicitud." />

      <InfoModal isOpen={showMaxDiasHabilesInfo} onClose={() => setShowMaxDiasHabilesInfo(false)} title="Máximo de días hábiles" content="Límite de días laborables consecutivos que pueden tomarse en una sola solicitud, sin contar fines de semana." />

      <InfoModal isOpen={showAnticipacionInfo} onClose={() => setShowAnticipacionInfo(false)} title="Anticipación mínima" content="Cantidad mínima de días de anticipación con los que el empleado debe solicitar sus vacaciones antes de la fecha de inicio." />

      <InfoModal isOpen={showFraccionadasInfo} onClose={() => setShowFraccionadasInfo(false)} title="Vacaciones fraccionadas" content="Permite dividir el período de vacaciones en múltiples solicitudes a lo largo del año, en lugar de tomarlo todo de una vez." />

      <InfoModal isOpen={showPdfTemplateInfo} onClose={() => setShowPdfTemplateInfo(false)} title="Plantilla PDF" content="Selecciona la plantilla de documento que se utilizará para generar el PDF de aprobación de vacaciones. Puedes gestionar las plantillas desde la sección de administración." />
    </PageLayout>
  );
}
