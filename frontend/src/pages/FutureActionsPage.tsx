...
export default function FutureActionsPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const [actionsResponse, statsData] = await Promise.all([
        getFutureActions(params),
        getFutureActionStats(),
      ]);
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-slate-600 dark:text-slate-400">
          Gestiona las acciones pendientes asociadas a pedidos
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-4 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select
              value={filterEstado}
              onChange={(e) => setFilterEstado(e.target.value as EstadoAccion | "all")}
              className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 py-2 text-slate-900 dark:text-slate-100"
            >
          ...
          <div>
            ...
            <select
              value={filterTipo}
              onChange={(e) => setFilterTipo(e.target.value as TipoAccionFutura | "all")}
              className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 py-2 text-slate-900 dark:text-slate-100"
            >
              <option value="all">Todos</option>
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start gap-3 flex-1">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        >
                          {estadoAccionLabels[action.estadoAccion]}
                        </span>
                        <span className="px-2 py-1 rounded-full text-xs font-medium bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                          {tipoAccionFuturaLabels[action.tipoAccionFutura]}
                        </span>
                      </div>
                      <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-2">
                        {action.descripcionAccion}
                      </h3>
                      <div className="space-y-1 text-sm text-slate-600 dark:text-slate-400">
                        <p>
                          <span className="font-medium">Responsable:</span>{" "}
                          {responsableAccionLabels[action.responsableAccion]}
                        </p>
                        {action.fechaLimite && (
                          <p>
                            <span className="font-medium">Fecha límite:</span>{" "}
                            {formatDate(action.fechaLimite)}
                            {daysRemaining !== null && (
                              <span
                                className={`ml-2 ${
                                  daysRemaining < 0
                              ...
                              >
                                ({daysRemaining < 0 ? "Vencido" : `${daysRemaining} días restantes`})
                              </span>
                            )}
                          </p>
                        )}
                        {action.documentoRequerido && (
                          <p>
                            <span className="font-medium">Documento requerido:</span>{" "}
                            {action.documentoRequerido}
                          </p>
                        )}
                        {action.plazoDias && (
                        ...
                        <p>
                          ...
                          {formatDate(action.fechaCreacionAccion)}
                        </p>
                      </div>
                    </div>
                  </div>
                  {action.estadoAccion === "pendiente" && (
                    <button
                      onClick={() => handleMarkCompleted(action._id)}
