...
const statusBadge = (status: string): "success" | "warning" | "blue" | "info" | "default" => {
  switch (status) {
    case "active":
    case "completed":
      return "success";
...
export const ProjectDetailPage: React.FC = () => {
  ...
  const filteredCampaigns = useMemo(() => {
    ...
    return campaigns.filter((c) => {
      ...
      // fecha (por createdAt). Si preferís por timeline, cambia `c.createdAt` por `c.timeline.startDate` o el rango que quieras.
      let dateOk = true;
      if (hasDates) {
  ...
  const submitCreateCampaign = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        // timeline en ISO; si tu backend espera YYYY-MM-DD, mandar date-only
        timeline: {
  ...
  const logoSrc = client?.brandKit?.logo ? (client.brandKit.logo.startsWith("http") ? client.brandKit.logo : `${import.meta.env.VITE_ASSETS_URL ?? ""}${client.brandKit.logo}`) : undefined;
    ...
    <PageLayout
      ...
      headerActions={
        <button onClick={openEditProject} className="btn-primary flex items-center justify-center text-sm p-2 gap-2">
          ...
          <span className="hidden lg:block">Editar Proyecto</span>
        </button>
      }
      preSearchContent={
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          <Card
            ...
            header={{
              title: "Objetivos / Público",
              icon: faBullseye,
              badges: [
                {
                  text: project.objectives && project.objectives.length > 0 ? "Definido" : "Pendiente",
                  variant: project.objectives && project.objectives.length > 0 ? "success" : "warning",
                },
              ],
            }}
          ...
          >
            {project.objectives?.length ? (
              <div className="space-y-3">
                <div>
                  <label className="text-xs font-medium text-gray-500 dark:text-gray-400">Objetivos</label>
                  <ul className="mt-1 space-y-1">
                    {project.objectives.slice(0, 2).map((o, i) => (
                    ...
                    {project.objectives.length > 2 && <li className="text-xs text-gray-500 dark:text-gray-500">+{project.objectives.length - 2} más</li>}
                  </ul>
                </div>
                {project.targetAudience && (
                  <div>
                    <label className="text-xs font-medium text-gray-500 dark:text-gray-400">Audiencia</label>
                    <p className="text-sm text-gray-900 dark:text-white mt-1 line-clamp-2">{project.targetAudience}</p>
                  </div>
                )}
              </div>
            ) : (
          ...
          <Card
            ...
            header={{
              ...
              badges: [
                {
                  text: project.budget?.total ? "Definido" : "Pendiente",
                  variant: project.budget?.total ? "success" : "warning",
                },
              ],
            }}
            onClick={openEditProject}
          >
            {project.budget?.total ? (
              <div className="text-center">
                <p className="text-2xl font-bold text-gray-900 dark:text-white">USD{(project.budget.total ?? 0).toLocaleString()}</p>
                <p className="text-sm text-gray-500 dark:text-gray-500">Total del proyecto</p>
              </div>
            ) : (
          ...
          <Card
            ...
            header={{
              ...
              badges: [
                {
                  text: `${campaigns.length} campaña${campaigns.length !== 1 ? "s" : ""}`,
                  variant: campaigns.length > 0 ? "success" : "default",
                },
              ],
            }}
          >
            <div className="space-y-3">
              <div className="flex justify-between">
                <span className="text-sm text-gray-600 dark:text-gray-400">Campañas:</span>
                <span className="text-sm font-medium text-gray-900 dark:text-white">{campaigns.length}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-600 dark:text-gray-400">Briefs:</span>
                <span className="text-sm font-medium text-gray-900 dark:text-white">{briefs.length}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-600 dark:text-gray-400">Creado:</span>
                <span className="text-sm font-medium text-gray-900 dark:text-white">{new Date(project.createdAt).toLocaleDateString()}</span>
              </div>
            </div>
          </Card>
        </div>
      ...
      preSearchActions={
        <button onClick={openCreateCampaign} className="btn-primary flex items-center justify-center text-sm p-2 gap-2">
          ...
          <span className="hidden lg:block">Nueva Campaña</span>
        </button>
      }
      searchAndFilters={
        campaigns.length > 0 ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <select value={campaignForm.status} onChange={(e) => setCampaignForm((p) => ({ ...p, status: e.target.value as Campaign["status"] }))} className="input-field">
        ...
        {filteredCampaigns.map((c) => (
          <Card
            ...
            header={{
              ...
              icon: faBullhorn,
              badges: [{ text: statusText(c.status), variant: statusBadge(c.status) }],
            }}
            footer={{
              leftContent: (
