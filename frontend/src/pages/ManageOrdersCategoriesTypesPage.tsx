...
export const ManageOrdersCategoriesTypesPage: React.FC = () => {
  ...
  const handleToggleActive = async (requestType: RequestTypeData) => {
    try {
      ...
      sweetAlert.success(requestType.isActive ? "Tipo desactivado" : "Tipo activado", "El tipo de pedido se actualizó correctamente");
      fetchRequestTypes();
    } catch (error: any) {
  ...
  const handleDelete = async (requestType: RequestTypeData) => {
    ...
    const result = await sweetAlert.confirm("¿Eliminar tipo de pedido?", "¿Estás seguro de eliminar este tipo de pedido?");
        ...
        content: (
          <form id="request-type-form" onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input type="text" required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="input-field" placeholder="Ej: Licencias médicas" />
            </div>
            {!editingType && (
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Clave *</label>
                <input type="text" required value={formData.key} onChange={(e) => setFormData({ ...formData, key: e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, "_") })} className="input-field" placeholder="Ej: medical_leave" pattern="[a-z0-9_]+" />
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Solo letras minúsculas, números y guiones bajos</p>
              </div>
            )}
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Descripción</label>
              <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} rows={3} className="input-field" placeholder="Descripción del tipo de pedido..." />
            </div>
          </form>
        ),
      }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {requestTypes.map((requestType) => (
            <Card
              ...
              footer={{
                leftContent: <span className="text-xs text-gray-500">{requestType.isDeletable ? "Eliminable" : "No eliminable"}</span>,
                actions: [
            ...
            >
              <p className="text-sm text-gray-600 dark:text-gray-400">{requestType.description || "Sin descripción"}</p>
            </Card>
          ))}
        </div>
