...
export default function BottomNav({ currentView, onNavigate }: BottomNavProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 z-10 w-full border-t border-slate-200 dark:border-slate-800 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-sm">
        ...
        {navItems.map(({ id, icon: Icon, label }) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-xs ${isActive ? 'font-bold' : 'font-medium group-hover:font-bold'}`}>
                {label}
              </span>
            </button>
          );
