          ...
          <div className="flex justify-between align-top">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">{getUserRole(order.userId)}</p>
              </div>
            </div>
            <div className="flex flex-col gap-3">
              <span className={`inline-flex items-center gap-1.5 text-xs font-medium py-1 px-3 rounded-full ${badge.style}`}>{badge.label}</span>
            </div>
          </div>
          ...
          <div className="bg-slate-100 dark:bg-slate-700/50 p-4 rounded-lg">
            <div className="flex justify-between items-start mb-2">
              <div className="flex justify-between items-center w-full">
                <p className="font-semibold text-xl text-slate-800 dark:text-slate-100">{order.title}</p>
                <p className="text-sm text-slate-500 dark:text-slate-400">Nº Pedido: {getOrderNumber(order)}</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            {order.amount && (
              <div>
                ...
                <p className="font-medium text-slate-800 dark:text-slate-100">${order.amount.toFixed(2)} USD</p>
              </div>
            )}
