          ...
          <div className="space-y-3">
            {loading ? (
              ...
              vacations.map((vacation) => (
                <div key={vacation._id} onClick={() => handleVacationClick(vacation)} className="bg-white dark:bg-slate-900/70 rounded-xl p-4 shadow-sm border border-slate-200 dark:border-slate-800 cursor-pointer hover:border-blue-200 dark:hover:border-blue-800 transition-colors">
                  <div className="flex items-center justify-between gap-2 mb-3">
                    <span className="px-2 py-1 rounded bg-slate-100 dark:bg-slate-800 text-xs font-medium text-slate-500 dark:text-slate-400">{vacation.vacationNumber || vacation._id.slice(-6).toUpperCase()}</span>
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    ...
                    <span className="px-3 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-xs font-medium">{vacation.daysRequested} días</span>
                  </div>

                  <p className="text-xs text-slate-400 dark:text-slate-500">Solicitado el: {format(parseISO(vacation.createdAt), "dd/MM/yyyy")}</p>
                </div>
              ))
            ) : (
