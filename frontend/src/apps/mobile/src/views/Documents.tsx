      ...
      <div className="px-4 pt-6">
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          {documentTypes.map((type) => (
            <button
              key={type.value}
              className="flex flex-col items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/70 p-4 transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98]"
            >
          ...
          {documents.map((doc) => (
            <div key={doc.id} className="bg-white dark:bg-slate-900/70 rounded-xl p-4 shadow-sm">
              <div className="flex items-start gap-4">
                <div
                  className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl ${getDocumentBg(
                    doc.type
                  )}`}
                >
