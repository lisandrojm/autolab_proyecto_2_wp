import { CalendarAPI } from "../mocks/types";

let events: CalendarAPI[] = [
  {
    _id: "event_001",
...
export const mockCalendarService = {
  getCalendarEvents: async (): Promise<CalendarAPI[]> => {
    await new Promise((resolve) => setTimeout(resolve, 300));
    return [...events];
  },

  getCalendarEvent: async (id: string): Promise<CalendarAPI | null> => {
    await new Promise((resolve) => setTimeout(resolve, 200));
    return events.find((e) => e._id === id) || null;
  },

  getCalendarEventsMonth: async (year: number, month: number): Promise<CalendarAPI[]> => {
    await new Promise((resolve) => setTimeout(resolve, 300));
  ...
  createCalendarEvent: async (data: Partial<CalendarAPI>): Promise<CalendarAPI> => {
    await new Promise((resolve) => setTimeout(resolve, 500));
    const newEvent: CalendarAPI = {
      _id: `event_${Date.now()}`,
      title: data.title || "",
  ...
  updateCalendarEvent: async (id: string, data: Partial<CalendarAPI>): Promise<CalendarAPI | null> => {
    await new Promise((resolve) => setTimeout(resolve, 500));
