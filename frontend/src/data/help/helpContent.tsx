...
const helpResources = {
  es: {
    help: {
      ...
      'empresaObrasSociales.items': ['**Es un subconjunto**: el catálogo tiene 494 obras sociales y cada empleadora declara las suyas. Una que no esté acá hace que ARCA rechace el alta.', '**El listado real sale del padrón**: en ARCA, logueado con este CUIT, Datos del Empleador → Obras Sociales. Acá se refleja cuáles son; hay que repetirlo con cada empleadora.', '**La obra social de cada persona NO se elige acá**: la define su convenio. Lo único que se decide en esta pantalla es la de los **excluidos de convenio (9999/99)**, que no tienen sindicato del que heredarla.', '**Orden en que se resuelve**: la propia de la persona → la excepción que esta empleadora haya puesto para ese convenio → la del convenio → la de excluidos → la global del catálogo.', '**Se guarda en lote**: los cambios se aplican con **Guardar cambios**, a diferencia del nomenclador de Configuración → ARCA, donde cada registro se guarda al instante.'],
      ...
      'empresaConvenios.items': ['**Definen qué categorías se pueden dar de alta**: ARCA solo ofrece las de los convenios registrados por este CUIT. Sin convenio no hay categoría posible y el alta no se puede generar.', '**La obra social viene del convenio**: la define el sindicato, y al sindicato lo define el CCT. Se carga una sola vez en Configuración → ARCA → Convenios y vale para todas las empresas.', '**Excepción por empleadora**: con el ✎ de la fila se puede usar OTRA obra social para ese convenio, solo en esta empresa. La tabla la marca como *pisada por esta empresa*.', '**El 🗑 no borra el convenio**: lo quita de esta empleadora. El registro maestro sigue en el nomenclador.', '**Se guarda en lote**: nada se aplica hasta apretar **Guardar cambios**, ni siquiera la excepción cargada en el modal (queda marcada como *sin guardar*).'],
