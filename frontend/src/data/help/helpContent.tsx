...
const helpResources = {
  es: {
    help: {
      ...
      'valoraciones.description': 'Los niveles comerciales de la productora (Bronce, Plata, Oro…). Cada proyecto tiene uno, y eso decide qué categoría del convenio se le ofrece al contratar: en un proyecto de nivel bajo, la más barata.',
      'valoraciones.items': ['**De dónde sale la del proyecto**: del **margen (%)** del proyecto, según el rango de cada valoración. Se carga en la ficha del proyecto, en la tarjeta *Valoración del Proyecto*.', '**Por defecto**: una sola valoración la tiene. Es la que recibe **todo proyecto que se crea** —a mano o desde FRAME—, y la de cualquiera cuyo margen no esté cargado o no entre en ningún rango.', '**Fijada a mano**: en el formulario del proyecto se puede elegir una en vez de *Automática*. Queda fija: cambiar el margen no la mueve, hasta volver a *Automática*.', '**Rangos**: el *Desde* entra en el nivel y el *Hasta* no, pasa al siguiente (con Plata hasta 20 y Oro desde 20, un 20 % es Oro). Dos valoraciones activas no pueden pisarse: no habría forma de saber cuál corresponde.', '**Orden**: la jerarquía, de menor a mayor —arriba el nivel más bajo—. Se cambia con *Ordenar*, arrastrando.', '**El cruce con Roles Empresa**: en cada función, cada categoría SAT tiene su valoración (la más barata de cada convenio, Plata; las demás, Oro). Al contratar se ofrecen solo las de la valoración del proyecto, y si queda una sola, se elige sola.', '**Elegir otra igual**: con *Elegir de todas formas* se ven las demás, pero hay que dejar un **motivo**, que queda guardado en el contrato.', '**Funciones sin valorar**: se ofrecen todas sus categorías, como siempre. Pasa con las que tienen una sola categoría, o todas al mismo sueldo: no hay una más barata que elegir.', '**Contratos ya hechos**: cambiar el margen no los toca. Los que quedan con otra valoración aparecen como *desalineados* en la ficha del proyecto, para revisarlos uno por uno.', '**No se borra una en uso**: se pasa a *Inactiva*. Deja de ofrecerse en lo nuevo y sigue valiendo en los proyectos y contratos que ya la tienen.', '**Color**: el del tag del nivel, que se ve en proyectos y al contratar.'],
