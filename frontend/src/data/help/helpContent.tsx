...
const helpResources = {
  es: {
    help: {
      ...
      'roles.items': ['**Un rol es un conjunto de permisos**, y cada permiso destapa una pantalla. Quien tenga el rol ve eso y nada más.', '**Dos secciones, cada una con su interruptor**: *Plataforma* (lo del escritorio) y *App Mobile* (las tarjetas de la app: Novedades, Pedidos, Vacaciones y Contratación). Un rol nuevo arranca siendo de la app; la plataforma se prende aparte.', '**No se puede guardar un rol sin permisos**: no dejaría entrar a ningún lado.', '**Rol por defecto**: es el que reciben las altas —los usuarios importados de FRAME y los que entran por el link de registro—, así que conviene que abra la app.', '**Quién supervisa turnos necesita *Novedades***: sin ese permiso no tiene dónde cargar las de su área, y el sistema no deja sacárselo hasta liberarle la supervisión desde el equipo del proyecto.'],
      ...
      'orders.description': 'Módulo para administrar las solicitudes realizadas por los Supervisores y Colaboradores. Aquí el área administrativa puede aprobar, rechazar y monitorear el estado de cada pedido.',
      ...
      // Valoraciones
      'valoraciones.title': 'Cómo funcionan las valoraciones',
      'valoraciones.description': 'Los niveles comerciales de la productora (Bronce, Plata, Oro…). El proyecto toma el suyo del MARGEN; cada categoría, de su BRUTO. Al contratar se cruzan: en un proyecto de nivel bajo se ofrece la categoría más barata.',
      'valoraciones.items': ['**De dónde sale la del proyecto**: del **margen (%)** del proyecto, según el rango de cada valoración. Se carga en el formulario del proyecto (al lado de *Valoración*) o en la ficha, en la tarjeta *Valoración del Proyecto*.', '**Por defecto**: una sola valoración la tiene. Es la que recibe **todo proyecto que se crea** —a mano o desde FRAME—, y la de cualquiera cuyo margen no esté cargado o no entre en ningún rango.', '**Fijada a mano**: en el formulario del proyecto se puede elegir una en vez de *Automática*. Queda fija mientras el margen no cambie: **si se cambia el margen, vuelve a *Automática*** y se recalcula con el nuevo.', '**Rangos**: el *Desde* entra en el nivel y el *Hasta* no, pasa al siguiente (con Plata hasta 20 y Oro desde 20, un 20 % es Oro). Dos valoraciones activas no pueden pisarse: no habría forma de saber cuál corresponde.', '**Orden**: la jerarquía, de mayor a menor —arriba el nivel más alto, con orden 1—. Se cambia con *Ordenar*, arrastrando. En las categorías decide qué le toca a cada nivel: al de abajo, la más barata.', '**Las categorías se valoran por BRUTO**: el margen define la valoración del proyecto; en las categorías de cada función, lo que la define es el sueldo. De menor a mayor: dentro de cada convenio, la más barata toma el nivel más bajo, la siguiente el que sigue, y las que sobran el más alto.', '**Dónde se aplica**: en Roles Empresa, al tildar una categoría la valoración se pone sola (y se puede cambiar a mano). La columna *Categorías · bruto* de esta tabla dice qué le toca a cada nivel.', '**Al contratar**: se ofrecen solo las categorías de la valoración del proyecto, y si queda una sola, se elige sola. En un proyecto del nivel más bajo, la más barata.', '**Elegir otra igual**: con *Elegir de todas formas* se ven las demás, pero hay que dejar un **motivo**, que queda guardado en el contrato.', '**Sin valorar**: las categorías sin elección por precio —la única de su convenio, o todas al mismo sueldo, o sin bruto cargado— quedan sin valorar y se ofrecen siempre, como antes.', '**Contratos ya hechos**: cambiar el margen no los toca. Los que quedan con otra valoración aparecen como *desalineados* en la ficha del proyecto, para revisarlos uno por uno.', '**No se borra una en uso**: se pasa a *Inactiva*. Deja de ofrecerse en lo nuevo y sigue valiendo en los proyectos y contratos que ya la tienen.', '**Color**: el del tag del nivel, que se ve en proyectos y al contratar.'],
...
const HELP_CONTENT: Record<HelpKey, HelpEntry> = {
  ...
  valoraciones: { title: 'Valoraciones', size: 'md', content: buildHelpContent('valoraciones') },
};
